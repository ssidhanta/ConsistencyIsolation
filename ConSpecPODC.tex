...
% \rightarrow \;  \exists S_p \\ \left( {R^i}^{'} F {R^j}^{'} \in S_p \right) \big). 
 \mathcal{E}^{s} = \forall  \mathit{st} \in \mathit{St}, {R}^{'}, {R}^{''} \in \mathit{st} \big( {R}^{'} F {R}^{''} \in \mathit{st}
 \rightarrow \\ \left( {R}^{'} \preccurlyeq {R}^{''} \right) \big), 
 %; \\
...
\end{split}
\end{align} where $\left( \mathcal{O}, \preccurlyeq \right)$ constrains  the order among conflicting reads in an equivalent $S_p$.
