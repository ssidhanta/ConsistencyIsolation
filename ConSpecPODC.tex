...
% solation level.
 %\begin{itemize}
 %\item
 It present the syntax and design of ConSpec, a language for formally specifying the consistency
...
%  models.
   ...
   A legal serialization $\mathit{Ser}$ of a given global execution is represented by a sequence of propositional variables ${\mathit{Op}^i}^{'}$, each variable ${\mathit{Op}^i}$ denoting an execution of an
  ...
  \big(\not\exists {\mathit{Op}^k}^{'} = {W^k}^{'} \left( {\mathit{Op}^i}^{'}  F {\mathit{Op}^k}^{'} F {\mathit{Op}^j}^{'} \right) \big) \wedge \left( {v_j}^{'} = {v_i}^{'} \right) \big)$.
   ...
   We formally define \emph{serialization of a client}, $S_c$, as the restriction of a legal serialization of a global execution
   ...
   corresponds to operations observed in a particular session trace $\mathit{st}$.
    $S_c$ = $ \Set{ {\mathit{Op}^i}^{'} } {{\mathit{Op}^i}^{'} \in \mathit{Ser} \wedge {\mathit{Op}^i}^{'} \in \mathit{st} } $.
...
%because of the absence of notions of $B$ and $C$. Those consistency models are specified as a conjunction of multiple restricting conditions.
   The consistency models, namely Read Your Write, Monotonic Read, Causal consistency, Write Follows Read, and Monotonic
   Write are
  defined in terms of constraints on an equivalent legal serialization $S_p$, which comprises all operations performed by
  the given client, along with write operations from other clients. Hence for these consistency models, $E^s$ comprises a
   conjunction of one or more LTL constraints on operations comprised in $S_p$. The consistency models Strict
   Serializability is defined in terms of LTL constraints on the order among operations performed each client in a global
   executions with respect to an equivalent legal serialization of a client $S_c$. For Strict serializablity, $E^s$ is
   represented by a
   conjunction of one
   or more LTL constraints on operations comprised in $S_c$. %Montonic Write consistency is defined in terms of LTL
...
%   constraints on operations comprised in $S$.
 Processor consistency and Total Store Order, which are not defined in terms
   of legal serialization, imposes constraints on operations comprised in the global execution composed of operations
   performed by one or more concurrent clients. Hence, for Processor consistency and Total Store Order $E^s$ is a
   conjunction of LTL constraints on the global execution $\mathit{St}$.
...
%  \end{definition}\label{def:parser}
 ...
 \section{List of ConSpec Specifications for Various Consistency and Isolation Levels}
  ...
  We use $S_p$, specified in Definition \ref{def:parser}, to denote a legal serialization of all operations comprising a given client
...
% \wedge \left( {v^j}^{'} = {v^k}^{''} \right) \right).
  \mathcal{E}^{s} = \forall  \mathit{St}, \mathit{st} \in \mathit{St}, {W^i}^{'}, {R^j}^{'} \in \mathit{st}
...
\mathcal{E}^{s} = \forall \mathit{St}, \mathit{st} \in \mathit{St}, {R^i}^{'}, {R^j}^{'} \in \mathit{st} \big( {R^i}^{'} F {R^j}^{'} \in \mathit{st}
...
E^{s} = \forall \mathit{St}, \mathit{st} \in \mathit{St}, {\mathit{Op}^i}^{'}, {\mathit{Op}^j}^{'} \in \mathit{st} \\ \big( \left( {\mathit{Op}^i}^{'} F {\mathit{Op}^j}^{'} \in \mathit{st} \right)
...
E^{s} =  \forall \mathit{St}, \mathit{st} \in \mathit{St}, {\mathit{Op}^i}^{'}, {\mathit{Op}^j}^{'} \in \mathit{st} \big( \mathit{Op}^i X^o {\mathit{Op}^j}^{'} \in \mathit{st}
\rightarrow
...
E^{s} =  \forall \mathit{St}, \mathit{st} \in \mathit{St}, {R^i}^{'}, {W^j}^{'} \in \mathit{st} \big( \left( {R^i}^{'} F {W^j }^{'} \in \mathit{st} \right)
...
\begin{split}
E^{s} = \forall \mathit{St}, \mathit{st} \in \mathit{St}, {W^i}^{'}, {W^j}^{'} \in \mathit{st} \big( {W^i}^{'} F {W^j}^{'} \in \mathit{st}  \rightarrow
...
%\\ \wedge  \not\exists R^k_\mathit{tx} \in \mathit{st} \left( W^i_\mathit{tx} F W^j_\mathit{tx} F R^k_\mathit{tx} \in \mathit{st} \right) \wedge \left( W^j_\mathit{tx} F R^k_\mathit{tx} \in \mathit{st} \right) \\ \wedge \left( v^k = v^i \right) \big),
 E^s = C = \forall  \mathit{St}, \mathit{st} \in \mathit{St}, \mathit{tx}, \mathit{ty} \in \mathit{St}, x, y,\\
...
E^{r} = C = \forall  \mathit{St},  \mathit{st}, \mathit{st}^{'} \in  \mathit{St}, \mathit{tx}, \mathit{ty} \in  \mathit{St} \\ \big(
 % \mathit{tx} F \mathit{ty} \rightarrow
...
\section{Rewriting CAP Theorem in terms of ConSpec}
 ...
 guarantees to favor  availability and partition tolerance according to the use case and system requirements. The
...
\begin{definition}
 It is impossible to satisfy a consistency model that enforces a total order among
   operations in a global execution, comprising operations performed by all clients in the system,
  while simultaneously providing high availability and partition tolerance.
\end{definition}
 In other words, if the specification $E^s$ of a consistency model, as per the generalized syntax in Definition \ref{def:form0},
 imposes a total order on $\mathit{St}$, a particular consistency model is determined to be CAP-strong. The above condition, in turn,
 implies that $\preccurlyeq$ must be a total order, instead of partial order, i.e., if Condition 1 in  Definition \ref{def:form0} can be replaced by
 ...
 must hold for all operations
  $\mathit{op}^i$ and $\mathit{op}^j$ comprised in the global execution, represented by the observed global session trace
  $\mathit{St}$.
\par \textbf{Proof:} The above theorem is proved by contradiction as follows. Let us consider a pair of concurrent clients, namely $\mathit{Cl}_1$ and $\mathit{Cl}_2$ executing
...
%p2 that are managing a single shared variable x with a global agreement on the order of writes.
 ...
 in $N_1$) applies a new
sequence of writes $\beta$ onto $x$ while $\mathit{Cl}_2$ (located in $N_2$) applies a different sequence of writes $\gamma$
to $x$. Since
these components are disconnected, none of these writes are propagated among the servers $N_1$ and $N_2$.
 ...
 for the conflicting object $x$; whereas  $\mathit{St}$, as observed by $\mathit{Cl}_2$,  is a sequence $\alpha, \gamma$.
  Since $\beta \not= \gamma$, this contradicts the requirement of a total order on the
operations comprised in $\mathit{St}$.
...
equivalent legal serialization $S_p$, composed of operations performed by the given client, along with only
writes from other concurrent clients. Thus, writes from concurrent clients are allowed to overwrite results of writes from
...
comprised of all operations in a global execution.
...
% eventually operator $F$ or special-purpose next operator $X^o$) among each pair of operations.
 TSO and PC
 imposes restrictions on the execution order and observed results among all operations performed by all clients observed in a global session trace $\mathit{St}$.
