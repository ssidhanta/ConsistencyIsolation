...
This allows us to define a shorthand notation for a session trace $\mathit{st} \in \mathcal{S}_t$ as a sequence of tuples  $\tuple{w, {v}}$ or
 $\tuple{r, {v}^{'}}$.
In our formulas, when a quantifiers restricts an operation to a given type (read or write), we simply us the letters R and W, i.e., we write $\forall R \in \mathcal{O}$ as a shorthand for $\forall r(x){v} \in \mathcal{O}$.
...
is defined and implemented by Lloyd et al.~\cite{Lloyd:2011:DSE:2043556.2043593}. It states that in addition to preserving the causality condition of causal consistency, writes from multiple clients on conflicting objects can not observe divergent results. We formalise this definition in terms of ConSpec  as follows. 
...
E^{s} = \forall \mathit{st}, \mathit{st}^{'}, \in \mathcal{S}_t, {\mathit{Op}}^{'}, {\mathit{Op}}^{''} \in \mathit{st} \big(  {\mathit{Op}}^{'} F _ \mathit{st} {\mathit{Op}}^{''} 
 \Rightarrow {\mathit{Op}}^{'} \preccurlyeq^{'}_{\mathit{st}+w} {\mathit{Op}}^{''}\big).  
  \end{split}
  \end{align}
 %\begin{align*}
...
\exists \mathit{st}\in \mathcal{S}_t:\mathit{Op}^{'} F  {\mathit{Op}}^{''} \in \mathit{st})
