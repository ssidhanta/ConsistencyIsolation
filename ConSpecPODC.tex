...
To remain available and partition tolerant, the system must continue to serve requests uninterruptedly from the clients even in the presence of the partition.  Thus, each of the  servers $N_1$ and  $N_2$ must immediately perform the operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ from the clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$, respectively,   and return the results, without waiting for the write from the other server to reach it. 
 ...
 for the conflicting object $x$. %; whereas  $\mathcal{S}_t$, as observed by $\mathit{Cl}_2$,  is a sequence $\alpha, \gamma$.  
  Let operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$  be the prefix of the sequences  $\beta$, and $\gamma$, comprised in $\mathit{st}$ and $\mathit{st}^{'}$, respectively. 
 ...
 %partial orders $\preccurlyeq_{st}$ and $\preccurlyeq_{st}^{'}$ restricted to session traces  $\mathit{st}$ and $\mathit{st}^{'}$ for the clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$, respectively.  Thus, any valid partial order  $ \preccurlyeq$ for  in  $\Pi(\mathcal{S}_t,E^s)$ does not comprise operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ observed from $\mathit{st}$ and $\mathit{st}^{'}$. 
 According to  $\mathcal{C}$, a valid $\preccurlyeq$ for $\mathcal{S}_t$ does not need to comprise both the operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ observed in the session traces $\mathit{st}$ and $\mathit{st}^{'}$.  
 ...
 Because of the above disruption of  communication, results of operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ observed in the session trace  $\mathit{st}$  for $\mathit{Cl}_1$  are not propagated among the servers  $N_1$ and  $N_2$. 
 %Hence, the partial order partial order $\alpha  \preccurlyeq_{st} \beta$, whereas $\mathit{st}^{'}$ observes a  partial order $\preccurlyeq_{st}$ = $\alpha  \preccurlyeq_{st} \gamma$.  
  That is,  $\preccurlyeq$ is given as either $\preccurlyeq$ = $\alpha  \preccurlyeq_{st} \beta$ or $\alpha  \preccurlyeq_{st} \gamma$.  Since prefixes of $\beta$ and $\gamma$ are $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$, respectively, $ \beta \not= \gamma$. Hence, the partial order $\preccurlyeq$  does not consist both operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$. 
 %$\preccurlyeq$ is either for the session $\mathit{st}$   does not comprise the operations  $\mathit{op}^{'}(x,v^{'})$ from the other  session  $\mathit{st}$.  Similarly, the partial order $\preccurlyeq_{st}^{'}$  for the session $\mathit{st}^{'}$ does not comprise operation  $\mathit{op}(x,v)$ from a different  session  $\mathit{st}$.  
$\preccurlyeq$ is valid according to  $\mathcal{C}$ since $\mathcal{C}$ does not need  a valid partial order to  constrain operations from different sessions. 
