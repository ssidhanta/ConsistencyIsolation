...
\affiliation{%
  \institution{NOVA LINCS, Universidade NOVA Lisboa \& SUSE Linux GmbH}
%  \city{Prague}
...
%  \institution{University of Minnesota}
  \country{Portugal}}
\author{Rodrigo Miragaia Rodrigues}
\email{rdias@gsd.inesc-id.pt}
\affiliation{%
  \institution{INESC-ID/IST}
%  \city{Charlottesville}
%  \state{VA}
%  \postcode{22903}
  \country{Portugal}
...
if there exists a partial order $\left( \mathcal{O}, \preccurlyeq \right)$ over the set $\mathcal{O}$ comprising operations  present in all session traces in $\mathit{St}$, where  $\mathcal{O} = \bigcup_{st \in St} \{ o \mid o^{'} \in st \},$  
...
% the same irrespective of the
 ...
 and 2) $\left( \mathcal{O}, \preccurlyeq \right)$ obeys $E^s$, which is an LTL expression restricting $\left( \mathcal{O}, \preccurlyeq \right)$.
\end{definition}
...
Condition 1, when applied to a system whose interface consists only of read and write, translates to a requirement that every read operation in $St$ must return the value of the most recent write according to $\preccurlyeq$.
  Condition 2 can be expressed as $ E^s\; \vDash \; \left( \mathcal{O}, \preccurlyeq \right),$ where %$R$ is a temporal operator, 
...
\section{Rewriting CAP Theorem in terms of ConSpec}\label{sec:cap}
 ...
 various degrees of relaxation gives rise to a lattice of consistency models, ranging from RYW to SC.
