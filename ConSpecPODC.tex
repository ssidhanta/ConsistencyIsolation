...
${\mathit{op}^i}(x), {\mathit{op}^j}(x)$  on an object $x$ in the temporal order specified in the global session trace $\mathit{St}$. Thus, $E^s = \forall x, \mathit{st}, \mathit{st}^{'} \in \mathit{St} \not\exists {\mathit{op}^i}(x), {\mathit{op}^j}(x), {\mathit{op}^k}(x), {\mathit{op}^l}(x) \in \mathcal{O}  \\ \big( \left( {\mathit{Op}^i}^{'}(x) F {\mathit{Op}^j}^{'} (x) \in \mathit{st} \right)  \vee \big(  \left( {\mathit{Op}^i}^{'}(x) F {\mathit{Op}^k}^{'} (x) \in \mathit{st} \right)  \wedge \\ \left( {\mathit{Op}^j}^{'}(x) F {\mathit{Op}^l}^{'} (x) \in \mathit{st}^{'} \right)  \wedge \left( {v^k}^{'} = {v^i}^{'} \right) \wedge \left( {v^j}^{'} = {v^l}^{'} \right)  \big) \big) \rightarrow \\ \left( {\mathit{Op}^i}^{'}(x) F {\mathit{Op}^k}^{'} (x) \in \mathit{Si} \right), $ %, i.e., there must exist a common serialization comprising all operations executed from the
...
where $ \mathit{Si} $ is an equivalent legal serialization for a global execution comprising all clients executing in the system, defined in Equation \ref{def:ser}.. The above expression can be further rewritten in terms of a partial order $ \preccurlyeq(x)$ over a given object $x$ as $E^s =  \forall x, \mathit{st}, \mathit{st}^{'} \in \mathit{St} \not\exists {\mathit{op}^i}(x), {\mathit{op}^j}(x), {\mathit{op}^k}(x), {\mathit{op}^l}(x) \in \mathcal{O}  \\ \big( \left( {\mathit{Op}^i}^{'}(x) F {\mathit{Op}^j}^{'} (x) \in \mathit{st} \right)  \vee \big(  \left( {\mathit{Op}^i}^{'}(x) F {\mathit{Op}^k}^{'} (x) \in \mathit{st} \right)  \wedge \\ \left( {\mathit{Op}^j}^{'}(x) F {\mathit{Op}^l}^{'} (x) \in \mathit{st}^{'} \right)  \wedge \left( {v^k}^{'} = {v^i}^{'} \right) \wedge \left( {v^j}^{'} = {v^l}^{'} \right)  \big) \big) \rightarrow \\ \left( {\mathit{Op}^i}^{'}(x) \preccurlyeq(x) {\mathit{Op}^j}^{'} (x) \right),$
