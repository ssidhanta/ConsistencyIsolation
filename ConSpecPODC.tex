...
E^{s} =  \forall \mathit{st} \in \mathit{St}, {R}^{'}, {W}^{'} \in \mathit{st} \big( \left( {R}^{'} F {W}^{'} \in \mathit{st} \right)
 \rightarrow \\ \left(  {R}^{'} \preccurlyeq {W}^{'} \right) \big), %; \\
...
E^{s} = \forall \mathit{st} \in \mathit{St}, {W}^{'}, {W}^{''} \in \mathit{st} \big( {W}^{'} F {W}^{''} \in \mathit{st}  \rightarrow \\ \left( {W}^{'} \preccurlyeq {W}^{''} \right) \big), %; \\
...
\end{split}
  \end{align} where the partial order $\left( \mathcal{O}, \preccurlyeq \right)$ restricts the order among conflicting writes  in an equivalent $S_p$.
   %\begin{align}\label{eqn:MW}
%\begin{split}
