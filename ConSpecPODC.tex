...
\par \textbf{Proof:} We start by proving the implication in the direction $(\rightarrow)$. Following the proof style of Gilbert et al.~\cite{Gilbert:2002:BCF:564585.564601}, we prove the implication in this direction by contradiction as follows.  Let us assume, by contradiction, that consistency model $E^s$ is implemented by an algorithm that is highly available during partitions but does not meet the condition at the end of the Theorem. Given that $E^s$  does not meet this condition, there there must exist a global session trace $\mathcal{S}_t$ and a pair of operations in distinct sessions, $o_s_1$ and $o_s_2$ where, 
