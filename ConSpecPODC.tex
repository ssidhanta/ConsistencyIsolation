...
\end{split}
 ...
 \par Processor Consistency \cite{Ahamad:1993:PPC:165231.165264} specifies: operations in a client application must execute according to the invocation order with respect to a given processor (client); i.e., result of a preceding operation invoked from a given client must be observed in all processors before result of a
...
\end{split}
  \end{align}  where $ \left( \mathcal{O}, \preccurlyeq_{st} \right)$ and $\left( \mathcal{O}, \preccurlyeq_{st}^{'} \right) $ are partial order sets restricted to operations performed by clients $\mathit{Cl}$ and $\mathit{Cl}^{'}$, respectively, observed in the respective local session traces $\mathit{st}$ and $\mathit{st}^{'}$. % all data objects with respect to a given global execution.
...
%\left( \not\exists {W^m}^{'} F {W^n}^{'} F {\mathit{Op}^j}^{'} \in \mathit{st} \right) \big) \big) \big)$
 ...
 can be given  in terms of a series of LTL expression that express the precedence of successive operations in the sequence.
  The precedence relation between any two successive operations $\mathit{op}$ and $\mathit{op}^{'}$ can be expressed as
   $\mathit{Op}^{'} X \mathit{Op}^{''}$.  Thus, the above condition can be expressed as: any two operations in an execution
   ...
   $\mathit{Op}^{'} X \mathit{Op}^{''} \in \mathit{st}
\rightarrow \exists S_c \left( \mathit{Op}^{'} X \mathit{Op}^{''} \in S_c \right)$. Since ${\mathit{Op}}^{'}  X {\mathit{Op}}^{''}  \in S_c$ denotes a partial order among ${\mathit{Op}}^{'} $ and ${\mathit{Op}}^{''}$, we can rewrite the above expression as  ${\mathit{Op}}^{'}  \preccurlyeq {\mathit{Op}}^{''} $. Thus, Chockler's definition reduces into the specification given in Equation \ref{eqn:Strict}. 
...
${\mathit{op}}(x), {\mathit{op}^{'}}(x)$  on an object $x$ in the temporal order specified in the global session trace $\mathit{St}$.  This, in turn,  implies there must exist a partial order $\preccurlyeq$ over all operations comprised in  $\mathit{St}$. Thus, $E^s = \forall x, \mathit{st}, \mathit{st}^{'} \in \mathit{St} \not\exists {\mathit{op}}(x), {\mathit{op}^{'}}(x), {\mathit{op}^{''}}(x), {\mathit{op}^{'''}}(x) \in \mathcal{O}  \big( \left( {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (x) \in \mathit{st} \right)  \vee \big(  \left( {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{'''} (x) \in \mathit{st} \right)  \wedge \\ \left( {\mathit{Op}}^{''}(x) F {\mathit{Op}}^{''''} (x) \in \mathit{st}^{'} \right)  \wedge \left( {v}^{''} = {v} \right) \wedge \left( {v}^{'} = {v}^{'''} \right)  \big) \big) \rightarrow  
 \left(  {\mathit{Op}}^{'}(x) \preccurlyeq {\mathit{Op}}^{''} (x) \right)$. %, i.e., there must exist a common serialization comprising all operations executed from the
...
%$.
 The specification for Processor consistency (PC) can be directly derived using the above line of reasoning. 
