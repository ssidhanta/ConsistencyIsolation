...
It is possible for a storage system to satisfy a consistency model $E^s$ while simultaneously providing high availability and partition tolerance if and only if for any global session trace $\mathcal{S}_t$ and its valid partial orderings, wen you consider the set of maxima of that partial order, it is always possible to make them depend only on the previous operation in the same session and still obtain a valid partial order, i.e.:
...
\noindent {\sc removeallexceptsession} $(\preccurlyeq,o) \equiv \preccurlyeq \setminus \langle o',o \rangle, \forall o': (... , o', o) \notin \mathcal{S}_t$
