...
if there exists a partial order
 ...
 $\left( \mathcal{O}, \preccurlyeq \right)$ over the set $\mathcal{O}$ comprising operations  present in all session traces in $\mathit{St}$, where  $\mathcal{O} = \bigcup_{st \in St} \{ o \mid o^{'} \in st \},$  
...
%  $\left(\mathit{Op}^1 X^{o} \mathit{Op}^2\right)_{o}^\mathit{po}$.
  ...
  $S_p$ is equivalent to $S_i$, i.e., both notations represent a legal serialization comprising all
   ...
   $W^{'}$ and $R^{''}$.  Let $\preccurlyeq_{st+w}$ denote a partial order comprising all operations from a given session trace along with writes from all other clients. Since $W^{'} F R^{''} \in S_p$ denotes a partial order among $W^{'}$ and $R^{''}$, we can rewrite the above expression as  $W^{'} \preccurlyeq_{st+w} R^{''}$. Thus, Chockler's definition reduces into the specification given in Equation \ref{eqn:RYW}. 
...
%\end{align}.
 ...
 As in the case of RYW, the expressions $\mathit{op}^1 \xrightarrow{\sigma} \mathit{op}^2$,
  and $\mathit{op}^1 \xrightarrow{S} \mathit{op}^2$  can be rewritten as LTL expressions
   $R^{'} F R^{''} \in \mathit{st}$ and $R^{'} F R^{''} \in S_p$, respectively. Again, since $R^{'} F R^{''} \in S_p$ denotes a partial order among $R^{'}$ and $R^{''}$, we can rewrite the above expression as  $R^{'} \preccurlyeq_{st+w} R^{''}j$, thus reducing the above specification  into Equation \ref{eqn:MR}. 
...
%  $\left(\mathit{Op}^1 X^{o} \mathit{Op}^2\right)_{o}^\mathit{po}$.
  ...
  can be expressed in terms of LTL as $R^{'} F W^{''} \in S_p$,  where
  $S_p$ is equivalent to $S_i$. Since $R^{'}  F W^{''}  \in S_p$ denotes a partial order among $R^{'}$ and $W^{''}$, we can rewrite the above expression as  $R^{'}  \preccurlyeq_{st+w} W^{''}$. Thus, Chockler's definition reduces into the specification given in Equation \ref{eqn:WFR}. 
  % Further,
...
\subsection{Derivations of Rest of the ConSpec Specifications}\label{sec:restderive}
 Chockler et al. states the MW consistency model as: if the condition represented by the expression
...
%  $\left(\mathit{Op}^1 X^{o} \mathit{Op}^2\right)_{o}^\mathit{po}$.
  ...
  $S_p$ is equivalent to $S_i$.  Further,
   ...
   $\mathit{Op}^1$ and   $\mathit{Op}^2$ in both Condition 1 and Condition 2 can safely be  replaced  by new
   propositional variables  $W^{'}$ and $W^{''}$. Since $W^{'}  F W^{''}  \in S_p$ denotes a partial order among $W^{'}$ and $W^{''}$, we can rewrite the above expression as  $W^{'}  \preccurlyeq_{st+w} W^{''}$. Thus, Chockler's definition reduces into the specification given in Equation \ref{eqn:MW}. 
