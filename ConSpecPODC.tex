...
\section{Rewriting CAP Theorem in terms of ConSpec}\label{sec:cap}
 ...
 various degrees of relaxation gives rise to a lattice of consistency models, ranging from RYW to TSO.
...
\begin{theorem}[Extended CAP theorem] 
 ...
 the following condition.
  Thus, the condition $\forall {\mathit{Op}}^{'}, {\mathit{Op}^j}^{''} \in \mathit{St}: {\mathit{Op}}^{'} \preccurlyeq {\mathit{Op}}^{''} \vee {\mathit{Op}}^{''} \preccurlyeq {\mathit{Op}}^{'}$
 must hold for operations
  $\mathit{op} \in \mathit{st}$ and $\mathit{op}^{'}\in \mathit{st}^{'}$ comprised in separate session traces $\mathit{st}$ and $\mathit{st}^{'}$ comprising results observed by operations performed by different clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$ . %, represented by the observed global session trace
%  $\mathit{St}$.
