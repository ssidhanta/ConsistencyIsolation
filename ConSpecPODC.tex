...
\section{Analysis of Consistency Models With Respect To CAP}\label{sec:capanalyze}
 According to Theorem \ref{thrm:cap}, a consistency model $\mathcal{C}$   that requires the existence of a valid partial order, hence a valid partial order enumeration, comprising only operations performed from the same session as a sufficient condition, are CAP-weak, i.e., they can be implemented in a highly available and partition tolerant manner.  Inspecting $E^s$-expressions for RYW, MR, WFR, MW,  and Processor Consistency, it can be observed that   all of these expressions specify that precedence order among operations from a session trace $\mathit{st}$ must be preserved in any partial order $\preccurlyeq_{\mathit{st}+w}$ belonging in a valid partial order enumeration $\Pi(\mathcal{S}_t,E^s) $ (refer to Definition \label{def:allpos}) for a valid global session trace $\mathcal{S}_t$ for $E^s$. Since a valid partial order  $\preccurlyeq_{\mathit{st}+w}$ for $E^s$ comprises operations from a given session trace $\mathit{st}$ plus all writes in  $\mathcal{S}_t$. Similarly, the $E^s$-expressions for Causal and Causal+ consistency models restrict the above condition must be respected in each session trace $\mathit{st}$ in $\mathcal{S}_t$, corresponding to each client in the system.  Similarly, Processor consistency (PC) restricts that the execution order among operations in a given client $\mathit{st}$ must always match the invocation order, i.e., no client may observe the operations from $\mathit{st}$ in an order that differs from the invocation order in $\mathit{st}$.   Thus, all of these consistency models are, in fact, CAP-weak. Intuitively, writes from concurrent sessions are allowed to interleave between operations in a given session $\mathit{st}$ in any manner in $\preccurlyeq_{\mathit{st}+w}$, as long as precedence order among operations from  $\mathit{st}$ in $\preccurlyeq_{\mathit{st}+w}$ matches the  precedence order in $\mathit{st}$. These models are not dependent on reliable communication among concurrent clients, like broadcasting of results or meta data, or blocking of conflicting operations Hence, the above consistency models can be implemented in a system without expensive synchronization protocols, which prevent or resolve conflicts among concurrent clients using locks or consensus.  Thus, they can be implemented using asynchronous techniques, like local snapshot or checkpointing, which can preserve correctness of local operations,  independent of global state of the system. Among these, Causal and Causal+ are stongest, since they require some mechanism for merging snapshots of executions from multiple clients. % Under these consistency models,
