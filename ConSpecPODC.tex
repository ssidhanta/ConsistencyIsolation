...
To remain available and partition tolerant, the system must continue to serve requests uninterruptedly from the clients even in the presence of the partition.  Thus, each of the  servers $N_1$ and  $N_2$ must immediately perform the operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ from the clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$, respectively,   and return the results, without waiting for the write from the other server to reach it. 
 ...
 The  system satisfies a consistency model $\mathcal{C}$, defined as $E^s$, which specifies that a valid partial order enumeration  $\Pi(\mathcal{S}_t,E^s)$  for a given global session trace $\mathcal{S}_t$  in the system does not constrain  operations performed by different sessions.  In other words, $\Pi(\mathcal{S}_t,E^s)$ does not comprise a partial order which restricts operations from different sessions. Let us consider that $\Pi(\mathcal{S}_t,E^s)$ comprises partial orders $\preccurlyeq_{st}$ and $\preccurlyeq_{st}^{'}$ restricted to session traces  $\mathit{st}$ and $\mathit{st}^{'}$ for the clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$, respectively.  Thus, any valid partial order  $ \preccurlyeq$ for  in  $\Pi(\mathcal{S}_t,E^s)$ does not comprise operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ observed from $\mathit{st}$ and $\mathit{st}^{'}$. This condition enables the system to remain available despite the disruption in the connection between $N_1$ and $N_2$ in the presence of the partition as follows.  Because of the above disruption of  communication,  $\mathcal{C}$, the session trace  $\mathit{st}$  for $\mathit{Cl}_1$  observes a partial order $\alpha  \preccurlyeq_{st} \beta$, whereas $\mathit{st}^{'}$ observes a  partial order $\preccurlyeq_{st}$ = $\alpha  \preccurlyeq_{st} \gamma$.  In presence of the partition, the partial order $\preccurlyeq_{st}$ for the session $\mathit{st}$   does not comprise the operations  $\mathit{op}^{'}(x,v^{'})$ from the other  session  $\mathit{st}$.  Similarly, the partial order $\preccurlyeq_{st}^{'}$  for the session $\mathit{st}^{'}$ does not comprise operation  $\mathit{op}(x,v)$ from a different  session  $\mathit{st}$.  However, a valid partial order for the consistency model  $\mathcal{C}$ does not need  to constrain operations from different sessions, hence the above partial orders are valid according to $\mathcal{C}$. This, in turn, implies that the corresponding partial orders enumeration $\Pi(\mathcal{S}_t,E^s)$ is valid according to $\mathcal{C}$. 
...
% for the conflicting object $x$. %; whereas  $\mathcal{S}_t$, as observed by $\mathit{Cl}_2$,  is a sequence $\alpha, \gamma$.  
 Let $\mathit{st}$ and $\mathit{st}^{'}$ be  given as a pair of execution sequences  $\alpha, \beta$ and  $\alpha, \gamma$, respectively. Let operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$  be the prefix of the sequences   $\beta$ and $\gamma$, respectively. To be deemed valid according to $\mathcal{C}$, $\preccurlyeq$  must comprise both operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ from$\mathit{st}$ and $\mathit{st}^{'}$. 
%That is, $\preccurlyeq_{st}$ implies ${W}^{'}  \preccurlyeq_{st} {W}^{''}$, and $\preccurlyeq_{st}^{'}$ implies $ {W}^{'}  \preccurlyeq_{st}^{'} {W}^{''}$, or $\preccurlyeq_{st}$ implies ${W}^{''}  \preccurlyeq_{st} {W}^{'}$, and $\preccurlyeq_{st}^{'}$ implies ${W}^{''}  \preccurlyeq_{st}^{'} {W}^{'}$. %, but not both simultaneously.  
  However, in the presence of the partition, results of $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ are not propagated among the disconnected servers, i.e., $\beta \not= \gamma$. Hence, there does not exist a  valid partial orders $\preccurlyeq$ that comprises both operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ from sessions $\mathit{st}$ and $\mathit{st}^{'}$. Thus, in presence of the partition, there does not exist a valid enumeration $\Pi(\mathcal{S}_t,E^s)$. %, observed in the session traces $\mathit{st}$ and $\mathit{st}^{'}$. 
...
%\end{align}.
 ...
 MR is expressed as: if the condition  (Condition 1) $\mathit{op}^1 \xrightarrow{\sigma} \mathit{op}^2$ holds for a given execution sequence $\sigma$, where both $\mathit{op}^1$ and $\mathit{op}^2$ must be read operations,
  ...
  can be expressed in terms of LTL as $R^{'} F R^{''} \in S_p$. 
    Following the same line reasoning as in RYW,  we can rewrite the expression $R^{'} F R^{''} \in S_p$ for Condition 2 as  $R^{'} \preccurlyeq_{\mathit{st}+w} R^{''}j$, thus reducing the above specification  into Equation \ref{eqn:MR}. 
...
%  $\left(\mathit{Op}^1 X^{o} \mathit{Op}^2\right)_{o}^\mathit{po}$.
  ...
  Similarly, the expression $R^{'}  F W^{''}  \in S_p$ for Condition 2 can be rewritten in terms of ConSpec as  $R^{'}  \preccurlyeq_{\mathit{st}+w} W^{''}$,  thus reducing the WFR definition to Equation \ref{eqn:WFR}. 
  % Further,
...
%   propositional variables  $W^{'}$ and $W^{''}$.
 Similarly, the expression $W^{'}  F W^{''}  \in S_p$ for Condition 2  can be rewritten as  $W^{'}  \preccurlyeq_{\mathit{st}+w} W^{''}$,  thus reducing the definition into Equation \ref{eqn:MW}. 
...
% total number of processors. We can express the condition the propositional logic variable $A$ must hold for any store
% $\mathit{store}_n$ with the expression  $\bigcap_{\mathit{processor}_n}^{\mathit{processors}} A$.
Consider that a given client observes write operations $w(x,v), w^{'}(y,v^{'})$ according to a temporal order $W^{'}(x) F W^{''} (y)$ observed in a session trace $\mathit{st}$. Then, no other session trace $\mathit{st}^{'}$ observed by any other concurrent client may observe the operations in an order which violates the temporal order in $\mathit{st}$. This, in turn, implies that the precedence $W^{'}(x) F W^{''} (y)$  must be respected in partial order sets   $\left( \mathcal{O}, \preccurlyeq_{{st}^{'}+w} \right)$ and $\left( \mathcal{O}, \preccurlyeq_{\mathit{st}+w} \right)$ for session traces $\mathit{st}$ and $\mathit{st}^{'}$, respectively. Thus, Processor Consistency can be expressed as 
   $\forall x, y, \mathit{st}, \mathit{st}^{'} \in \mathcal{S}_t  \big( \left( W^{'}(x) F W^{''} (y) \in \mathit{st} \right) \rightarrow  \left(  \left( W^{'}(x) \preccurlyeq_{\mathit{st}+w} W^{''} (y)   \right) \wedge  \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right) \right) \big)$. %\\
%E^{s} = C =   \forall \mathit{st} \in \mathcal{S}_t \not\exists \big( \left(  {W^i}^{'}, {W^j}^{'} \in \mathit{st}  \right)
%\wedge \left( {R^k}^{'}, {R^l}^{'} \in \mathcal{S}_t \right) \big) \\
%\big( \left( {W^i}^{'} F {W^j}^{'} \in \mathit{st} \right) \wedge
