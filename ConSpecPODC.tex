...
Our notation defines special purpose LTL operators as shorthands to longer expressions. These correspond to scope operators
  ...
  E.g., an expression ${w} X^{o} {r}$ denotes that among operations performed on the object
     ${o}$, the execution of the operation $r(o){v_m}$ immediately follows the execution operation $w(o,v_l)$.
%% ; the
...
if there exists a partial order
 ...
 $\left( \mathcal{O}, \preccurlyeq \right)$ over the set $\mathcal{O}$ comprising operations  present in all session traces in $\mathit{St}$, where  $\mathcal{O} = \bigcup_{st \in St} \{ o^i \mid o^i \in st \},$  
 such that 1) for every operation $\mathit{op}^i$ in $\mathit{St}$, its output is equal to the one obtain by executing the sequential specification of a linear extension of the operations preceding $\mathit{op}^i$ in $\preccurlyeq$,
...
%  \not\exists {W^m}^{'} \left( {W^l}^{'} F {W^m}^{'} F {R^j}^{'} \in \mathit{St} \right) \big) \big).
  \end{split}
  \end{align}\label{eqn:RYW} where $st$ is the session from the stanpoint of which the session guarantees are being upheld, and $\preccurlyeq_{st+w}$ denotes the restriction of $\preccurlyeq$ to the elements of $st$ and the write operations of all other clients.
...
% \not\exists  {W^m}^{'} \left( {W^l}^{'} F {W^m}^{'} F {R^j}^{'} \in \mathit{St} \right) \big) \big) \big)
 \\ $E^s = \forall x, \mathit{st}, \mathit{st}^{'} \in \mathit{St} \not\exists {\mathit{op}^i}(x), {\mathit{op}^j}(x), {\mathit{op}^k}(x), {\mathit{op}^l}(x) \in \mathcal{O}  \\ \big( \left( {\mathit{Op}^i}^{'}(x) F {\mathit{Op}^j}^{'} (x) \in \mathit{st} \right)  \vee \big(  \left( {\mathit{Op}^i}^{'}(x) F {\mathit{Op}^k}^{'} (x) \in \mathit{st} \right)  \wedge \\ \left( {\mathit{Op}^j}^{'}(x) F {\mathit{Op}^l}^{'} (x) \in \mathit{st}^{'} \right)  \wedge \left( {v^k}^{'} = {v^i}^{'} \right) \wedge \left( {v^j}^{'} = {v^l}^{'} \right)  \big) \big) \rightarrow  
 \left(  {\mathit{Op}^j}^{''}(x) F {\mathit{Op}^i}^{''} (x) \in \mathit{Si} \right)$,   \\
 %E^r = C = \\
