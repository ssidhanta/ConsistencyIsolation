...
%\newtheorem{definition}{Definition}
  ...
  %%   operation ${\mathit{op}}^i$, comprised in the set of operations $\mathcal{O}$ executed by a given client, observed a global session trace $\mathcal{S}_t$, as follows.
  %%  $\mathit{Ser}$ = $\Set{ {\mathit{Op}^i}^{'} } {{\mathit{Op}^i}^{'} \in \mathit{st}}, $ such that
  %%  $\forall {\mathit{op}^i}^{'}, {\mathit{op}^j}^{'} \in \mathcal{O} \\ \big( {\mathit{Op}^i}^{'} = {W^i}^{'} \wedge {\mathit{Op}^j}^{'} = {R^j}^{'}  \wedge \exists \mathit{st} \in \mathcal{S}_t  \; \big( \big( {\mathit{Op}^i}^{'} F {\mathit{Op}^j}^{'} \in \mathit{st}  \; \wedge \\
...
if there exists a partial order $\left( \mathcal{O}_{St}, \preccurlyeq \right)$ over the set $\mathcal{O}_{St}$ comprising operations  present in all session traces in $\mathcal{S}_t$, where  $\mathcal{O}_{St} = \bigcup_{st \in St} \{ o \mid o^{'} \in st \},$  
 such that 1) for every operation $\mathit{op}$ in $\mathcal{S}_t$, its output is equal to the one obtain by executing the sequential specification of a linear extension of the operations preceding $\mathit{op}$ in $\preccurlyeq$,
...
%\not\exists  {W^n}^{'} \in \mathcal{S}_t \left( {W^m}^{'} F {W^n}^{'} F {\mathit{Op}^j}^{'} \in \mathit{st} \right) \big) \big) \big)
 \end{split}
  ...
  where $st$ is the session in context of session guarantees, and $ \preccurlyeq$ is a partial order over operations across all data objects with respect to a given global session trace $ \mathcal{S}_t$. 
 %\begin{align*}
...
$\forall x, y, \mathit{st}, \mathit{st}^{'} \in \mathcal{S}_t  \big( \left( W^{'}(x) F W^{''} (y) \in \mathit{st} \right) \rightarrow  \left(  \left( W^{'}(x) \preccurlyeq_{st+w} W^{''} (y)   \right) \wedge  \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right) \right) \big)$,
%\end{split}
%  \end{align}
...
%  The condition $\forall \mathit{st}, \mathit{st}^{'} \in \mathcal{S}_t, {\mathit{op}} \in \mathit{st}, {\mathit{op}}^{'} \in \mathit{st}^{'}: {\mathit{Op}}^{'} \preccurlyeq {\mathit{Op}}^{''} \vee {\mathit{Op}}^{''} \preccurlyeq {\mathit{Op}}^{'}$
...
To remain available and partition tolerant, the system must continue to serve requests uninterruptedly from the clients even in the presence of the partition.  Thus, each of the  servers $N_1$ and  $N_2$ must immediately perform the operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ from the clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$, respectively,   and return the results, without waiting for the write from the other server to reach it. 
 ...
 for the conflicting object $x$. %; whereas  $\mathcal{S}_t$, as observed by $\mathit{Cl}_2$,  is a sequence $\alpha, \gamma$.  
  Let operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$  be the prefix of the executions  $\beta$, and $\gamma$ observed by clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$, respectively. 
 The  system satisfies a consistency model $\mathcal{C}$, defined as $E^s$, which specifies that a valid partial order enumeration  $Pi(\mathcal{S}_t,E^s)$  for a given global session trace $\mathcal{S}_t$  in the system does not constrain  operations performed by different sessions.  In other words, $Pi(\mathcal{S}_t,E^s)$ does not comprise a partial order which restricts operations from different sessions. Let us consider that $Pi(\mathcal{S}_t,E^s)$ comprises partial orders $\preccurlyeq_{st}$ and $\preccurlyeq_{st}^{'}$ restricted to session traces  $\mathit{st}$ and $\mathit{st}^{'}$ for the clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$, respectively.  Thus, any valid partial order  $ \preccurlyeq$ for  in  $Pi(\mathcal{S}_t,E^s)$ does not comprise operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ observed from $\mathit{st}$ and $\mathit{st}^{'}$. This condition enables the system to remain available despite the disruption in the connection between $N_1$ and $N_2$ in the presence of the partition as follows.  Because of the above disruption of  communication,  $\mathcal{C}$, the session trace  $\mathit{st}$  for $\mathit{Cl}_1$  observes a partial order $\alpha  \preccurlyeq_{st} \beta$, whereas $\mathit{st}^{'}$ observes a  partial order $\preccurlyeq_{st}$ = $\alpha  \preccurlyeq_{st} \gamma$.  In presence of the partition, the partial order $\preccurlyeq_{st}$ for the session $\mathit{st}$   does not comprise the operations  $\mathit{op}^{'}(x,v^{'})$ from the other  session  $\mathit{st}$.  Similarly, the partial order $\preccurlyeq_{st}^{'}$  for the session $\mathit{st}^{'}$ does not comprise operation  $\mathit{op}(x,v)$ from a different  session  $\mathit{st}$.  However, the above partial orders satisfy the consistency model  $\mathcal{C}$, which does not require a valid partial order to constrain operations from different sessions. 
...
% It is possible for a storage system to satisfy a consistency model that enforces a partial order among results observed by operations performed by the same client executing on the system, while simultaneously providing high availability and partition tolerance.
 \end{theorem}
\par \textbf{Proof:}   Following the approach of the proof to Theorem \ref{thrm:cap}, this theorem is proved  using contradiction as follows.  
We consider two servers $N_1$ and $N_2$ that serve requests from clients  $\mathit{Cl}_1$ and $\mathit{Cl}_2$. We consider that a partition occurs at a particular instant between the servers $N_1$ and $N_2$. 
 ...
 The session traces  $\mathit{st}$ and $\mathit{st}^{'}$ comprise sequences of operations, given as  $\alpha, \beta$ and  $\alpha, \gamma$, respectively.  Again, let the prefix of the executions  $\beta$, and $\gamma$ observed by clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$ be writes
  $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$, respectively.  
 Because of the lack of communication of updates between the servers $N_1$ and $N_2$ serving $\mathit{Cl}_1$ and $\mathit{Cl}_2$ in presence of a partition, results of the operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ are not propagated among the servers $N_1$ and $N_2$.  
 ...
 However,  even while working in such a disconnected mode, the system satisfies the specified consistency model $\mathcal{C}$ despite the partition, because of the following reasons. 
 Consider that the storage system enforces a consistency model $\mathcal{C}$  which specifies  that there exists a partial order enumeration $Pi(\mathcal{S}_t,E^s)$ for a given global session trace $\mathcal{S}_t$  that comprises valid partial orders which restricts operations from different sessions. %Let $\mathit{st}$ and $\mathit{st}^{'}$ for the clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$, respectively. %By definition, apart from transitivity and antisymmetry, a total order $\preccurlyeq$ relation requires satisfaction of the 
%\emph{totality} condition, i.e., 
According to the above condition implied by $\mathcal{C}$, any valid partial order in  $Pi(\mathcal{S}_t,E^s)$ must comprise operations from different sessions. Therefore,  each of the session traces $\mathit{st}$ and $\mathit{st}^{'}$ must observe a partial order $\preccurlyeq$ comprising operations from the two  clients $\mathit{Cl}_1$  and  $\mathit{Cl}_2$. This implies that $\preccurlyeq$ must orders the observed sequences $\alpha, \beta$ and $\alpha, \gamma$ as $\alpha \preccurlyeq \beta$ and $\alpha \preccurlyeq \gamma$. 
 for the conflicting object $x$. %; whereas  $\mathcal{S}_t$, as observed by $\mathit{Cl}_2$,  is a sequence $\alpha, \gamma$.  
  Let operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$  be the prefix of the executions  $\beta$ and $\gamma$, respectively observed in session traces $\mathit{st}$ and $\mathit{st}^{'}$ for  $\mathit{Cl}_1$  and  $\mathit{Cl}_2$, respectively.  Thus, a valid $\preccurlyeq$  must order  operations operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$   from different session traces  $\mathit{st}$ and $\mathit{st}^{'}$. 
%That is, $\preccurlyeq_{st}$ implies ${W}^{'}  \preccurlyeq_{st} {W}^{''}$, and $\preccurlyeq_{st}^{'}$ implies $ {W}^{'}  \preccurlyeq_{st}^{'} {W}^{''}$, or $\preccurlyeq_{st}$ implies ${W}^{''}  \preccurlyeq_{st} {W}^{'}$, and $\preccurlyeq_{st}^{'}$ implies ${W}^{''}  \preccurlyeq_{st}^{'} {W}^{'}$. %, but not both simultaneously.  
  However, in the presence of the partition, results of $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ are not propagated among the disconnected servers, i.e., $\beta \not= \gamma$. Hence, there does not exist a a partial order enumeration comprising one or more valid partial orders $\preccurlyeq$ that observes both operations $\mathit{op}(x,v)$ and $\mathit{op}^{'}(x,v^{'})$ from sessions $\mathit{st}$ and $\mathit{st}^{'}$  in presence of the partition. %, observed in the session traces $\mathit{st}$ and $\mathit{st}^{'}$. 
%   Thus, ${W}^{'}  \preccurlyeq_{st} {W}^{''}$ holds in $\beta$, while ${W}^{''}  \preccurlyeq_{st}^{'} {W}^{'}$ holds in $\gamma$. 
 This contradicts our assumption that $\mathcal{C}$ implies the existence of a valid partial order enumeration order comprising  operations from different sessions.  Hence, $\mathcal{C}$ can not achieve high availability and partition tolerance at the same time.
