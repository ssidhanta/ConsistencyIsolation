...
The remainder of this paper is organized as follows. Section  \ref{sec:definitions} presents the assumed system model, and basic definitions and terminology used in the paper.  Section \ref{sec:syntax} presents the syntax of the ConSpec language, and the general format of specifications based on ConSpec. Section \ref{sec:list} lists the specifications of various  consistency models, expressed using ConSpec.  Section \ref{sec:cap} presents a reformulation of the CAP theorem, and formally proves the same.  Section \ref{sec:impl} presents the design of a prototype automated verification tool that can validate a session trace with respect to a ConSpec specification. In Sections \ref{sec:derive} and \ref{sec:restderive}, we prove equivalence of each of the listed ConSpec specifications with their orginal definitions.    Because of lack of space, we include in the appendix a section (Section \ref{sec:hierarchy}) that presents a hierarchical organizaton of the lattice of consistency models. %Section \ref{sec:capanalyze} analyzes the lattice of consistency models in terms of the restated CAP theorem.  
 Section \ref{sec:ser} comprises ConSpec expressions for the various serialization definitions used in the original consistency definitions.  Section \ref{sec:equiv} illustrates the equivalence between the syntax of ConSpec expressions and the syntax of state-of-the-art  consistency definitions.  %Section \ref{sec:complex} in the appendix comprises a comparison of the complexity of verification using ConSpec with the state-of-the-art verification techniques. 
...
%% superscript $o$ specifies the context of an object $o$ for the enclosed relation; $o$ is equivalent to an operand for a
 %%  storage operation.
  E.g., an expression ${w} X^{o} {r}$ denotes that among operations performed on the object
     ${o}$, the execution of the operation $r(o){v}$ immediately follows the execution operation $w(o,v)$.
%% ; the
...
Given a global session trace $\mathit{St}$,
% representing a global execution of all concurrent clients on a given storage
%system, 
we say  that $\mathit{St}$ satisfies a consistency model $\mathcal{C}$
if there exists a partial order $\left( \mathcal{O}_{St}, \preccurlyeq \right)$ over the set $\mathcal{O}_{St}$ comprising operations  present in all session traces in $\mathit{St}$, where  $\mathcal{O}_{St} = \bigcup_{st \in St} \{ o \mid o^{'} \in st \},$  
 such that 1) for every operation $\mathit{op}$ in $\mathit{St}$, its output is equal to the one obtain by executing the sequential specification of a linear extension of the operations preceding $\mathit{op}$ in $\preccurlyeq$,
...
Condition 1, when applied to a system whose interface consists only of read and write, translates to a requirement that every read operation in $St$ must return the value of the most recent write according to $\preccurlyeq$.
...
We start with the Read Your Writes (RYW) session guarantee, which informally precludes a read operation $r$ from reading a value for object $o$ that precedes a value the same client previously wrote to the same object in the same session.
...
%  \big( \left( {W^i}^{'} F {R^j}^{'} \in \mathit{st} \right)
% \rightarrow \\
% \left(  {W^i}^{'} \preccurlyeq_{st+w} {R^j}^{'} \right)\big),
  \mathcal{E}^{s} = \forall  {W}^{'}, {R}^{'} \in \mathit{st}
  \big( \left( {W}^{'} F {R}^{'} \in \mathit{st} \right)
 \rightarrow 
 \left(  {W}^{'} \preccurlyeq_{st+w} {R}^{'} \right)\big),
...
%  \not\exists {W^m}^{'} \left( {W^l}^{'} F {W^m}^{'} F {R^j}^{'} \in \mathit{St} \right) \big) \big).
  \end{split}
  \end{align}\label{eqn:RYW} where $st$ is the session from the standpoint of which the session guarantees are being upheld, and $\preccurlyeq_{st+w}$ denotes the restriction of $\preccurlyeq$ to the elements of $st$ and the write operations of all other clients.
...
% \mathit{st} \in \mathit{St},
 \mathcal{E}^{s} = \forall   {R}^{'}, {R}^{''} \in \mathit{st} \big( {R}^{'} F {R}^{''} \in \mathit{st}
 \rightarrow \left( {R}^{'} \preccurlyeq_{st+w} {R}^{''} \right) \big), 
 %; \\
...
\end{split}
\end{align} with the same meaning for $st$ and $\preccurlyeq_{st+w}$ as in the previous definition.
...
E^{s} =  \forall  {R}^{'}, {W}^{'} \in \mathit{st} \big( \left( {R}^{'} F {W}^{'} \in \mathit{st} \right)
 \rightarrow \left(  {R}^{'} \preccurlyeq_{st+w} {W}^{'} \right) \big), %; \\
...
with the same meaning for $st$ and $\preccurlyeq_{st+w}$ as in RYW.
% where $st$ denotes the session in context of the session guarantees, and $\preccurlyeq_{st+w}$ denotes the restriction of $\preccurlyeq$ in the context of the elements of $st$ and the write operations performed by all other clients.
 %\begin{align}\label{eqn:WFR}
%\begin{split}
...
E^{s} = \forall  {W}^{'}, {W}^{''} \in \mathit{st} \big( {W}^{'} F {W}^{''} \in \mathit{st}  \rightarrow \left( {W}^{'} \preccurlyeq_{st+w} {W}^{''} \right) \big), %; \\
...
\end{split}
  \end{align} with the same meaning for $st$ and $\preccurlyeq_{st+w}$ as in RYW.
%where $st$ denotes the local session trace, and $\preccurlyeq_{st+w}$ denotes the restriction of $\preccurlyeq$ to the elements of $st$ and the write operations of all other clients. %where the partial order $\left( \mathcal{O}, \preccurlyeq \right)$ restricts the order among conflicting writes  in an equivalent $S_p$.
   %\begin{align}\label{eqn:MW}
%\begin{split}
...
\begin{align}\label{eqn:Causal}
\begin{split}
E^{s} = \forall \mathit{st} \in \mathit{St}, {\mathit{Op}}^{'}, {\mathit{Op}}^{''} \in \mathit{st} \big( \left( {\mathit{Op}}^{'} F {\mathit{Op}}^{''} \in \mathit{st} \right)
...
%\not\exists  {W^n}^{'} \in \mathit{St} \left( {W^m}^{'} F {W^n}^{'} F {\mathit{Op}^j}^{'} \in \mathit{st} \right) \big) \big) \big)
 \end{split}
  ...
  where $st$ is the session in context of session guarantees, and $ \preccurlyeq$ is a partial order over operations across all data objects with respect to a given global session trace $ \mathit{St}$. 
 %\begin{align*}
...
\begin{split}
  E^{s} =  \forall \mathit{st} \in \mathit{St}, {\mathit{Op}}^{'}, {\mathit{Op}}^{''} \in \mathit{st} \big( \mathit{Op}^{'} F  {\mathit{Op}}^{''} \in \mathit{st}
...
%E^{s} =  \forall \mathit{st} \in \mathit{St}, {\mathit{Op}}^{'}, {\mathit{Op}}^{''} \in \mathit{st} \big( \mathit{Op}^{'} X^o {\mathit{Op}}^{''} \in \mathit{st}
...
$\forall x, y, \mathit{st}, \mathit{st}^{'} \in \mathit{St}  \big( \left( W^{'}(x) F W^{''} (y) \in \mathit{st} \right) \rightarrow  \left(  \left( W^{'}(x) \preccurlyeq_{st+w} W^{''} (y)   \right) \wedge  \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right) \right) \big)$,
%\end{split}
%  \end{align}
%  \forall x, y, \mathit{st}, \mathit{st}^{'} \in \mathit{St}  \left( {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (y) \in \mathit{st} \right) \rightarrow  \left( {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (y)  \in \left( \mathcal{O}, \preccurlyeq_{st} \right) \wedge {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (y)  \in \left( \mathcal{O}, \preccurlyeq_{{st}^{'}} \right)  \right),
%\end{split}
%  \end{align} 
   where $ \preccurlyeq_{st+w}$ and $ \preccurlyeq_{{st}^{'}+w}$ are partial orders restricted to operations performed by clients $\mathit{Cl}$ and $\mathit{Cl}^{'}$ and writes from all other clients, respectively, observed in the respective local session traces $\mathit{st}$ and $\mathit{st}^{'}$.  The partial order relations $  \left( W^{'}(x) \preccurlyeq_{st+w} W^{''} (y)   \right)$ and $ \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right)$ can be expressed in terms of the corresponding partial order sets $\left( \mathcal{O}, \preccurlyeq_{st+w} \right)$  and $\left( \mathcal{O}, \preccurlyeq_{{st}^{'}+w} \right)$. By that logic, the condition $ \left( W^{'}(x) \preccurlyeq_{st+w} W^{''} (y)   \right) \wedge  \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right) $ can be expressed in terms of an intersection $\left( \mathcal{O}, \preccurlyeq^{'} \right)$ of the partial order sets  $\left( \mathcal{O}, \preccurlyeq_{st+w} \right)$  and $\left( \mathcal{O}, \preccurlyeq_{{st}^{'}+w} \right)$, where the partial order $ \preccurlyeq^{'} $ represents the relation among elements of the new partial order set formed out of the intersection of $\left( \mathcal{O}, \preccurlyeq_{st+w} \right)$  and $\left( \mathcal{O}, \preccurlyeq_{{st}^{'}+w} \right)$. Thus the  TSO equation can be expressed in terms of the new partial order $ \preccurlyeq^{'} $ as follows. 
    \begin{align}\label{eqn:PC}
\begin{split}
\forall x, y, \mathit{st}, \mathit{st}^{'} \in \mathit{St}  \big( \left( W^{'}(x) F W^{''} (y) \in \mathit{st} \right) \rightarrow  \left( W^{'}(x) \preccurlyeq^{'} W^{''} (y)   \right)  \big),
\end{split}
  \end{align} % all data objects with respect to a given global execution.
...
%  \end{align}, where $\preccurlyeq$ is a composition   $\preccurlyeq =  \preccurlyeq^{'} \circ  ... \circ^{'} \preccurlyeq^{''}$and $\circ, ... \circ^{''}$ are again composition operators, like $\cap$ or $\cup$, over partial order sets (posets) $\preccurlyeq^{'}, ...  \preccurlyeq^{''}$ comprising partial orders defined over all data objects.
 \par Owen et al. \cite{Owens:2009:BXM:1616077.1616107} specifies Total Store Order (TSO) as: a read operation performed by a given client on a given data object can not return the result of write on the same object by a different client unless the former write has been observed by all clients. To understand this, consider an execution comprising write operation $w(x,v)$ performed by a given client application (observed in a local session trace $\mathit{st}^{'}$) on a given data object $x$ in a storage system, followed by a read $r^{'}(x,v^{'})$ on $x$. Another client invokes a read operation $r^{''}(x)$ on the same object $x$  followed by a write $w^{'''}(x,v^{'''})$ on $x$, observed in the session trace $\mathit{st}^{'}$). Under TSO, the read $r^{'}(x,v^{'})$ can not observe the result of $w^{'''}(x,v^{'''})$ before the read $r^{''}(x)$ observes the result of $w(x,v)$. For such an execution must there must exist an equivalent serialization for the operations from each client that orders  $w(x,v)$ before $w^{'''}(x,v^{'''})$. In terms of ConSpec, the above constraint can be restated as:  there must exist partial orders $\preccurlyeq(x)_{{st}^{'}}$ and $\preccurlyeq(x)_{{st}}$ over the operations in the session traces $\mathit{st}$ and $\mathit{st}^{'}$ which preserves the above order among writes to each data object.  %This operation can not succeeded by another operation on the same  object   can not be observed  by any given client (i.e., can not occur in a  given session trace) return a subsequent (or newer) value unless the result of a given write is observed by all client applications, i.e., observed in all  session traces. 
% A session trace for a client application can not view results of a sequence of write operations in an
 %order which differs from the invocation order. \mathit{op}(x), \mathit{op}^{'}(x), \mathit{op}^{''}(x), \mathit{op}^{'''}(x)  %Let $\mathit{stores}$ be an integer term that denotes the total number of stores.
...
% \\ $E^s = \forall x, \mathit{st}, \mathit{st}^{'} \in \mathit{St} \not\exists {\mathit{op}}(x), {\mathit{op}^{'}}(x), {\mathit{op}^{''}}(x), {\mathit{op}^{'''}}(x) \in \mathcal{O} \big( \left( {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (x) \in \mathit{st} \right)  \vee \big(  \left( {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{'''} (x) \in \mathit{st} \right)  \wedge \\ \left( {\mathit{Op}}^{''}(x) F {\mathit{Op}}^{''''} (x) \in \mathit{st}^{'} \right)  \wedge \left( {v}^{''} = {v} \right) \wedge \left( {v}^{'} = {v}^{'''} \right)  \big) \big)  \rightarrow  
...
%  \end{align} 
  %which can be rewritten as follows. 
 $E^s = \forall x, \mathit{st}, \mathit{st}^{'} \in \mathit{St}, w(x,v), r^{'}(x,v^{'}), r^{''}(x), w^{'''}(x,v^{'''}) \in \mathcal{O}  \big( \left( W^{'}(x) F R^{''} (x) \in \mathit{st} \right)  \wedge  \left( \mathit{R}^{'''}(x) F W^{''''} (x) \in \mathit{st}^{'} \right)  \wedge \left( v^{''} = v \right) \wedge \left( v^{'''} = v^{'} \right)  \big) \rightarrow   \big( \left( W^{'}(x) \preccurlyeq(x)_{st} W^{'''} (x) \right) \wedge \left( W^{'}(x) \preccurlyeq(x)_{{st}^{'}} W^{'''} (x) \right)  \big) \big).$
 Following the same approach as in PC, we can rewrite the TSO Equation in terms of intersection of the partial order sets  $\left( \mathcal{O}, \preccurlyeq(x)_{st} \right)$  and $\left( \mathcal{O}, \preccurlyeq(x)_{{st}^{'}} \right)$. 
 Using a new partial order relation $\preccurlyeq(x)^{'}$ that denotes the partial order relation among elements
  in the partial order set formed out of the intersection of the partial order sets $\left( \mathcal{O}, \preccurlyeq(x)_{st} \right)$  and $\left( \mathcal{O}, \preccurlyeq(x)_{{st}^{'}} \right)$, we express TSO as follows. 
   \begin{align}\label{eqn:TSO}
\begin{split}
E^s = \forall x, \mathit{st}, \mathit{st}^{'} \in \mathit{St}, w(x,v), r^{'}(x,v^{'}), r^{''}(x), w^{'''}(x,v^{'''}) \in \mathcal{O}  \big( \left( W^{'}(x) F R^{''} (x) \in \mathit{st} \right)  \wedge  \\ \left( \mathit{R}^{'''}(x) F W^{''''} (x) \in \mathit{st}^{'} \right)  \wedge \left( v^{''} = v \right) \wedge \left( v^{'''} = v^{'} \right)  \big) \rightarrow    \left( W^{'}(x) \preccurlyeq(x)^{'} W^{'''} (x) \right) \big)
\end{split}
  \end{align}.% where $ \preccurlyeq(x)$ is a partial order  over  all operations on a given object $x$ comprised in a given global execution, i.e., comprising all clients executing on a system. %and $ \mathit{Si}$ is an equivalent legal serialization of a global execution defined in Equation \ref{def:ser}.
...
%  {W}(x)^{'''}_\mathit{ty}, {W}(y)^{''''}_\mathit{ty} \in  \mathit{St} \\
% %\mathit{tx} F \mathit{ty} \rightarrow
%  \big( \left( \not\exists {R^{'''''}}(x)_\mathit{tx}, {R^{''''''}}(y)_\mathit{ty} \in \mathit{St} \right) \\
...
%  {W}(x)^{'''}_\mathit{ty}, {W}(y)^{''''}_\mathit{ty}, {R}(x)^{'''''}_\mathit{tx}, {R}(y)^{''''''}_\mathit{ty} \in \mathit{St} \\ \big)
%% \big( \left( {W^i}(x)^{'}_\mathit{tx} F c_\mathit{tx} F  {W^k}(x)^{'}_\mathit{ty} F {R^m}(x)^{'}_\mathit{ty} \in \mathit{st} \right) \\
%%\wedge \left( {W^j}(y)^{'}_\mathit{tx} F c_\mathit{tx} F {W^l}(y)^{'}_\mathit{ty} F {R^n}(y)^{'}_\mathit{ty} \in \mathit{St} \right) \wedge \\
...
%%   \left( {W^j}(y)^{'}_\mathit{tx} F c_\mathit{tx} F {W^l}(y)^{'}_\mathit{ty} F {R^n}(y)^{'}_\mathit{ty} \in \mathit{St} \right) \wedge \\
%% \left( v_n^{'} = v_k^{'} \right) \wedge \left( v_m^{'} = v_i^{'} \right)  \big) \wedge \\
%    \big( \not\exists {R}(x)^{'}_\mathit{tx}, {W}(x)^{''}_\mathit{ty}, {R}(x)^{'''}_\mathit{ty}, {R}(y)^{''''}_\mathit{ty}, {W}(y)^{'''''}_\mathit{tx}, {R}(y)^{''''''}_\mathit{tx} \in \mathit{St} \big)  \\
%     \big( \big( {R}(x)^{'}_\mathit{tx} F {W}(x)^{''}_\mathit{ty} F {R}(x)^{'''}_\mathit{ty} \in \mathit{St} \big) \wedge \\
%  \big(  {R}(y)^{''''}_\mathit{ty} F {W}(y)^{'''''}_\mathit{tx} F {R}(y)^{''''''}_\mathit{tx} \in \mathit{St} \big)  \big)  \\
...
%%   \left( {W^j}(y)^{'}_\mathit{tx} F c_\mathit{tx} F {W^l}(y)^{'}_\mathit{ty} F {R^n}(y)^{'}_\mathit{ty} \in \mathit{St} \right) \wedge \\
%% \left( v_n^{'} = v_k^{'} \right) \wedge \left( v_m^{'} = v_i^{'} \right)  \big) \wedge \\
%    \big( \not\exists {R}(x)^{'}_\mathit{tx}, {W}(x)^{''}_\mathit{ty}, {R}(x)^{'''}_\mathit{ty}, {R}(x)^{''''}_\mathit{ty}, {W}(x)^{'''''}_\mathit{tx}, {R}(x)^{''''''}_\mathit{tx} \in \mathit{St} \big)  \\
%     \big( \big( {R}(x)^{'}_\mathit{tx} F {W}(x)^{''}_\mathit{ty} F {R}(x)^{'''}_\mathit{ty} \in \mathit{St} \big) \wedge \\
%  \big(  {R}(x)^{''''}_\mathit{ty} F {W}(x)^{'''''}_\mathit{tx} F {R}(x)^{''''''}_\mathit{tx} \in \mathit{St} \big)  \big)  \\
%  \big(  \left( v^{''} = v^{'} \right) \wedge
% \left( v^{'''''} = v^{''''} \right) \big) \big) \big)
...
To begin with, we need a helper definition to enumerate all admissible partial orders for a given restriction condition $E^s$ and set of operations in a global session trace $St$.

\begin{definition}{Partial order enumeration}\label{def:allpos}
Given a global session trace $St$ and a restrictive condition $E^s$
for a ConSpec definition, we define the partial order enumeration
of this session trace and condition, $\Pi(St,E^s)$ as
the set of partial orders over the elements of $St$ that are valid
under $E^s$, i.e.:

$\Pi(St,E^s) \equiv  \{\preccurlyeq:  E^s\; \vDash \; \left( \mathcal{O}_{St}, \preccurlyeq \right)\}$
\end{definition}
...
It is possible for a storage system to satisfy a consistency model $E^s$ while simultaneously providing high availability and partition tolerance if and only if for any global session trace $St$, there exists a corresponding partial order enumeration that does not constrain elements of different sessions according to the respective partial order, i.e.:

$\forall St \in \mathcal{S}_t ~ \exists \prec \in \Pi(St,E^s) ~ \forall o, o' \in St (o \in st \wedge o' \in st' \wedge st \not= st' \rightarrow o \not\prec o' \wedge o' \not\prec o)$
...
Global Write Order per Object (GWO)  is a property which requires a global order among results of write operations to each object across all clients in a system. A partial order $\preccurlyeq$ satisfies GWO if $\preccurlyeq \; = \; \forall  x, \mathit{st}, \mathit{st}^{'} \in \mathit{St}, w(x,v), w^{'}(x,v^{'}) \in \mathcal{O}  \left( {W}^{'}  \preccurlyeq_{st} {W}^{''} \wedge {W}^{'}  \preccurlyeq_{st}^{'} {W}^{''} \right),$ where $\preccurlyeq_{st}$ and $\preccurlyeq_{st}^{'}$ are the restriction of the partial order  $\preccurlyeq$ to operations from a pair of clients, ovserved in the respective session traces $\mathit{st}$ and $\mathit{st}^{'}$.
\end{definition}
...
% It is impossible for a storage system to satisfy a consistency model that enforces restrictions on order  among the  results of operations performed by different clients executing on the system, while simultaneously providing high availability and partition tolerance. 
 ...
 To elaborate, if the specification $E^s$ (following the generic format given in Definition \ref{def:form0}) of a consistency model $\mathcal{C}$ imposes a partial order $\preccurlyeq$ which implies that GWO must be satisfied for writes across all clients in the system,  $\mathcal{C}$ is regarded as a CAP-strong consistency model.   For instance, the ConSpec specifications of Causal, SC, PC, and TSO satisfy GWO, since the RHS of the specifications for all of them impose partial order on the results observed for writes across all clients. Hence, according to Theorem \ref{th:cap}, these consistency models can not be implemented in a highly available and partition tolerant manner. 
 On the other hand, the specifications for RYW, MR, WFR, and  MW do not imply GWO, i.e., they imposes a partial order $\preccurlyeq_{st+w}$ only among results observed by operations performed by a given client in a global execution is regarded as CAP-weak. Since they do not consider results of reads from other clients in the partial order $\preccurlyeq_{st+w}$,  they do not impose restrictions on the results of writes across all clients.  
...
%  The condition $\forall \mathit{st}, \mathit{st}^{'} \in \mathit{St}, {\mathit{op}} \in \mathit{st}, {\mathit{op}}^{'} \in \mathit{st}^{'}: {\mathit{Op}}^{'} \preccurlyeq {\mathit{Op}}^{''} \vee {\mathit{Op}}^{''} \preccurlyeq {\mathit{Op}}^{'}$
...
% implies that the specified partial order $\preccurlyeq$ as per Definition \ref{def:form0} must be a partial order comprising only operations from a given client. In other words, a CAP-weak consistency model restricts a partial order $\preccurlyeq_{st}$, comprising results observed in  a given session trace $\mathit{st}$ (comprising operations performed by a single client), or a partial order $\preccurlyeq_{st+w}$ comprising operations performed by a single client as well as all writes from other clients (since writes alone from the other clients do not have any implication with respect to the observed results,  $\preccurlyeq_{st+w}$ can be considered as  partial order over operations from he given client only). 
...
clients, i.e., all communication between $\mathit{Cl}_1$  and  $\mathit{Cl}_2$ are lost. Further let us assume
 that $\mathit{Cl}_1$  and  $\mathit{Cl}_2$ comprise a sequence of write operations on a common data object $x$.
...
to $x$. Since
these components are disconnected, none of these writes are propagated among the servers $N_1$ and $N_2$ during the partition. However, in order to maintain high availability, the system must continue serving the clients even in the presence of the partition. Thus, each of the servers  $N_1$ and $N_2$  must immediately apply the writes from the disconnected clients $\mathit{Cl}_1$  and  $\mathit{Cl}_2$, respectively, and return the results, without waiting for the writes from the other client to be propagated. 
 ...
 for the conflicting object $x$. %; whereas  $\mathit{St}$, as observed by $\mathit{Cl}_2$,  is a sequence $\alpha, \gamma$.  
  Let write operations $w(x,v)$ and $w^{'}(x,v^{'})$  be the prefix of the executions  $\beta$, and $\gamma$ observed by clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$, respectively. 
 Consider that the storage system enforces a consistency model $\mathcal{C}$  which specifies  that the GWO property be respected for any given execution. %Let $\mathit{st}$ and $\mathit{st}^{'}$ for the clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$, respectively. %By definition, apart from transitivity and antisymmetry, a total order $\preccurlyeq$ relation requires satisfaction of the 
%\emph{totality} condition, i.e., 
According to the GWO condition implied by $\mathcal{C}$, the precedence order among writes observed by $\mathit{Cl}_1$ and $\mathit{Cl}_2$ must match. According to the definition of GWO (refer to Definition \ref{def:gwo}), this implies  that results of write operations $w(x,v)$ and $w^{'}(x,v^{'})$ in the session traces $\mathit{st}$ and $\mathit{st}^{'}$ observed by  $\mathit{Cl}_1$ and $\mathit{Cl}_2$ must follow a global precedence order. That is, the precedence orders among writes comprised in the  respective partial orders  $\preccurlyeq_{st}$  and $\preccurlyeq_{st}^{'}$, restricted to the session traces  $\mathit{st}$ and $\mathit{st}^{'}$, must match. 
%That is, $\preccurlyeq_{st}$ implies ${W}^{'}  \preccurlyeq_{st} {W}^{''}$, and $\preccurlyeq_{st}^{'}$ implies $ {W}^{'}  \preccurlyeq_{st}^{'} {W}^{''}$, or $\preccurlyeq_{st}$ implies ${W}^{''}  \preccurlyeq_{st} {W}^{'}$, and $\preccurlyeq_{st}^{'}$ implies ${W}^{''}  \preccurlyeq_{st}^{'} {W}^{'}$. %, but not both simultaneously.  
  However, in the presence of the partition, results of $w(x,v)$ and $w^{'}(x,v^{'})$ are not propagated among the disconnected servers in the same order, i.e., $\beta \not= \gamma$. Hence, the order among writes $w(x,v)$ and $w^{'}(x,v^{'})$ do not match in the partial orders $\preccurlyeq_{st}$  and $\preccurlyeq_{st}^{'}$, %, observed in the session traces $\mathit{st}$ and $\mathit{st}^{'}$. 
%   Thus, ${W}^{'}  \preccurlyeq_{st} {W}^{''}$ holds in $\beta$, while ${W}^{''}  \preccurlyeq_{st}^{'} {W}^{'}$ holds in $\gamma$. 
resulting in violation of the GWO property.  This contradicts our assumption that $\mathcal{C}$ implies the GWO property to be respected for writes across different clients executing in the system.  Hence, $\mathcal{C}$ can not achieve high availability and partition tolerance at the same time. Next, we state the reverse of CAP theorem. 
 \begin{theorem}
...
% It is possible for a storage system to satisfy a consistency model that enforces a partial order among results observed by operations performed by the same client executing on the system, while simultaneously providing high availability and partition tolerance.
 \end{theorem}
Following the approach of the proof to Theorem \ref{th:cap}, this theorem is proved as follows. We consider two servers $N_1$ and $N_2$ that serve requests from clients  $\mathit{Cl}_1$ and $\mathit{Cl}_2$. We consider that a partition occurs at a particular instant between the servers $N_1$ and $N_2$. 
Let us assume that the underlying storage system satisfies a consistency model $\mathcal{C}$ which does not require global ordering of writes (GWO) across different clients. Let us consider that $\mathcal{C}$ imposes partial orders $\preccurlyeq_{st}$ and $\preccurlyeq_{st}^{'}$ among operations comprised in session traces  $\mathit{st}$ and $\mathit{st}^{'}$ for the clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$, respectively.  %must, by definition of a strict partial order, satisfy the following properties: 1)  irreflexivity, 2) transitivity, and 3) asymmetry. The results in each of the above  global session traces $\mathit{St}$ are resulting from a linear execution of operations from each client $\mathit{Cl}_1$ and $\mathit{Cl}_2$, i.e., each client executes its operations according to the sequence specified in its respective program order.  Thus each operation can not loop into itself (state diagram for an execution must be devoid of self loops); hence, condition 1 is trivially satisfied in the above execution.  Since the operations performed by clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$ are executed in  a linear temporal order which follows from the program order of each client, there can not be any cycle among the operations in each of the above session traces; hence, condition 2 also holds for the above execution. 
 The session traces  $\mathit{st}$ and $\mathit{st}^{'}$ comprise sequences of operations, given as  $\alpha, \beta$ and  $\alpha, \gamma$, respectively.  Again, let the prefix of the executions  $\beta$, and $\gamma$ observed by clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$ be writes
  $w(x,v)$ and $w^{'}(x,v^{'})$, respectively.  
 Because of the lack of communication of updates between the servers $N_1$ and $N_2$ serving $\mathit{Cl}_1$ and $\mathit{Cl}_2$ in presence of a partition, results of the writes $w(x,v)$ and $w^{'}(x,v^{'})$ are not propagated among the servers $N_1$ and $N_2$.  To remain available and partition tolerant, the system must continue to serve requests uninterruptedly from the clients even in the presence of the partition.  Thus, each of the  servers $N_1$ and  $N_2$ must immediately apply the writes $w(x,v)$ and $w^{'}(x,v^{'})$ from the clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$, respectively,   and return the results, without waiting for the write from the other server to reach it. However,  even while working in such a disconnected mode, the system satisfies the specified consistency model $\mathcal{C}$ despite the partition, because of the following reasons.   According to $\mathcal{C}$, the system does not require a global ordering among writes to $x$ across different clients.  Hence, the order among the writes $w(x,v)$ and $w^{'}(x,v^{'})$ observed in $\mathit{st}$ and $\mathit{st}^{'}$ does not need to match. Thus the partial orders $\preccurlyeq_{st}$ and $ \preccurlyeq_{st}^{'}$, observed in $\mathit{st}$ and $\mathit{st}^{'}$, specified by $\mathcal{C}$,  with respect to $\mathit{st}$ and $\mathit{st}^{'}$, can observe $w(x,v)$ and $w^{'}(x,v^{'})$ in different order. Because of the breakage of connection between $N_1$ and $N_2$ in the presence of the partition, the session trace  $\mathit{st}$  for $\mathit{Cl}_1$  observes a partial order  $\preccurlyeq_{st}$ = $\alpha  \preccurlyeq_{st} \beta$. On the other hand,  the session trace $\mathit{st}^{'}$ observed by $\mathit{Cl}_2$ observes a different partial order $\preccurlyeq^{'}$ = $\alpha \preccurlyeq_{st}^{'} \gamma$. Since  the prefix of $\beta$ and $ \gamma$ are $w(x,v)$ and $w^{'}(x,v^{'})$, respectively, the partial orders  $\preccurlyeq_{st}$ and  $\preccurlyeq_{st}^{'}$ do not preserve a global order the writes for $\mathit{st}$ and $\mathit{st}^{'}$. Thus, the system fails to preserve GWO in the presence of partition.  However, since the  consistency model $\mathcal{C}$ does not require GWO, the system continues to satisfy $\mathcal{C}$. 
 Hence, in presence of partitions, the above system remains available, while satisfying the consistency model $\mathcal{C}$. Thus, such a consistency model can be implemented in a manner which simultaneously achieves high availability and partition tolerance.  
 

 \section{Implementation}\label{sec:impl}
 We provide an open source automated verification tool built using Spin, an open source software verification tool  \cite{Holzmann:2003:SMC:1405716}. Our tool can verify whether a given session trace satisfies a given consistency model or a given isolation level, specified in terms of a ConSpec expression. The source code and instructions for running the ConSpec tool is provided in an open source github repository \url{https://github.com/ssidhanta/ConSpecTool}. The tool accepts specifications of a given ConSpec definition in the form of an LTL expression. The behaviour of the clients acting on the underlying storage system is provided as an interaction of processes specified using the  PROMELA meta language. The tool runs the Spin model checker over the session trace, and the serialization list, and determines whether the session trace satisfies the specified consistency model or not. 
 From the  PROMELA specification, Spin captures the specified semantics of a given storage system. The PROMELA specifications are translated into C code. Spin acts as a driver that verifies the generated C code  against the  ConSpec definitions expressed in terms of LTL formulas. Spin parses the given session trace and the set of equivalent serializations generated for a given storage system.   The LTL model checker embedded within Spin verifies whether a given session trace satisfies a given ConSpec specifications. %The github repository comprises a PROMELA specification for each of the consistency models and isolation levels, the Spin-based main source file, and the code for generating the list of  equivalent serializations.
...
\section{Deriving ConSpec Specifications From the State-of-the-Art Definitions}\label{sec:derive}
 ...
 %ogic expressions.  
 Consistency definitions are specified by Chockler et al. \cite{Chockler2000} as follows.
...
%   a linear sequence of operations formed with the operations  in a given execution sequence.
 Chockler et al. uses the notation $\sigma |i + w$ to denoe a partial
    execution comprising all operations performed by a process (or a client application) and all write operations from
    from other processes. They denote an equivalent legal serialization for the above partial execution as $S_p$;
    abusing the notation, we denote a legal serialization as $S_p$s in this paper.  For means and
...
%   \cite{Chockler2000}of Chockler et al.  is equivalent to the program order $\mathit{po}$ in ConSpec.
 Hence, without any loss of information,  we replace each reference to an execution sequence $\sigma$  with the notation  $\mathit{st}$  denoting a session trace.
  We can rewrite the precedence relation $ \mathit{op}^1 \xrightarrow{\sigma} \mathit{op}^2 $, in terms of Linear Temporal Logic 
 (i.e., LTL) as $\mathit{op}^1 F \mathit{op}^2 \in \mathit{st}$, replacing the precedence  operator $\rightarrow$ with the
 equivalent LTL operator $F$ that denotes ``eventually".
  As postcondition, the definitions of Chockler et al.  specify a precedence relation $\mathit{op}^1 \rightarrow \mathit{op}^2$ among $mathit{op}^1$ and $mathit{op}^2$  in a serialization ${S_p}$. Using the above terminology, the postcondition  is expressed as $\mathit{op}^1 \xrightarrow{S_p} \mathit{op}^2$,  where $\xrightarrow{S_p}$ denotes that the precedence operator $\rightarrow$ is defined in the context of an equivalent legal serialization ${S_p}$. Again, we can rewrite the above precedence relation 
  $ \mathit{op}^1 \xrightarrow{S_p} \mathit{op}^2 $, in terms of Linear Temporal Logic (i.e., LTL)
  $\mathit{op}^1 F \mathit{op}^2 \in S_p$,
...
%  we denote such a serialization as $S_p$. 
 ...
 %\par 
 %Let $w^i$ and $r^j$ be signatures of read and write operations $\mathit{op}^1$ and
% $\mathit{op}^2$, respectively.
  Using the above notations, Chockler et al. states the RYW consistency model as follows.  For any pair of read and write operations $\mathit{op}^1$ and $\mathit{op}^2$, if the condition  (let us call this Condition 1) 
     $\mathit{op}^1 \xrightarrow{\sigma} \mathit{op}^2$ holds for a given execution sequence $\sigma$, 
 comprising the above write and read operations, there must exist a serialization ${S_p}$,comprising the
   operations $\mathit{op}^1$ and $\mathit{op}^2$, for which the condition (let us call this Condition 2) 
  $\mathit{op}^1 \xrightarrow{S_p} \mathit{op}^2$ must hold. %The
   %precedence relationships among operations $\mathit{op}^1$ and $\mathit{op}^2$ in 
   %Condition 1 can be directly
 % expressed in terms of LTL as follows. 
 Let $\mathit{Op}^1$ and $\mathit{Op}^2$  be propositional logic
  variables that indicate whether operations $\mathit{op}^1$ and $\mathit{op}^2$  have
  returned (if $\mathit{Op}^1$ and $\mathit{Op}^2$ is TRUE) or not. Since Condition1  denotes precedence among  $\mathit{op}^1$ and $\mathit{op}^2$ with respect to
   a given execution order $\sigma$, it can be expressed in terms of an LTL expression
    ...
    same session trace $\mathit{st}$.  In other
     words, the operation $\mathit{op}^2$ follows the operation $\mathit{op}^1$ in the
...
%  $\left(\mathit{Op}^1 X^{o} \mathit{Op}^2\right)_{o}^\mathit{po}$.
   Hence, the expression $\mathit{op}^1 \xrightarrow{S_p} \mathit{op}^2$ in Condition 2
...
%  $S_p$ is equivalent to $S_p$, i.e., both notations represent a legal serialization comprising all
  ...
  Since RYW only considers a sequence where a write operations is followed by a read, $\mathit{Op}^1$ and $\mathit{Op}^2$ can be  replaced  by new propositional variables
   $W^{'}$ and $R^{''}$, without any loss of information. Thus, Condition 2 is expressed as $W^1 F W^2 \in S_p$. Further, we can replace $S_p$ with an equivalent partial order $\preccurlyeq_{st+w}$, defined in Definition \ref{def:form0}, because of the following reasons.  Now, a legal serialization $S_p$ is a sequence of operations that satisfies the following properties: Property 1) it comprises all operations from a given client plus writes from all other clients, and Property 2) each read in the sequence $S_p$ returns the result of the preceding write in $S_p$.  First, by definition $\preccurlyeq_{st+w}$ is a partial order  comprising all operations in a session trace plus writes from other operations, thus satisfies Property 1 for $S_p$. Second, following by Condition 1 in Definition \ref{def:form0}, output of each operation in $\preccurlyeq_{st+w}$ is equivalent to that obtained by executing a linear sequence of the operations preceding that operation, thus satisfying Property 2 for $S_p$. Hence, we can rewrite the postcondition $W^1 F W^2 \in S_p$  for Conditon 2 as  $W^{'} \preccurlyeq_{st+w} R^{''}$. Thus, Chockler's definition of RYW reduces into the ConSpec specification in Equation \ref{eqn:RYW}. 
...
%\end{align}.
 ...
 MR is expressed as: if the condition  (Condition 1) $\mathit{op}^1 \xrightarrow{\sigma} \mathit{op}^2$ holds for a given execution sequence $\sigma$, where both $\mathit{op}^1$ and $\mathit{op}^2$ must be read operations,
  there must exist a serialization ${S_p}$,comprising the
   operations $\mathit{op}^1$ and $\mathit{op}^2$, for which the condition (Condition 2)
  $\mathit{op}^1 \xrightarrow{S} \mathit{op}^2$ must hold.
   Again, following the same logic as in RYW, the above precedence relationships among operations $\mathit{op}^1$ and $\mathit{op}^2$ in Condition 1 can be directly
  expressed in terms of an LTL expression
    $R^i F R^j \in \mathit{st}$.   Similarly, the expression $\mathit{op}^1 \xrightarrow{S} \mathit{op}^2$ in Condition 2
  can be expressed in terms of LTL as $R^{'} F R^{''} \in S_p$. 
    Following the same line reasoning as in RYW,  we can rewrite the expression $R^{'} F R^{''} \in S_p$ for Condition 2 as  $R^{'} \preccurlyeq_{st+w} R^{''}j$, thus reducing the above specification  into Equation \ref{eqn:MR}. 
...
The derivations of rest of the consistency models are given in Section \ref{sec:restderive}.

\section{Conclusions}
In this paper, we presented precise formal definitions for the existing body of consistency semantics and isolation levels using a generalized specification language, ConSpec. In contrast with the existing definitions which are tied to the specific syntax, protocols, and implementation-level details of a particular class of storage systems, our definitions are implementation-agnostic and flexible.  Thus, our definitions can be used as a standard baseline for  verifying the consistency and isolation levels of existing storage systems, as well as for correctly implementing new prototype storage systems. Based on the above generalized definitions, we have developed a  technique for organizing  consistency models into a hierarchical arrangement, based on their degree of strength. We have extended the CAP theorem to state a property which makes it possible for  a consistency model to be implemented in a manner that can achieve both high availability and partition tolerance; we also provided a formal proof of the above theorem.  We also provided formal  proofs showing the equivalence of each our consistency and isolation definitions with the existing state-of-the-art definitions.  We also presented an automated tool for verifying whether a given session trace satisfies a given consistency model or isolation level, specified using ConSpec.  We believe that verification tools based on ConSpec specifications (like the one we presented) should perform better and more reliably than current state-of-the-art tools. In future, we plan to keep working in this direction, and aim to develop an end-to-end framework for building and verifying correct applications and storage systems, based on our generalized specifications.  
...
\subsection{Derivations of Rest of the ConSpec Specifications}\label{sec:restderive}
 \par Chockler et al. states WFR consistency as: if the condition (Condition 1) 
  ...
  followed by write operation, there must exist a serialization ${S_p}$,comprising the
   operations $\mathit{op}^1$ and $\mathit{op}^2$, for which the condition (Condition 2) 
  $\mathit{op}^1 \xrightarrow{S_p} \mathit{op}^2$ must hold.
   As in the case of RYW and MR, the expressions $\mathit{op}^1 \xrightarrow{\sigma} \mathit{op}^2$,
  and $\mathit{op}^1 \xrightarrow{S_p} \mathit{op}^2$  can be rewritten as LTL expressions
 ...
 % Thus, the expressions $\left(\mathit{op}^1 \rightarrow \mathit{op}^2\right)^\sigma$ can be rewritten as LTL formula
%  $\left(\mathit{Op}^1 X^{o} \mathit{Op}^2\right)_{o}^\mathit{po}$.
   %Similarly, the expression $\mathit{op}^1 \xrightarrow{S} \mathit{op}^2$ in Condition 2
  %can be expressed in terms of LTL as $R^{'} F W^{''} \in S_p$$. 
  Similarly, the expression $R^{'}  F W^{''}  \in S_p$ for Condition 2 can be rewritten in terms of ConSpec as  $R^{'}  \preccurlyeq_{st+w} W^{''}$,  thus reducing the WFR definition to Equation \ref{eqn:WFR}. 
  % Further,
...
%   (refer to Equation \ref{eqn:WFR}).
  \par Chockler et al. states the MW consistency model as: if the condition (Condition 1)   $\mathit{op}^1 \xrightarrow{\sigma} \mathit{op}^2$ holds for a given execution sequence $\sigma$ of write operations, there must exist a serialization ${S_p}$,comprising the
   operations $\mathit{op}^1$ and $\mathit{op}^2$, for which the condition (lCondition 2) 
  $\mathit{op}^1 \xrightarrow{S} \mathit{op}^2$ must hold.   As in the case of RYW, MR, and WFR, the expressions $\mathit{op}^1 \xrightarrow{\sigma} \mathit{op}^2$,
  and $\mathit{op}^1 \xrightarrow{S_p} \mathit{op}^2$  can be rewritten as LTL expressions
...
%   propositional variables  $W^{'}$ and $W^{''}$.
 Similarly, the expression $W^{'}  F W^{''}  \in S_p$ for Condition 2  can be rewritten as  $W^{'}  \preccurlyeq_{st+w} W^{''}$,  thus reducing the definition into Equation \ref{eqn:MW}. 
...
\emph{direct precedence relation} (which we denote as $\xRightarrow{\sigma}$) between operations $\mathit{op}$
 and$\mathit{op}^{'} $ in the execution order $\sigma$ as follows. $\mathit{op} \xRightarrow{\sigma} \mathit{op}^{'} $
  implies that either $\mathit{op}^{'} $ reads values written by operation $\mathit{op}$, or  a precedence relationship
exists in the execution order $\sigma$ between $\mathit{op}$  and $\mathit{op}^{'} $, i.e.,
$\mathit{op} \xrightarrow{\sigma}\mathit{op}^{'} $ exists. The former part of the precondition can be expressed in the
 form ${\mathit{Op}}^{'} F {\mathit{Op}}^{''}$, where the LTL-like operator $F$ is equivalent to the precedence operator.
 The latter part of the precondition implies read operation $\mathit{Op}^{'}$ reads the value
 written by $\mathit{Op}^{''}$, i.e., $ \left( {\mathit{Op}}^{'} = {W}^{'} \right) \wedge \left( {\mathit{Op}}^{''} = {R}^{''} \right) \wedge
   \left( v = v^{'} \right)$. The precondition of causal consistency specifies that a transitive closure
...
the condition $ {\mathit{Op}}^{'} F {\mathit{Op}}^{''} \vee
 \left( \left( {\mathit{Op}}^{'} = {W}^{'} \right) \wedge \left( {\mathit{Op}}^{''} = {R}^{''} \right) \wedge
   \left( v_i = v_j \right) \right)$ must hold. From the condition 1 in Definition \ref{def:form0}, every operation must reflect results of prior operations being performed in an equivalent sequence; hence, the above precondition can be rewritten as ${\mathit{Op}}^{'}  F {\mathit{Op}}^{''}  \in \mathit{st}$. % The precondition specifies that operations $\mathit{op}^j$  and $\mathit{op}^k$ are comprised in the execution
...
% The above expression matches the LHS of Equation \ref{eqn:Causal}.
 Chockler et al. specifies that for each client, if the above
precondition holds, the condition $\mathit{op} \xrightarrow{S}\mathit{op}^{'} $ must hold,
i.e., an occurrence of $\mathit{op}$ must be followed by an occurrence of $\mathit{op}^{'} $ in any
...
S_p \right)$, for each client.  ${\mathit{Op}}^{'}  F {\mathit{Op}}^{''}   \in S_p$ denotes a partial order among ${\mathit{Op}}^{'} $ and ${\mathit{Op}}^{''} $ with respect to each client. Since the above condition must hold for each client, it directly follows that there must exist a partial order $\preccurlyeq$ comprising all operations in the global session trace which respects the order specified in each session trace for each client. Thus, we can rewrite the above expression as  ${\mathit{Op}}^{'}  \preccurlyeq {\mathit{Op}}^{''}  $. Thus, Chockler's definition reduces into the specification given in Equation \ref{eqn:Causal}. 
...
%\left( \not\exists {W^m}^{'} F {W^n}^{'} F {\mathit{Op}^j}^{'} \in \mathit{st} \right) \big) \big) \big)$
 ...
 can be given  in terms of a series of LTL expression that express the precedence of successive operations in the sequence.
  The precedence relation between any two successive operations $\mathit{op}$ and $\mathit{op}^{'}$ can be expressed as
   $\mathit{Op}^{'} F \mathit{Op}^{''}$.  Thus, the above condition can be expressed as: any two operations in an execution
...
\rightarrow \exists S_c \left( \mathit{Op}^{'} F \mathit{Op}^{''} \in S_c \right)$, where $S_c$ denotes an equivalent legal seriaization of a global execution restricted to operations from the given client (refer to Equation \ref{def:clientser}). Since ${\mathit{Op}}^{'}  F {\mathit{Op}}^{''}  \in S_c$ implies a total order among ${\mathit{Op}}^{'} $ and ${\mathit{Op}}^{''}$, we can rewrite the above expression as  ${\mathit{Op}}^{'}  \prec {\mathit{Op}}^{''} $, where ${\mathit{Op}}^{'}  \prec {\mathit{Op}}^{''}$  implies $\left( {\mathit{Op}}^{'} \preccurlyeq {\mathit{Op}}^{''} \right) \vee \left( {\mathit{Op}}^{''}  \preccurlyeq {\mathit{Op}}^{'}\right)$. Thus, Chockler's definition reduces into the specification given in Equation \ref{eqn:SC}. 
...
% (refer to Equation \ref{eqn:Strict}).
 ...
 \par The specification for Processor consistency (PC) can be directly derived using the above line of reasoning. 
 %PC talks about preserving the order of operations executed by individual processors (or client applications). 
 PC specifies: result of a preceding operation invoked from a given client must be observed in all processors before result of a
 succeeding operation can be observed in any processor (or client application). %Reads against  write operations should
  %view the results of the writes
  %according to the invocation order of these writes in a client application. % Let $\mathit{processors}$ be an  integer term that denotes the
% total number of processors. We can express the condition the propositional logic variable $A$ must hold for any store
% $\mathit{store}_n$ with the expression  $\bigcap_{\mathit{processor}_n}^{\mathit{processors}} A$.
Consider that a given client observes write operations $w(x,v), w^{'}(y,v^{'})$ according to a temporal order $W^{'}(x) F W^{''} (y)$ observed in a session trace $\mathit{st}$. Then, no other session trace $\mathit{st}^{'}$ observed by any other concurrent client may observe the operations in an order which violates the temporal order in $\mathit{st}$. This, in turn, implies that the precedence $W^{'}(x) F W^{''} (y)$  must be respected in partial order sets   $\left( \mathcal{O}, \preccurlyeq_{{st}^{'}+w} \right)$ and $\left( \mathcal{O}, \preccurlyeq_{st+w} \right)$ for session traces $\mathit{st}$ and $\mathit{st}^{'}$, respectively. Thus, Processor Consistency can be expressed as 
   $\forall x, y, \mathit{st}, \mathit{st}^{'} \in \mathit{St}  \big( \left( W^{'}(x) F W^{''} (y) \in \mathit{st} \right) \rightarrow  \left(  \left( W^{'}(x) \preccurlyeq_{st+w} W^{''} (y)   \right) \wedge  \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right) \right) \big)$. %\\
%E^{s} = C =   \forall \mathit{st} \in \mathit{St} \not\exists \big( \left(  {W^i}^{'}, {W^j}^{'} \in \mathit{st}  \right)
%\wedge \left( {R^k}^{'}, {R^l}^{'} \in \mathit{St} \right) \big) \\
%\big( \left( {W^i}^{'} F {W^j}^{'} \in \mathit{st} \right) \wedge
...
% \left( \not\exists {W^l}^{'} F {W^m}^{'} F {R^j}^{'} \in \mathit{St} \right) \big) \big) \big)
%E^s = \left( \mathcal{O}, \preccurlyeq \right), 
%$\forall x, y, \mathit{st} \in \mathit{St}  \left( {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (y) \in \mathit{st} \right)  \rightarrow \left( {\mathit{Op}}^{'}(x) \preccurlyeq {\mathit{Op}}^{''} (y) \right),$  where $ \preccurlyeq$ is a partial order over operations across all data objects with respect to a given global session trace $ \mathit{St}$.
\par Total Store Order (TSO or Total Order) is specified by Owen et al. \cite{Owens:2009:BXM:1616077.1616107}
 ...
 a pair of operations on a common storage system object $x$ must be executed in the same precedence order.  %serializations ${S_p}_i$ and ${S_p}_j$, the write operations on common storage system objects in ${S_p}_i$ and
% ${S_p}_j$ must be executed in the same order, i.e., ${S_p}_i|w$ = ${S_p}_j|w$.
 The above condition implies that partial orders for any session trace $\mathit{st}$ and $\mathit{st}^{'}$ must preserve the precedence among writes $ w(x,v)$  and $w^{'''}(x,v^{'''}$ to a  common data object $x$ with resect to succeeding reads.  Thus, $E^s = \forall x, \mathit{st}, \mathit{st}^{'} \in \mathit{St}, w(x,v), r^{'}(x,v^{'}), r^{''}(x), w^{'''}(x,v^{'''}) \in \mathcal{O}  \big( \left( W^{'}(x) F R^{''} (x) \in \mathit{st} \right)  \wedge  \left( \mathit{R}^{'''}(x) F W^{''''} (x) \in \mathit{st}^{'} \right) \\ \wedge \left( v^{''} = v \right) \wedge \left( v^{'''} = v^{'} \right)  \big) \rightarrow   \big( \left( W^{'}(x) \preccurlyeq(x)_{st} W^{'''} (x) \right) \wedge \left( W^{'}(x) \preccurlyeq(x)_{{st}^{'}} W^{'''} (x) \right)  \big)
...
%  $\not\exists \big(
% {W}(x)^{'}_\mathit{tx}, {R}(y)^{''}_\mathit{tx}, {W}(y)^{'''}_\mathit{ty}, {R}(x)^{''''}_\mathit{ty} \in  \mathit{St} \big)
%   \big( \left( v^{'''} = v \right) \\ 
...
% $\big( \not\exists  \left({R}(x)^{'}_\mathit{tx} F {W}(x)^{''}_\mathit{ty} F {R}(x)^{'''}_\mathit{ty} \in \mathit{St} \right) \wedge \\
%  \big( {R}(x)^{''''}_\mathit{ty} F {W}(x)^{'''''}_\mathit{tx} F {R}(x)^{''''''}_\mathit{tx} \in \mathit{St} \big)  \big) \big(  \left( v^{''} = v^{'} \right) \wedge \\ 
...
equivalent legal serialization of a partial order $\preccurlyeq_{st+w}$, composed of operations performed by the given client, along with only
writes from other concurrent clients. Since $\preccurlyeq_{st+w}$ does not comprise read operations from all clients in   $\mathit{St}$, the above consistency models impose only a strict partial order with respect to  operations in $\mathit{St}$. Thus, writes from concurrent clients are allowed to overwrite results of writes from
...
performed by each client in a global execution must be preserved in a partial order $\preccurlyeq$ comprising all operations in  in a global execution (similar to an equivalent  legal serialization $\mathit{Ser}$ in the definitions of Chockler et al. defined in Equation \ref{def:ser}). Thus, Strict serializability imposes a partial order among  operations from all clients comprised in  $\mathit{St}$.
...
A serialization is said to be \emph{legal} if every read operation returns the value written by the latest write
 ...
 State-of-the-art definitions specify consistency levels in terms of dependency relations, namely
  ...
  \wedge \left(  R^{''} \in \mathit{St} \right) \big)  \left( v  = {v}^{'} \right)$, where we represent the write and read operations $w$ and $r$
...
\begin{split}
 ...
 % \left( {W^i}^{'} \in \mathit{st} \right)
  ...
  \big( \left( {W^i}^{''} F { R^l}^{''} \in \mathit{St} \right) \wedge \\
   \left(  \not\exists {W}^{'} F {W}^{'''''} F { R}^{''''} \in \mathit{St} \right)  \big) \big) \big) %\left( \not\exists {W^j}^{''} F {W^m}^{''} F { R^l}^{''} \in \mathit{st}^{'} \; \wedge \; {v_l}^{'} = {v_j}^{'}
%  \right) \\
...
\left( {R}^{'} \in \mathit{st} \right)
   \wedge \left( {W}^{''}  \in  \mathit{St} \right) \wedge
   \left( \exists  {W}^{'''} \in \mathit{st}  \right) 
   \left( v = {v}^{''} \right) \wedge
  \left( \exists {R}^{''''} \in \mathit{St}  \right) \left( {v}^{'''} = {v}^{'} \right)  \big)
 \big( \left( {v}^{''} = {v}^{'''} \right) \vee \\
  \big(  \left( {W}^{'''} F { R}^{''''} \in \mathit{St} \right) \wedge
    \left(  \not\exists {W}^{'''} F {W}^{'''''} F { R}^{''''} \in \mathit{St}  \right) \big) \big)
...
% \left( \not\exists {W^m}^{'} F {R^i}^{'} \in \mathit{st}^{''} \right) \left( {v_l}^{'} = {v_m}^{'} \right) \big)
 % \big( {v_l}^{'} = {v_j}^{'}
  ...
  \wedge \left(  R^{''}_\mathit{ty} \in \mathit{St} \right) \big) \left( v = {v}^{'} \right) $. The $ww$ dependency relation
    between transactions $\mathit{tx}$ and $\mathit{ty}$ can be translated to \begin{align}\label{eqn:ww}
\begin{split}
 \exists \big( %\left( \mathit{st}, \mathit{st}^{'} \in \mathit{St} \right) \wedge
 \left( {W}^{''}_\mathit{tx} \in \mathit{st} \right)
   \wedge \left( {W}^{''}_\mathit{tx}  \in  \mathit{St} \right)  \big)
   \big( %\wedge
 % \left( {W^i}^{'} \in \mathit{st} \right)
  \exists %\big( % \left( \mathit{st}^{''} \in \mathit{St} \right) \wedge
  \left( { R}^{''''}\mathit{ty} \in \mathit{St} \right) %\big)
  %\exists \left( \mathit{st}, \mathit{st}^{'} \in \mathit{St} \right)  %{W^i}^{'}  F { R^k}^{''} \in \mathit{st}^{''}  \wedge \\
  %\wedge {R^l}^{'''} F { R^k}^{'''} \in \mathit{st}^{''} \wedge
    \left( {v}^{'''} = {v}^{'} \right)
   \wedge
   \big( \left(  {v}^{'''} = v \right) \vee
  \big( \left( {W}^{''} F { R}^{''''}_\mathit{ty} \in \mathit{St} \right) \wedge \\
   \big( \left(  \not\exists {W}^{'}_\mathit{tx} F {W}^{'''''}_\mathit{tx} F { R}^{''''}_\mathit{ty} \in \mathit{St} \right)  
  \wedge  \left(  \not\exists {W}^{'}_\mathit{tx} F {W}^{'''''}_\mathit{ty} F { R}^{''''}_\mathit{ty} \in \mathit{St} \right) \big) \big) \big) \big)
  %\left( \not\exists {W^j}^{''} F {W^m}^{''} F { R^l}^{''} \in \mathit{st}^{'} \; \wedge \; {v_l}^{'} = {v_j}^{'}
...
\left( {R}^{'}_\mathit{tx} \in \mathit{st} \right)
   \wedge \left( {W}^{''}_\mathit{ty}  \in  \mathit{St} \right) \wedge
   \left( \exists  {W}^{'''}_\mathit{tx} \in \mathit{st}  \right) 
   \left( v = {v}^{''} \right)  \wedge 
  \left( \exists {R}^{''''}_\mathit{ty} \in \mathit{St}  \right) \left( {v}^{'''} = {v}^{'} \right)  \big)
  \big( \left( {v}^{''} = {v}^{'''} \right)  \\
   \vee  \big(  \left( {W}^{'''}_\mathit{tx} F { R}^{''''}_\mathit{ty} \in \mathit{st}^{''} \right)  \wedge
    \big( \left(  \not\exists {W}^{'''}_\mathit{tx} F {W}^{'''''}_\mathit{tx} F { R}^{''''}_\mathit{ty} \in \mathit{St} \right)  \wedge
   \left(  \not\exists {W}^{'''}_\mathit{tx} F {W}^{'''''}_\mathit{ty} F { R}^{''''}_\mathit{ty} \in \mathit{St} \right) \big) \big)
...
%%% Let $\mathit{op}$ be signature of an 
%%%$i^\mathit{th}$ operation invoked from a given client application. $\mathit{op}$ can be a read or write operation that reads (or writes) a unique value from (or to) a storage system object $o$; signature of a read and write operation are of the form $r(o){v_m}$ and $w(o,v_l)$, respectively.
%%% Let $v_k$ be the current value observed against the operation $\mathit{op}$ on an object $o$. Let the length of the given session trace be denoted by the integer term $s_t$. A a session trace $\mathcal{S}_t$ can be expressed as a collection of tuples, where each tuple comprises: 1) the unique signature of the operation, 2) the name of the storage system object that the operation accesses, 3) the value written by the operation. Using the above notations, a session trace $\mathcal{S}_t$ is given as $\mathcal{S}_t$ = \[ \Set{\tuple{\mathit{op},o,v_k}} {1\le i \le s_t, 1\le j\le m, 1\le k\le n}{.} \]
