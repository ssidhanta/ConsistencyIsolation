...
In this section, we present the ConSpec specifications for several common consistency models and isolation levels documented in the literature~\cite{Chockler2000, Terry:1994:SGW:645792.668302, Burckhardt:2014:PEC:2693641.2693642}. Subsequently, in Section~\ref{sec:derive}, we prove the equivalence between these definitions and their original counterparts.
...
Since $\preccurlyeq$  enforces a partial order among results of operations performed by  different concurrent clients, the results observed by clients  $\mathit{Cl}_1$ and $\mathit{Cl}_2$ must be restricted to a common partial order $\preccurlyeq$. This implies  that results of operations $\mathit{op}$ and $\mathit{op}^{'}$ in each of the global session traces observed by  the clients  $\mathit{Cl}_1$ and $\mathit{Cl}_2$ must either satisfy $\mathit{op} \preccurlyeq \mathit{op}^{'}$ or $\mathit{op}^{'} \preccurlyeq \mathit{op}$, respectively. %, but not both simultaneously.  
  Since $\beta \not= \gamma$ in $\mathit{St}$, hence $\mathit{op} \preccurlyeq \mathit{op}^{'}$ holds in $\beta$, while $\mathit{op}^{'} \preccurlyeq \mathit{op}$ holds in $\gamma$; hence the above condition of partial order among operations from different clients is violated in the global session traces $\alpha, \beta$ and  $\alpha, \gamma$.  This contradicts the requirement of a common partial order $\preccurlyeq$ on the
...
\subsection{Derivations of Rest of the ConSpec Specifications}\label{sec:restderive}
  Causal consistency is specified by Chockler et al. as follows. Chockler et al. defines a
