...
%\end{figure}
 ...
 \section{Related Work} \label{sec:related}
  As indicated in the CAP theorem \cite{brew:cap}, which was later proved formally by Gilbert et al. \cite{Gilbert:2002:BCF:564585.564601}, it is not possible to provide strong consistency in a system while providing high availability in presence of partitions.  By strong consistency, Brewer referred to linerarizability  \cite{Herlihy:1990:LCC:78969.78972}, which requires that writes should be observed in a system instantaneously, i.e., a read operation should always return result of the latest write.  To remain available during partitions, systems must choose to implement a consistency model that is not too strict to render the system unavailable during partitions (because of high degree to synchronization), and not too weak to violate the application-level correctness. According to specific application behaviour and correctness requirements,  a plethora of  consistency models have been defined and implemented till date to support weaker semantics (weaker than linearizability). Each consistency model specify a specific  set of constraints and restriction with respect to ordering among observed operations in a valid execution. In one end of the ever-expanding consistency spectrum, a  group of systems, e.g.,  shared memory implementations, assume that partitions are rare, and favour stronger consistency semantics. There has been considerable work attempting to develop formal semantics for precisely defining such consistency models  \cite{Ahamad94causalmemory:, Attiya:1994:SCV:176575.176576, Herlihy:1990:LCC:78969.78972, Lamport:1979:MMC:1311099.1311750, opac-b1024856, Mizuno1995, Raynal1995}.  But such definitions are specific to consistency models for shared memory systems, and can not be generalized for applying to other systems and applications, e.g., replicated storage systems.   
  \par Consistency models in the other side of the consistency spectrum lack standardized formal definitions. Such consistency models are implemented by applications that hail from a growing ecosystem of modern web-based applications. These systems trade off stronger consistency models for weaker models, favouring high availability and partition tolerance \cite{Lakshman:2010:CDS:1773912.1773922, Sumbaly_servinglarge-scale, Meiklejohn:2013:RPD:2505305.2505309, Plugge:2010:DGM:1869938, Schutt:2008:SRT:1411273.1411280, DeCandia:2007:DAH:1323293.1294281}. This is particularly true for production-level large-scale geo-replicated systems which replicate data across multiple servers located in geographically distributed datacenters to provide high availability and lower latency to clients from different regions.  Typically, each of these systems define and implement a consistency model in terms of their own system design and protocol-level  assumptions; resulting in ambiguity and confusion.  Terry et al. \cite{bayou}  defined a group weak session guarantees in  prosaic  form, using use cases and terminology that is  very specific to the target applications and the  prototype system discussed in the paper.  To the best of our knowledge,  Chockler et al. \cite{Chockler2000} arguably provides the most comprehensive (though incomplete) collection  of prosaic consistency definitions, which are somewhat implementation agnostic  and flexible. Apart from the fact that the paper is outdated, and does not cover many important consistency models, it also lack axiomatic formalization and a generalized syntax. Mahajan et al. \cite{mahajan11cacTR} defined Real Time Causal Consistency, a stronger variant of causal consistency, and proved that it is the strongest consistency model that can be provided into a highly available and eventually consistent implementation.  
Burckhardt et al. \cite{Burckhardt:2014:PEC:2693641.2693642} present a  formal approach for  expressing consistency semantics in the form of axiomatic specifications of system behaviour, using   properties like visibility and happens before order as building blocks. However, the resulting  formalization is still much complicated, and is targeted towards replicated systems, thus lacking flexibility. Recently, Attiya et al.  \cite{Attiya:2015:LHE:2767386.2767419}  provided a formal specification of systems that implement causal consistency, and proved that Observable Causal Consistency, a stronger variant of causal consistency, is the strongest consistency model that can be provided in a highly available, partition tolerant manner. But their formulation is also tied to replicated systems  and addresses  Observable Causal Consistency, hence can not be generalized.  To conclude, the current literature lacks generalized formal definitions of different consistency models, which would be crucial to designing, implementing, analyzing, and verifying correctness of a system with respect to a particular consistency model.
...
We assume a set of clients that access the storage system by invoking operations. A very large class of consistency definitions (namely those describing the behavior of loads and stores on computer hardware) assume that these operations are partitioned into two classes, namely read operations that do not affect the result of subsequent operations and write operations that do. Given that many of the consistency levels we describe follow this model, we will also incorporate this possibility into our definitions, whenever required for the definition to make sense. Note that such definitions can be generalized in a straightforward way to other models. For example, databases make a similar distinction between queries and updates, and the state machine replication model distinguishes between read-only and read-write requests.

Similarly, many consistency definitions reason about an interface that exposes the existence of multiple objects (e.g., different memory addresses seen by a CPU or different keys in a key-value store). As such, whenever required by the consistency definition, we allow for the possibility that the interface allows the programmer to specify an object $o \in \mathcal{O}$ associated with reads and writes.
...
When incorporating the possibility that the set of possible operations is partitioned into the two classes mentioned previously, we denote an invocation of a write operation that writes a value $v$ to an object $x$, as $w(x,v)$ (with an empty response). Conversely, a read operation on object $x$ that outputs a value $v^{'}$ is denoted $r(x){v^{'}}$. %{\bf [RR: I am inclined to remove the subscript $j$ and $k$.]}
 \def\tuple#1{\langle #1\rangle}
This allows us to define a shortand notation for a session trace $\mathit{st} \in \mathcal{S}_t$ as a sequence of tuples  $\tuple{w, {v}}$ or
...
%  session trace. 
The \emph{global session trace} $\mathcal{S}_t$ (resp.\ global session invocationt trace $\mathcal{S}_{it}$) denotes the set of all
...
%% superscript $o$ specifies the context of an object $o$ for the enclosed relation; $o$ is equivalent to an operand for a
 %%  storage operation.
  E.g., an expression ${w} F_\mathit{st} {r}$ denotes that the operation $w(x,v)$ is followed by the operation  $r(x){v}$ in a session trace $\mathit{st}$.  
%% ; the
...
% \left(  {W^i}^{'} \preccurlyeq_{\mathit{st}+w} {R^j}^{'} \right)\big),
  ...
  \big( \left( {W}^{'} F _ \mathit{st} {R}^{'} \right)
 \rightarrow 
...
% \mathit{st} \in \mathcal{S}_t,
 \mathcal{E}^{s} = \forall   {R}^{'}, {R}^{''} \in \mathit{st} \big( {R}^{'} F _ \mathit{st} {R}^{''}
...
E^{s} =  \forall  {R}^{'}, {W}^{'} \in \mathit{st} \big( \left( {R}^{'} F _ \mathit{st} {W}^{'}  \right)
...
E^{s} = \forall  {W}^{'}, {W}^{''} \in \mathit{st} \big( {W}^{'} F _ \mathit{st} {W}^{''}  \rightarrow \left( {W}^{'} \preccurlyeq_{\mathit{st}+w} {W}^{''} \right) \big), %; \\
...
\begin{align}\label{eqn:Causal}
\begin{split}
E^{s} = \forall \mathit{st} \in \mathcal{S}_t, {\mathit{Op}}^{'}, {\mathit{Op}}^{''} \in \mathit{st} \big( \left( {\mathit{Op}}^{'} F _ \mathit{st} {\mathit{Op}}^{''} \right)
...
%\not\exists  {W^n}^{'} \in \mathcal{S}_t \left( {W^m}^{'} F {W^n}^{'} F {\mathit{Op}^j}^{'} \in \mathit{st} \right) \big) \big) \big)
 \end{split}
  ...
  \par Causal+ consistency, which is the strongest consistency that can  be implemented in  a highly available manner (i.e.,  the system is available in presence of partitions) is defined and implemented by Lloyd et al.  \cite{Lloyd:2011:DSE:2043556.2043593}. It states: in addition to preserving the causality condition of causal consistency, writes from multiple clients on conflicting objects can not observe divergent results. We formalise their definitions in terms of ConSpec  as follows. 
   \begin{align}\label{eqn:CausalC}
\begin{split}
E^{s} = \forall \mathit{st}, \mathit{st}^{'}, \in \mathcal{S}_t, {\mathit{Op}}^{'}, {\mathit{Op}}^{''} \in \mathit{st} \big(  {\mathit{Op}}^{'} F _ \mathit{st} {\mathit{Op}}^{''} 
 \rightarrow {\mathit{Op}}^{'} \preccurlyeq^{'}_{\mathit{st}+w} {\mathit{Op}}^{''}.  
  \end{split}
  \end{align}
 %\begin{align*}
...
%   where $ \preccurlyeq_{\mathit{st}+w}$ and $ \preccurlyeq_{{st}^{'}+w}$ are partial orders restricted to operations performed by clients $\mathit{Cl}$ and $\mathit{Cl}^{'}$ and writes from all other clients, respectively, observed in the respective local session traces $\mathit{st}$ and $\mathit{st}^{'}$.  The partial order relations $  \left( W^{'}(x) \preccurlyeq_{\mathit{st}+w} W^{''} (y)   \right)$ and $ \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right)$ can be expressed in terms of the corresponding partial order sets $\left( {\mathcal{O}_{St}}, \preccurlyeq_{\mathit{st}+w} \right)$  and $\left( {\mathcal{O}_{St}}, \preccurlyeq_{{st}^{'}+w} \right)$. By that logic, the condition $ \left( W^{'}(x) \preccurlyeq_{\mathit{st}+w} W^{''} (y)   \right) \wedge  \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right) $ can be expressed in terms of an intersection $\left( {\mathcal{O}_{St}}, \preccurlyeq^{'} \right)$ of the partial order sets  $\left( {\mathcal{O}_{St}}, \preccurlyeq_{\mathit{st}+w} \right)$  and $\left( {\mathcal{O}_{St}}, \preccurlyeq_{{st}^{'}+w} \right)$, where the partial order $ \preccurlyeq^{'} $ represents the relation among elements of the new partial order set formed out of the intersection of $\left( {\mathcal{O}_{St}}, \preccurlyeq_{\mathit{st}+w} \right)$  and $\left( {\mathcal{O}_{St}}, \preccurlyeq_{{st}^{'}+w} \right)$. 
 Thus, PC can be expressed as follow. %$\forall x, y, \mathit{st}, \mathit{st}^{'} \in \mathcal{S}_t  \big( \left( W^{'}(x) F _ \mathit{st} W^{''} (y) \right) \rightarrow  \left(  \left( W^{'}(x) \preccurlyeq_{\mathit{st}+w} W^{''} (y)   \right) \wedge  \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right) \right) \big)$.   
% The partial order relations $  \left( W^{'}(x) \preccurlyeq_{\mathit{st}+w} W^{''} (y)   \right)$ and $ \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right)$ can be expressed in terms of the corresponding partial order sets $\left( {\mathcal{O}_{St}}, \preccurlyeq_{\mathit{st}+w} \right)$  and $\left( {\mathcal{O}_{St}}, \preccurlyeq_{{st}^{'}+w} \right)$. By that reasoning, the condition $ \left( W^{'}(x) \preccurlyeq_{\mathit{st}+w} W^{''} (y)   \right) \wedge  \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right) $ can be expressed in terms of an intersection $\left( {\mathcal{O}_{St}}, \preccurlyeq^{'} \right)$ of the partial order sets  $\left( {\mathcal{O}_{St}}, \preccurlyeq_{\mathit{st}+w} \right)$  and $\left( {\mathcal{O}_{St}}, \preccurlyeq_{{st}^{'}+w} \right)$, where $ \preccurlyeq^{'} $ denotes the partial order relation  among elements of the new partial order set formed out of the intersection of $\left( {\mathcal{O}_{St}}, \preccurlyeq_{\mathit{st}+w} \right)$  and $\left( {\mathcal{O}_{St}}, \preccurlyeq_{{st}^{'}+w} \right)$. 
 %Following the approach as in Causal+, we rewrite PC specification as %n terms of a partial order $ \preccurlyeq^{'} $ as follows. 
    \begin{align}\label{eqn:PC}
\begin{split}
\forall x, y, \mathit{st}, \mathit{st}^{'} \in \mathcal{S}_t  \big( \left( W^{'}(x) F _ \mathit{st} W^{''} (y) \right) \rightarrow  \left( W^{'}(x) \preccurlyeq^{'} W^{''} (y)   \right)  \big),
\end{split}
...
%  \end{align} 
  %which can be rewritten as follows. 
 %$E^s = \forall x, \mathit{st}, \mathit{st}^{'} \in \mathcal{S}_t, w(x,v), r^{'}(x,v^{'}), r^{''}(x), w^{'''}(x,v^{'''}) \in {\mathcal{O}_{St}}  \big( \left( W^{'}(x) F _ \mathit{st} R^{''} (x) \right)  \wedge  \left( \mathit{R}^{'''}(x) F _ \mathit{st}^{'} W^{''''} (x) \right)  \wedge \left( v^{''} = v \right) \wedge \left( v^{'''} = v^{'} \right)  \big) \rightarrow   \big( \left( W^{'}(x) \preccurlyeq(x)_{st} W^{'''} (x) \right) \wedge \left( W^{'}(x) \preccurlyeq(x)_{{st}^{'}} W^{'''} (x) \right)  \big) \big).$
 %Following the same approach as in PC, we can rewrite the above Equation as follows. 
   \begin{align}\label{eqn:TSO}
\begin{split}
E^s = \forall x, \mathit{st}, \mathit{st}^{'} \in \mathcal{S}_t, w(x,v), r^{'}(x,v^{'}), r^{''}(x), w^{'''}(x,v^{'''}) \in {\mathcal{O}_{St}}  \big( \left( W^{'}(x) F R^{''} (x) \in \mathit{st} \right)  \wedge  \\ \left( \mathit{R}^{'''}(x) F W^{''''} (x) \in \mathit{st}^{'} \right)  \wedge \left( v^{''} = v \right) \wedge \left( v^{'''} = v^{'} \right)  \big) \rightarrow    \left( W^{'}(x) \preccurlyeq(x)^{'} W^{'''} (x) \right) \big)
\end{split}
...
{\bf [Need to pin down execution model, and the previous definition needs to have the operation signature but not its result.]}
...
In an asynchronous system, it is possible to implement a consistency model $E^s$ while simultaneously providing availability and partition tolerance if and only if for any global session trace $\mathcal{S}_t$ and all of its partial orderings that are allowed by $E^s$, when you consider the set of maxima of each partial order, it is always possible to make them depend only on the previous operation in the same session and still obtain a valid partial order, i.e.:
...
\noindent $\forall \mathcal{S}_t ~ \forall \preccurlyeq \in \Pi(\mathcal{S}_t,E^s) ~ \forall o \in max(\mathcal{S}_t, \preccurlyeq) ~ (\textsc{RemoveAllExceptSession}(\preccurlyeq,o) \in \Pi(\mathcal{S}_t,E^s) ~ ) $

where we define {\sc RemoveAllExceptSession} as a partial order where the maxima are only ordered with respect to prior operations in the same session, i.e.:

\noindent {\sc RemoveAllExceptSession}$(\preccurlyeq,o) \equiv \preccurlyeq \setminus \{ \langle o',o \rangle: (... , o', ..., o) \notin \mathcal{S}_t\}$
...
Given any global session trace $\mathcal{S}_t$, we prove this by induction on the length of the execution that produced $\mathcal{S}_t$. The base case with an empty execution is vacuously true, since an empty trace meets any consistency condition (no safety properties are ever violated by an empty trace). For the induction step, we need to prove that, given an execution for which an available and partition-tolerant implementation produced a trace that conforms to $E^s$, it is possible for a client to invoke a new operation and produce an output that is also consistent. This is true because, even in the case that the client that invokes the new operation is partitioned from the remaining of the clients, it is always legal to produce an operation that depends on a prior operation from the same session and all the operations that transitively precede it according to $\prec$. Furthermore, the valid output of this operation can be determined by using only information that is local to the session, by running the sequential specification of the system on the graph of preceding operations.
...
\section{Analysis of Consistency Models With Respect To CAP}\label{sec:capanalyze}
 The $E^s$ for RYW, MR, WFR, MW,  and Processor Consistency specifies that precedence order among operations in a valid global session trace $\mathcal{S}_t$ must be preserved in any valid partial order belonging in a valid partial order enumeration $\Pi(\mathcal{S}_t,E^s) $, where each valid partial order $\preccurlyeq_{\mathit{st}+w}$ in a valid $\Pi(\mathcal{S}_t,E^s) $  comprises operations from a given session trace $\mathit{st}$ plus all writes in  $\mathcal{S}_t$.  Thus, writes from concurrent sessions are allowed to interleave between operations in a given session $\mathit{st}$ in any manner  as long as the precedence order among operations in $\mathit{st}$ follows their mutual precedence order in $\mathit{st}$.   Similarly, Processor consistency (PC) restricts that the order among operations in a given client must be preserved in all executions, i.e., no client may observe the operations from a given client in a different order than the invocation order. Hence, these consistency models can be implemented in a system without expensive synchronization protocols between concurrent clients such as locks. Thus, these are CAP-weak, i.e., they are not dependent among reliable communication among concurrent clients, like broadcasting of results on meta data, or blocking of conflicting operations. Hence, they can be implemented using local state preserving techniques, like local snapshot or checkpoint protocols, that can provide correctness of operations  independent of global state of the system; this enables systems based on these CAP-weak models to maintain high availability in presence of partitions. % Under these consistency models,
...
% Thus, 
 Causal and Causal+ consistency restricts that for all clients in the system, the precedence order among operations invoked by each client in a valid partial order in a valid $\Pi(\mathcal{S}_t,E^s) $ for a valid $\mathcal{S}_t$ must respect the precedence order in a given session trace for that client, where a valid $\Pi(\mathcal{S}_t,E^s) $ comprises operations from $\mathit{st}$ plus writes from other clients. Since these consistency models preserve the precedence across all clients in the system, a valid $\Pi(\mathcal{S}_t,E^s) $, they belong to the CAP-strong class of consistency models. 
...
% eventually operator $F$ or special-purpose next operator $X^o$) among each pair of operations.
 Going one step further, SC requires a total order $\preccurlyeq$ to preserve precedence among operations from all clients in a system. Similarly, TSO 
 imposes restrictions on the precedence order among writes performed by concurrent clients observed in a global session trace $\mathcal{S}_t$. 
Thus, these consistency models can not be implemented using the mechanisms described above.  They require some synchronization among concurrent clients; at the least, Causal and Causal+ they require some form of merging of results from multiple clients. SC requires the stringent form of  synchronization, maybe some form of locking mechanism.  
...
\subsection{Deriving ConSpec Specifications From Original Definitions}\label{sec:derive}
 ...
 $\mathit{o}^2$ a pair of  operations invoked from a session $\mathit{st}$ of the $i^{th}$  process $ p_i$ in the system, which  read or write to an object $x$. % , such that $\mathit{o}^1$ is a write operation that updates
...
%   \cite{Chockler2000}of Chockler et al.  is equivalent to the program order $\mathit{po}$ in ConSpec.
 Hence, without any loss of information,  we replace each reference to $\sigma_i$ in the  the definitions of Chockler et al.  with the notation  $\mathit{st}$.
  We can rewrite a precedence relation $ \mathit{o}^1 \xrightarrow{\sigma_i} \mathit{o}^2 $ found in Chockler et al.\textquotesingle s defintions, in terms of Linear Temporal Logic 
...
\par Causal+ consistency is defined by Lloyd et al. in prosaic form \cite{Lloyd:2011:DSE:2043556.2043593}. We formalise their definitions in terms of ConSpec  as follows. Valid partial orders $\preccurlyeq_{\mathit{st}+w}$ and  $\preccurlyeq(x)_{\mathit{st}+w}$ comprising operations from 
sessions $\mathit{st}$ and$ \mathit{st}^{'}$ must preserve the precedence order among pairs of  writes $w(x)$ and $ ws^{'} (x)$ on a common data object $x$.  We express the above definition in terms of LTL as follow. 
 ...
 \rightarrow {\mathit{Op}}^{'} \preccurlyeq_{\mathit{st}+w} {\mathit{Op}}^{''}  \wedge W^{'}(x) \preccurlyeq(x)_{\mathit{st}+w} W^{'} (x) \rightarrow W^{'}(x) \preccurlyeq(x)_{{st}^{'}+w} W^{''} (x)  \big),  
 $ where $ \preccurlyeq_{\mathit{st}+w}$ is a partial order  restricted to  the elements of sessions $\mathit{st}$ and $\mathit{st}^{'}$ plus writes from other clients.  $ \preccurlyeq(x)_{\mathit{st}+w}$ and $\preccurlyeq(x)_{{st}^{'}+w}$ denote  partial orders  over operations on a given object $x$ restricted to  the elements of sessions $\mathit{st}$ and $\mathit{st}^{'}$ plus writes from other clients, respectively.  
 ...
 \rightarrow {\mathit{Op}}^{'} \preccurlyeq_{\mathit{st}+w} {\mathit{Op}}^{''}  \wedge W^{'}(x) \preccurlyeq(x)_{\mathit{st}+w}  W^{''} (x) \wedge W^{'} (x) \preccurlyeq(x)_{{st}^{'}+w} W^{''}(x)  \big)$.  let ${\mathcal{W}_{St}}$ denotes the set comprising write operations  present in all session traces in $\mathcal{S}_t$. 
 Next, the partial order relations  ${\mathit{Op}}^{'} \preccurlyeq_{\mathit{st}+w} {\mathit{Op}}^{''} $, $  \left( W^{'}(x) \preccurlyeq_{\mathit{st}+w} W^{''} (x)   \right)$, and $ \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{'} (x)  \right)$ can be expressed in terms of the corresponding partial order sets  $\left( {\mathcal{O}_{St}}, \preccurlyeq_{\mathit{st}+w} \right)$, $\left( {\mathcal{W}_{St}}, \preccurlyeq(x)_{\mathit{st}+w} \right)$, and $\left( {\mathcal{W}_{St}}, \preccurlyeq(x)_{{st}^{'}+w} \right)$. By that reasoning, the condition $ {\mathit{Op}}^{'} \preccurlyeq_{\mathit{st}+w} {\mathit{Op}}^{''}  \wedge W^{'}(x) \preccurlyeq(x)_{st+w} W^{''} (x) \wedge W^{'} (x) \preccurlyeq(x)_{{st}^{'}+w} W^{''}(x)$ can be expressed in terms of an intersection $\left( {\mathcal{O}_{St}}, \preccurlyeq^{'}_{st+w} \right)$ of the partial order sets  $\left( {\mathcal{O}_{St}}, \preccurlyeq_{\mathit{st}+w} \right)$, $\left( {\mathcal{W}_{St}}, \preccurlyeq(x)_{\mathit{st}+w} \right)$,  and $\left( {\mathcal{W}_{St}}, \preccurlyeq(x)_{{st}^{'}+w} \right)$, where $ \preccurlyeq^{'}_{\mathit{st}+w} $ denotes the partial order relation  among elements of the new partial order set formed out of the intersection of $\left( {\mathcal{O}_{St}}, \preccurlyeq_{\mathit{st}+w} \right)$, $\left( {\mathcal{W}_{St}}, \preccurlyeq_{\mathit{st}+w} \right)$, and $\left( {\mathcal{W}_{St}}, \preccurlyeq_{{st}^{'}+w} \right)$. Thus, Cusal+ can be rewritten into the form of Equation \ref{eqn:CausalC}. 
...
Thus, PC can be expressed as $\forall x, y, \mathit{st}, \mathit{st}^{'} \in \mathcal{S}_t  \big( \left( W^{'}(x) F_\mathit{st}  W^{''} (y) \in \right) \rightarrow  \left(  \left( W^{'}(x) \preccurlyeq_{\mathit{st}+w} W^{''} (y)   \right) \wedge  \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right) \right) \big)$.   The partial order relations $  \left( W^{'}(x) \preccurlyeq_{\mathit{st}+w} W^{''} (y)   \right)$ and $ \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right)$ can be expressed in terms of the corresponding partial order sets $\left( {\mathcal{O}_{St}}, \preccurlyeq_{\mathit{st}+w} \right)$  and $\left( {\mathcal{O}_{St}}, \preccurlyeq_{{st}^{'}+w} \right)$. By that reasoning, the condition $ \left( W^{'}(x) \preccurlyeq_{\mathit{st}+w} W^{''} (y)   \right) \wedge  \left( W^{'}(x)  \preccurlyeq_{{st}^{'}+w}  W^{''} (y)  \right) $ can be expressed in terms of an intersection $\left( {\mathcal{O}_{St}}, \preccurlyeq^{'} \right)$ of the partial order sets  $\left( {\mathcal{O}_{St}}, \preccurlyeq_{\mathit{st}+w} \right)$  and $\left( {\mathcal{O}_{St}}, \preccurlyeq_{{st}^{'}+w} \right)$, where $ \preccurlyeq^{'} $ denotes the partial order relation  among elements of the new partial order set formed out of the intersection of $\left( {\mathcal{O}_{St}}, \preccurlyeq_{\mathit{st}+w} \right)$  and $\left( {\mathcal{O}_{St}}, \preccurlyeq_{{st}^{'}+w} \right)$.  Thus, the definitions of Ahmad et al. directly reduces to the expression given in Equation \ref{eqn:PC}.
...
According to TSO, in a valid global session trace $\mathcal{S}_t$, the read $r^{'}(x,v^{'})$ can not return the value written by $w^{'''}(x,v^{'''})$ before $r^{''}(x)$ returns the result of $w(x,v)$.  For such an $\mathcal{S}_t$  there must exist a valid legal serialization, comprising operations from each client,  where $w(x,v)$ occurs before $w^{'''}(x,v^{'''})$., i.e., there must exist a valid legal serializations for each session trace that orders $w(x,v)$ occurs before $w^{'''}(x,v^{'''})$.  In other words, a valid legal seriallizaton for partial execution of each client (i.e., for each session trace corresponding to the respective clients) must apply writes to each data object in the same precedence order.  Using the same approach that used in the prior derivations, the above condition of a valid legal serialization can be restated in terms of valid partial orders, i.e.,  there must exist valid partial orders $\preccurlyeq(x)_{{st}^{'}}$ and $\preccurlyeq(x)_{{st}}$ over operations comprised in the session traces $\mathit{st}$ and $\mathit{st}^{'}$ that order writes to each data object according to an identical  precedence order.  
 ...
 $E^s = \forall x, \mathit{st}, \mathit{st}^{'} \in \mathcal{S}_t, w(x,v), r^{'}(x,v^{'}), r^{''}(x), w^{'''}(x,v^{'''}) \in {\mathcal{O}_{St}}  \big( \left( W^{'}(x) F_\mathit{st}  R^{''} (x) \right)  \wedge  \left( \mathit{R}^{'''}(x) F_\mathit{st}^{'} W^{''''} (x) \right)  \wedge \left( v^{''} = v \right) \wedge \left( v^{'''} = v^{'} \right)  \big) \rightarrow   \big( \left( W^{'}(x) \preccurlyeq(x)_{st} W^{'''} (x) \right) \wedge \left( W^{'}(x) \preccurlyeq(x)_{{st}^{'}} W^{'''} (x) \right)  \big) \big).$
  TSO can be expressed in terms of an intersection of the partial order sets  $\left( {\mathcal{O}_{St}}, \preccurlyeq(x)_{st} \right)$  and $\left( {\mathcal{O}_{St}}, \preccurlyeq(x)_{{st}^{'}} \right)$, where $\preccurlyeq(x)^{'}$  denotes a partial order among elements  of the intersection-set comprising an intersection of the partial order sets $\left( {\mathcal{O}_{St}}, \preccurlyeq(x)_{st} \right)$  and $\left( {\mathcal{O}_{St}}, \preccurlyeq(x)_{{st}^{'}} \right)$. %In any two executions of a sequence of operations by two processors (or client applications), a pair of writes on a common system object $x$ must be executed in the same precedence order.  %serializations ${S_p}_i$ and ${S_p}_j$, the write operations on common storage system objects in ${S_p}_i$ and
% ${S_p}_j$ must be executed in the same order, i.e., ${S_p}_i|w$ = ${S_p}_j|w$.
 Thus,  the above expression for TSO can be reduced to the ConSpec form $E^s = \forall x, \mathit{st}, \mathit{st}^{'} \in \mathcal{S}_t, w(x,v), r^{'}(x,v^{'}), r^{''}(x), w^{'''}(x,v^{'''}) \in {\mathcal{O}_{St}}  \big( \left( W^{'}(x) F\mathit{st} R^{''} (x) \right)  \wedge  \left( \mathit{R}^{'''}(x) F_\mathit{st}^{'} W^{''''} (x)  \right)  \wedge \left( v^{''} = v \right) \wedge \left( v^{'''} = v^{'} \right)  \big) \rightarrow   \big( \left( W^{'}(x) \preccurlyeq(x)_{st} W^{'''} (x) \right) \wedge \left( W^{'}(x) \preccurlyeq(x)_{{st}^{'}} W^{'''} (x) \right)  \big).
$. 
