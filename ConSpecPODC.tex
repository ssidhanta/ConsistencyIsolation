...
%\end{figure}
 ...
 \section{Related Work} \label{sec:related}
  As indicated in the CAP theorem \cite{brew:cap}, and proved formally by Gilbert et al. \cite{Gilbert:2002:BCF:564585.564601}, it is not possible to provide strong consistency in a system while providing high availability even in presence of partitions.  By strong consistency, Brewer referred to limerarizability  \cite{Herlihy:1990:LCC:78969.78972}, or atomic consistency; which refers to the property that writes should occur in a system instantaneously, i.e., a read operation should always return result of the latest writes. A group of applications, such as the ones that use shared memory implementations, assume that partitions are rare, and favour stronger consistency semantics. There has been considerable work done in the frontier of distributed shared memory systems for precisely defining the semantics of different consistency models using formal semantics \cite{Ahamad94causalmemory:, Attiya:1994:SCV:176575.176576, Herlihy:1990:LCC:78969.78972, Lamport:1979:MMC:1311099.1311750, opac-b1024856, Mizuno1995, Raynal1995}.  
  \par On the other hand, there is a general lack of formalization of consistency models implemented in the growing ecosystem of group of modern web-based applications. These systems typically trade off stronger semantics for weaker versions of consistency, favouring availability and partition tolerance \cite{Lakshman:2010:CDS:1773912.1773922, Sumbaly_servinglarge-scale, Meiklejohn:2013:RPD:2505305.2505309, Plugge:2010:DGM:1869938, Schutt:2008:SRT:1411273.1411280, DeCandia:2007:DAH:1323293.1294281}. This is particularly true for production-level large-scale geo-replicated systems that keep multiple copies of data distributed across a number of replica servers located in geographically distributed datacenters, to serve clients in different geographic regions with low latency. According to specific application behaviour and correctness requirements, many different consistency models have been defined and implemented till date to support weaker semantics, each specifying different constraints and restriction with respect to the ordering among observed operations. Bayou \cite{bayou} was the first work which defined a group weak session guarantees in  prosaic  form, though very specific to the target implementation discussed in the paper.  The paper by Chockler et al. is the most comprehensive collection  of consistency definitions \cite{Chockler2000}, which is agnostic to protocol and design level details, making them flexible; though they lack axiomatic formulation and generic format. Mahajan et al. \cite{mahajan11cacTR} defined Real Time Causal Consistency, a stronger form of causal consistency, and proved that it is the strongest consistency model that can be implemented into a highly available and eventually consistent implementation.  
Burckhardt et al. \cite{Burckhardt:2014:PEC:2693641.2693642} provides a  formal way of expressing consistency semantics in the form of axiomatic specification of system behaviour, using   properties like visibility and happens before order as building blocks, but in the end, resulting in complicated formalization. Recently, Attiya et al.  \cite{Attiya:2015:LHE:2767386.2767419} have provided formal specification of systems which implement causal consistency, and proved that observable causal consistency, a stronger variant of causal consistency, is the strongest consistency model possible to be provided in a highly available manner tolerating partitions.  Thus, the current literature of databases lack a generalized formulation of different consistency models using formal syntax.
...
A \emph{session trace} $\mathit{st}$ is a sequence of operations executed by the same client, ordered by the time when they were invoked. In this paper we assume that clients are well-formed, i.e., a client only invokes an operation after the preceding operation has returned its response value. As such, session traces can be modeled as sequences of pairs $\langle \textit{invocation},\textit{response}\rangle$. We denote the set of all possible session traces as $\mathcal{S}_t$. We define a session invocation trace as the sequence of invocations that are obtained from the projection
...
Under incorporating the possibility that the set of possible operations is partitioned into two classes, we denote a write operation that writes a value $v$ to an object $x$, as $w(x,v)$. Similarly, a read operation on object $x$ that outputs a value $v^{'}$ is denoted $r(x){v^{'}}$. %{\bf [RR: I am inclined to remove the subscript $j$ and $k$.]}
...
if there exists a partial order $\left( {\mathcal{O}_{St}}_{St}, \preccurlyeq \right)$ over the set ${\mathcal{O}_{St}}_{St}$ comprising operations  present in all session traces in $\mathcal{S}_t$, where  ${\mathcal{O}_{St}}_{St} = \bigcup_{st \in St} \{ o \mid o^{'} \in st \},$  
...
% the same irrespective of the
 ...
 and 2) $\left( {\mathcal{O}_{St}}_{St}, \preccurlyeq \right)$ obeys $E^s$, which is an LTL expression restricting $\left( {\mathcal{O}_{St}}_{St}, \preccurlyeq \right)$.
\end{definition}
...
Condition 1, when applied to a system whose interface consists only of read and write, translates to a requirement that every read operation in $\mathcal{S}_t$ must return the value of the most recent write according to $\preccurlyeq$.
  Condition 2 can be expressed as $ E^s\; \vDash \; \left( {\mathcal{O}_{St}}_{St}, \preccurlyeq \right),$ where %$R$ is a temporal operator, 
...
$\Pi(\mathcal{S}_t,E^s) \equiv  \{\preccurlyeq:  E^s\; \vDash \; \left( {\mathcal{O}_{St}}_{St}, \preccurlyeq \right)\}$
\end{definition}
...
\par \textbf{Proof:} We start by proving the implication in the direction $(\rightarrow)$. Following the proof style of Gilbert et al.~\cite{Gilbert:2002:BCF:564585.564601}, we prove this by contradiction as follows.  Let us assume, by contradiction, that consistency model $E^s$ is implemented by an algorithm that is highly available during partitions but does not meet the condition at the end of the Theorem. The fact that $E^s$  does not meet this condition means precisely that there must exist a global session trace $\mathcal{S}_t$ with a valid partial order $\prec$ that has a maximum element $o_{s1}$ such that $o_{s2}\prec o_{s1}$, where $o_{s1},o_{s2}$ belong to different sessions (from clients $c_1$ and $c_2$, respectively), and where it is not admissible to have a partial order where $o_{s2}\not\prec o_{s1}$. Without loss of generality, consider that $c_1$ and $c_2$ are the only two clients in the system. {\bf [CHECK IF IT REALLY DOES NOT LOSE GENERALITY!]}
