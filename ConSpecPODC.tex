...
% \end{theorem} \label{th:cap}
 %To elaborate, if a partial order enumeration exists for the specification  $E^s$  of a consistency model $\mathcal{C}$  that imposes constraints on operations from different sessions (i.e., clients),  $\mathcal{C}$ is regarded as a CAP-strong consistency model.   
 For instance, the consistency models Causal, SC, PC, and TSO are CAP-strong, since any valid $\mathcal{S}_t$ for  these consistency models require the existence of a valid partial order, hence a valid partial order enumeration, comprising operations performed from different sessions. %Hence, according to Theorem \ref{thrm:cap}, these consistency models can not be implemented in a highly available and partition tolerant manner. 
...
\section{Deriving ConSpec Specifications From Original Definitions}\label{sec:derive}
 ...
 $\mathit{o}^2$ a pair of  operations invoked from a session $\mathit{st}$ of a process $ p_i$, which  read or write to an object $o$ in the system % , such that $\mathit{o}^1$ is a write operation that updates
...
%\end{align}.
 ...
 MR is expressed as: if the condition  (Condition 1) $\mathit{o}^1 \xrightarrow{\sigma} \mathit{o}^2$ holds for a given execution sequence $\sigma$, where both $\mathit{o}^1$ and $\mathit{o}^2$ are read operations,
  ...
  $\mathit{o}^1 \xrightarrow{S} \mathit{o}^2$ holds.
...
\subsection{Derivations of Rest of the ConSpec Specifications}\label{sec:restderive}
  Chockler et al. states WFR consistency as: if the condition (Condition 1) 
...
\emph{direct precedence relation} (which we denote as $\xRightarrow{\sigma}$) between operations $\mathit{o}$
 and$\mathit{o}^{'} $ in the execution order $\sigma$ as follows. $\mathit{o} \xRightarrow{\sigma} \mathit{o}^{'} $
  implies that either $\mathit{o}^{'} $ reads values written by operation $\mathit{o}$, or  a precedence relationship
exists in the execution order $\sigma$ between $\mathit{o}$  and $\mathit{o}^{'} $, i.e.,
$\mathit{o} \xrightarrow{\sigma}\mathit{o}^{'} $ exists. The former part of the precondition can be expressed in the
...
precondition holds, the condition $\mathit{o} \xrightarrow{S}\mathit{o}^{'} $ must hold,
i.e., an occurrence of $\mathit{o}$ must be followed by an occurrence of $\mathit{o}^{'} $ in any
...
%\left( \not\exists {W^m}^{'} F {W^n}^{'} F {\mathit{Op}^j}^{'} \in \mathit{st} \right) \big) \big) \big)$
 ...
 can be given  in terms of a series of LTL expression that express the precedence of successive operations in the sequence.
  The precedence relation between any two successive operations $\mathit{o}$ and $\mathit{o}^{'}$ can be expressed as
