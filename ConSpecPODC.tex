...
It is possible for a storage system to satisfy a consistency model $E^s$ while simultaneously providing high availability and partition tolerance if and only if for any global session trace $\mathcal{S}_t$ and all of its valid partial orderings, when you consider the set of maxima of each partial order, it is always possible to make them depend only on the previous operation in the same session and still obtain a valid partial order, i.e.:
...
\noindent $\forall \preccurlyeq \in \Pi(\mathcal{S}_t,E^s) ~ \forall o \in max(\mathcal{S}_t, \preccurlyeq) ~ (\textsc{removeallexceptsession}(\preccurlyeq,o) \in \Pi(\mathcal{S}_t,E^s) ~ ) $

where we define {\sc removeallexceptsession} as a partial order where the maxima are only ordered with respect to the previous operation in the same session, i.e.:
...
Now let us construct an execution:

[run with whatever network conditions that lead to partial order up until s1 and s2 are invoked. Then partition forever. Now s1 and s2 must conclude and s1 must depend on s2 (check if latter is true.)]

Now we consider an execution of the system where the above mentioned algorithm runs in an environment where each client invokes the sequence of operations in the corresponding sessions of $\mathcal{S}_t$, the network is behaving such that clients $c_1$ and $c_2$ are partitioned from each other (and within each partition we have the asynchronous model), and that leads to $\prec$ being its valid partial order. 


In the execution we are considering, by the availability and partition-tolerance properties, the operations $o_{s1}$ and $o_{s2}$ will eventually complete and, by our initial assumption in the previous paragraph, the former operation saw the effects of the latter, i.e., the partial order that supports that execution is such that $o_{s2}\prec o_{s1}$. Now run the exact same execution, but where the network behavior is modified so that all the processes in the same side of the partition as $c_2$ crash immediately at time zero. Because of the partition, this execution is indistinguishable from all the processes on the same side of the partition as $c_1$ (including $c_1$ itself). Thus $c_1$ will follow the same sequence of states and produce the same outputs as in the previous execution. This would mean that the algorithm would not meet its ConSpec specification, since $o_{s1}$ would reflect the execution of an operation that was not part of the global session trace $\mathcal{S}_t$, namely $o_{s2}$. This contradicts the fact that the algorithm that was used meets that specification and the CAP properties.

Next we focus on the implication in the direction $(\leftarrow)$. Here, we need to prove that if a consistency model $E^s$ meets the condition that for any global session trace $\mathcal{S}_t$:

\noindent $\forall \mathit{st}, \mathit{st}^{'} \in \mathcal{S}_t ~ \exists \preccurlyeq \in \Pi(\mathcal{S}_t,E^s) ~ \forall o, o' \in \mathcal{S}_t (o \in \mathit{st} \wedge o' \in \mathit{st}^{'} \wedge \mathit{st} \not= \mathit{st}^{'} \rightarrow o \not\preccurlyeq o' \wedge o' \not\preccurlyeq o)$
...
Given any global session trace $\mathcal{S}_t$, we prove this by induction on the length of the execution that produced $\mathcal{S}_t$. {\bf [RR: We need to define mathematically the execution model.]} The base case with an empty execution is vacuously true, since an empty trace meets any consistency condition (no safety properties are ever violated by an empty trace). For the induction step, we need to prove that, given an execution for which an available and partition-tolerant implementation produced a trace that conforms to $E^s$, it is possible for such implementation to receive a new operation and produce an output that is also consistent. Suppose, by contradiction, that it is not. ...
