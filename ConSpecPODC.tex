...
%\not\exists  {W^n}^{'} \in \mathcal{S}_t \left( {W^m}^{'} F {W^n}^{'} F {\mathit{Op}^j}^{'} \in \mathit{st} \right) \big) \big) \big)
 ...
 \rightarrow {\mathit{Op}}^{'} \preccurlyeq {\mathit{Op}}^{''}  \wedge W^{'}(x) \preccurlyeq(x)_{st} W^{'} (x) \rightarrow W^{'}(x) \preccurlyeq(x)_{{st}^{'}} W^{''} (x)  \big),  
 $ where $ \preccurlyeq(x)_{st}$ and $\preccurlyeq(x)_{{st}^{'}}$ are  partial orders  over operations on a given object $x$ restricted to  the elements of sessions $\mathit{st}$ and $\mathit{st}^{'}$, respectively.  
 ...
 \rightarrow {\mathit{Op}}^{'} \preccurlyeq {\mathit{Op}}^{''}  \wedge W^{'}(x) \preccurlyeq(x)_{st} W^{''} (x) \wedge W^{'} (x) \preccurlyeq(x)_{{st}^{'}} W^{''}(x)  \big)$. 
 Next, the partial order relations  ${\mathit{Op}}^{'} \preccurlyeq {\mathit{Op}}^{''} $, $  \left( W^{'}(x) \preccurlyeq_{\mathit{st}} W^{''} (x)   \right)$, and $ \left( W^{'}(x)  \preccurlyeq_{{st}^{'}}  W^{'} (x)  \right)$ can be expressed in terms of the corresponding partial order sets  ${\mathit{Op}}^{'} \preccurlyeq {\mathit{Op}}^{''} $, $\left( \mathcal{O}, \preccurlyeq \right)$, $\left( \mathcal{O}, \preccurlyeq_{\mathit{st}} \right)$  and $\left( \mathcal{O}, \preccurlyeq_{{st}^{'}} \right)$. By that reasoning, the condition $ {\mathit{Op}}^{'} \preccurlyeq {\mathit{Op}}^{''}  \wedge W^{'}(x) \preccurlyeq(x)_{st} W^{''} (x) \wedge W^{'} (x) \preccurlyeq(x)_{{st}^{'}} W^{''}(x)$ can be expressed in terms of an intersection $\left( \mathcal{O}, \preccurlyeq^{'} \right)$ of the partial order sets  $\left( \mathcal{O}, \preccurlyeq \right)$, $\left( \mathcal{O}, \preccurlyeq_{\mathit{st}} \right)$,  and $\left( \mathcal{O}, \preccurlyeq_{{st}^{'}} \right)$, where $ \preccurlyeq^{'} $ denotes the partial order relation  among elements of the new partial order set formed out of the intersection of $\left( \mathcal{O}, \preccurlyeq_{\mathit{st}} \right)$, $\left( \mathcal{O}, \preccurlyeq_{\mathit{st}} \right)$, and $\left( \mathcal{O}, \preccurlyeq_{{st}^{'}} \right)$. Thus, CusalC can be expressed as follows. 
