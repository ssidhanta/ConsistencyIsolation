...
%   \left( v_i = v_j \right) \big)
 \rightarrow \left( {\mathit{Op}}^{'} \preccurlyeq_{st+w} {\mathit{Op}}^{''} \right) \big), 
   %; \\
...
%\not\exists  {W^n}^{'} \in \mathit{St} \left( {W^m}^{'} F {W^n}^{'} F {\mathit{Op}^j}^{'} \in \mathit{st} \right) \big) \big) \big)
 \end{split}
  ...
  where $st$ is the session in context of session guarantees, and $\preccurlyeq_{st+w}$ denotes the restriction of $\preccurlyeq$ to the elements of $st$ and the write operations of all other clients. 
 %\begin{align*}
