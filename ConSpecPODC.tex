...
We start with the Read Your Writes (RYW) session guarantee, which informally precludes a read operation $r$ from reading a value 
...
%  \not\exists {W^m}^{'} \left( {W^l}^{'} F {W^m}^{'} F {R^j}^{'} \in \mathit{St} \right) \big) \big).
  \end{split}
  \end{align}\label{eqn:RYW} where the partial order $\left( \mathcal{O}, \preccurlyeq \right)$ restricts the   order among conflicts pair of writes and reads in an equivalent serialization of a partial execution $S_p$, defined in Definition \ref{def:parser}.
