...
\end{split}
\end{align} where $st$ is the session from the standpoint of which the session guarantees are being upheld, and $\preccurlyeq_{st+w}$ denotes the restriction of $\preccurlyeq$ to the elements of $st$ and the write operations of all other clients.
...
E^{s} =  \forall \mathit{st} \in \mathit{St}, {R}^{'}, {W}^{'} \in \mathit{st} \big( \left( {R}^{'} F {W}^{'} \in \mathit{st} \right)
...
E^{s} = \forall \mathit{st} \in \mathit{St}, {W}^{'}, {W}^{''} \in \mathit{st} \big( {W}^{'} F {W}^{''} \in \mathit{st}  \rightarrow \\ \left( {W}^{'} \preccurlyeq_{st+w} {W}^{''} \right) \big), %; \\
...
\end{split}
  \end{align} where $st$ denotes the local session trace, and $\preccurlyeq_{st+w}$ denotes the restriction of $\preccurlyeq$ to the elements of $st$ and the write operations of all other clients. %where the partial order $\left( \mathcal{O}, \preccurlyeq \right)$ restricts the order among conflicting writes  in an equivalent $S_p$.
   %\begin{align}\label{eqn:MW}
%\begin{split}
...
%  $\left(\mathit{Op}^1 X^{o} \mathit{Op}^2\right)_{o}^\mathit{po}$.
  ...
  $S_p$ is equivalent to $S_i$, i.e., both notations represent a legal serialization comprising all
   ...
   $W^{'}$ and $R^{''}$.  Since $W^{'} F R^{''} \in S_p$ denotes a partial order among $W^{'}$ and $R^{''}$, we can rewrite the above expression as  $W^{'} \preccurlyeq R^{''}$. Thus, Chockler's definition reduces into the specification given in Equation \ref{eqn:RYW}. 
...
%\end{align}.
 ...
 As in the case of RYW, the expressions $\mathit{op}^1 \xrightarrow{\sigma} \mathit{op}^2$,
  and $\mathit{op}^1 \xrightarrow{S} \mathit{op}^2$  can be rewritten as LTL expressions
   $R^{'} F R^{''} \in \mathit{st}$ and $R^{'} F R^{''} \in S_p$, respectively. Again, since $R^{'} F R^{''} \in S_p$ denotes a partial order among $R^{'}$ and $R^{''}$, we can rewrite the above expression as  $R^{'} \preccurlyeq R^{''}j$, thus reducing the above specification  into Equation \ref{eqn:MR}. 
...
%  $\left(\mathit{Op}^1 X^{o} \mathit{Op}^2\right)_{o}^\mathit{po}$.
  ...
  $S_p$ is equivalent to $S_i$. Since $R^{'}  F W^{''}  \in S_p$ denotes a partial order among $R^{'}$ and $W^{''}$, we can rewrite the above expression as  $R^{'}  \preccurlyeq W^{''}$. Thus, Chockler's definition reduces into the specification given in Equation \ref{eqn:WFR}. 
  % Further,
...
%  $\left(\mathit{Op}^1 X^{o} \mathit{Op}^2\right)_{o}^\mathit{po}$.
  ...
  $S_p$ is equivalent to $S_i$.  Further,
   ...
   $\mathit{Op}^1$ and   $\mathit{Op}^2$ in both Condition 1 and Condition 2 can safely be  replaced  by new
   propositional variables  $W^{'}$ and $W^{''}$. Since $W^{'}  F W^{''}  \in S_p$ denotes a partial order among $W^{'}$ and $W^{''}$, we can rewrite the above expression as  $W^{'}  \preccurlyeq W^{''}$. Thus, Chockler's definition reduces into the specification given in Equation \ref{eqn:MW}. 
...
\section{Conclusions}
In this paper, we presented precise formal definitions of existing body of consistency semantics and isolation levels using a generalized specification language, ConSpec. In contrast with the existing definitions which are tied to the specific syntax, protocols, and implementation-level details of a particular class of storage systems, our definitions are implementation-agnostic and flexible.  Thus, our definitions can be used as a standard baseline for  verifying the consistency and isolation levels of existing storage systems, as well as for developing correct implementations of new prototype storage systems. From the above generalized definitions, we have presented a  technique of organizing  consistency models into a hierarchical arrangement, based on their degree of strength. We have extended the CAP theorem to state the property which makes a it possible for  a consistency model to be implemented in a manner that can achieve both high availability and partition tolerance, and formally proved the same.  We also provided formal  proofs showing the equivalence of each our consistency and isolation definitions with the existing state-of-the-art definitions.  We also presented an automated tool for verifying whether a given session trace satisfies a given consistency model or isolation level according to the generalized specification.  We have shown how verification tools based on ConSpec specifications (like the one we presented) should perform better and more reliably than current state-of-the-art tools. In future, we plan to keep working in this direction, and build an end-to-end framework for building and verifying correct applications and storage systems, based on ConSpec .  
...
\subsection{Complexity of Verification: Comparison with the State-of-the-art}\label{sec:complex}
 A session trace is comprised of results of execution of storage operations invoked from a client application on a target storage system, observed from a client machine. Let $\mathit{op}$ be signature of an 
