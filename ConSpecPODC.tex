...
\end{split}
  \end{align} where the partial order $\preccurlyeq$ is a partial order over operations in a global execution comprising operations executed by all clients executing on the system, and $\left( \mathcal{O}, \preccurlyeq \right)$ is the corresponding partial order set. 
...
%  \end{align} where the partial order $\left( \mathcal{O}, \preccurlyeq \right)$ is a partial order over operations in a global execution comprising operations executed by all clients executing on the system. %It restricts an equivalent serialization of a client $S_c$, defined in Definition \ref{def:clientser}, to respect the order of sucessive operations in a given session trace. %where $\mathit{op}_i$ is any operation (read or write), and $\mathcal{S}$ is the set of all session traces for a given client application. % scope.
 \par Processor Consistency \cite{Ahamad:1993:PPC:165231.165264} specifies: operations performed from each client application must be observed by all clients (i.e., must occur in all  session traces) according to their invocation orders. In other words, the order in which any pair of  operations, invoked from a particular client, are observed in session traces of all other clients must always respect the invocation order of the above operations. Reads against  write operations should
...
\forall x, y, \mathit{st}, \mathit{st}^{'} \in \mathit{St}  \left( {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (y) \in \mathit{st} \right) \rightarrow  \left(  \left( {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (y) \preccurlyeq_{st} \in \mathit{st}  \right) \wedge  \left( {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (y)  \in \preccurlyeq_{{st}^{'}}  \right) \right),
\end{split}
  \end{align}
...
%\end{split}
%  \end{align} 
   where $ \preccurlyeq_{st}$ and $ \preccurlyeq_{{st}^{'}}$ are partial orders restricted to operations performed by clients $\mathit{Cl}$ and $\mathit{Cl}^{'}$, respectively, observed in the respective local session traces $\mathit{st}$ and $\mathit{st}^{'}$. % all data objects with respect to a given global execution.
...
%  \end{align}, where $\preccurlyeq$ is a composition   $\preccurlyeq =  \preccurlyeq^{'} \circ  ... \circ^{'} \preccurlyeq^{''}$and $\circ, ... \circ^{''}$ are again composition operators, like $\cap$ or $\cup$, over partial order sets (posets) $\preccurlyeq^{'}, ...  \preccurlyeq^{''}$ comprising partial orders defined over all data objects.
 \par Owen et al. \cite{Owens:2009:BXM:1616077.1616107} specifies Total Store Order (TSO) as follows. Consider an operation $\mathit{op}(x)$ performed by a given client application  on a given data object $x$ in a storage system. In a given global execution, no client can observe a ``later'' operation  on the same object $x$ before  it observes the result of the operation $\mathit{op}(x)$. This constraint can be interpreted in terms of local session traces as follows. If either of the following conditions hold, there must exist an equivalent serialization for given the global execution that orders  $\mathit{op}(x)$ before $\mathit{op}^{'}(x)$: condition 1: the operation $\mathit{op}^{'}(x)$ is succeeded by another operation $\mathit{op}^{'}(x)$ on the same object  $x$, or condition 2: if there is another pair of operations $\mathit{op}^{''}(x)$ and $\mathit{op}^{'''}(x)$ on $x$ such that $\mathit{op}^{''}(x)$  is followed by $\mathit{op}^{'''}(x)$ in invocation order of a given client,$ and \mathit{op}^{''}(x)$  and $\mathit{op}^{'''}(x)$ are invoked by two  clients after  $\mathit{op}(x)$ and  $\mathit{op}^{'}(x)$ respectively.  The equivalent serialization in the above definition can be replaced by a  partial order  $\preccurlyeq(x)$  over  all operations on a given object $x$ comprised in a given global execution, i.e., comprising all clients executing on a system  %This operation can not succeeded by another operation on the same  object   can not be observed  by any given client (i.e., can not occur in a  given session trace) return a subsequent (or newer) value unless the result of a given write is observed by all client applications, i.e., observed in all  session traces. 
...
E^s = \forall x, \mathit{st}, \mathit{st}^{'} \in \mathit{St} \not\exists \mathit{op}(x), \mathit{op}^{'}(x), \mathit{op}^{''}(x), \mathit{op}^{'''}(x) \in \mathcal{O}  \big( \left( \mathit{Op}^{'}(x) F \mathit{Op}^{''} (x) \in \mathit{st} \right)  \vee \big(  \left( \mathit{Op}^{'}(x) F \mathit{Op}^{'''} (x) \in \mathit{st} \right)  \\ \wedge 
 \left( \mathit{Op}^{''}(x) F \mathit{Op}^{''''} (x) \in \mathit{st}^{'} \right) \wedge \left( \mathit{Op}^{'''}(x) F \mathit{Op}^{''''} (x) \in \mathit{st}^{'} \right) % \wedge\ \left( v^{''} = v \right) \wedge \left( v^{'} = v^{'''} \right)  
 \big) \big) \rightarrow   \left( \mathit{Op}^{'}(x) \preccurlyeq(x) \mathit{Op}^{''} (x) \right),
\end{split}
...
\rightarrow \exists S_c \left( \mathit{Op}^{'} F \mathit{Op}^{''} \in S_c \right)$, where $S_c$ denotes an equivalent legal seriaization of a global execution restricted to operations from the given client (refer to Equation \ref{def:clientser}). Since ${\mathit{Op}}^{'}  F {\mathit{Op}}^{''}  \in S_c$ denotes a partial order among ${\mathit{Op}}^{'} $ and ${\mathit{Op}}^{''}$, we can rewrite the above expression as  ${\mathit{Op}}^{'}  \preccurlyeq {\mathit{Op}}^{''} $. Thus, Chockler's definition reduces into the specification given in Equation \ref{eqn:SC}. 
...
% total number of processors. We can express the condition the propositional logic variable $A$ must hold for any store
% $\mathit{store}_n$ with the expression  $\bigcap_{\mathit{processor}_n}^{\mathit{processors}} A$.
Consider that a given client observes operations ${\mathit{op}}(x), {\mathit{op}^{'}}(y)$ in a temporal order ${\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (y)$ observed in a session trace $\mathit{st}$. Then, no other session trace $\mathit{st}^{'}$ observed by any other concurrent client may observe the operations in an order which violates the temporal order in $\mathit{st}$. This, in turn, implies that the precedence ${\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (y)$  must be respected in partial order sets   $\left( \mathcal{O}, \preccurlyeq_{st} \right)$ and $\left( \mathcal{O}, \preccurlyeq_{st} \right)$ for session traces $\mathit{st}$ and $\mathit{st}^{'}$, respectively. Thus, Processor Consistency can be expressed as 
   $\forall x, y, \mathit{st}, \mathit{st}^{'} \in \mathit{St}  \left( {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (y) \in \mathit{st} \right) \rightarrow  \left( {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (y)  \in \left( \mathcal{O}, \preccurlyeq_{st} \right) \wedge {\mathit{Op}}^{'}(x) F {\mathit{Op}}^{''} (y)  \in \left( \mathcal{O}, \preccurlyeq_{st}^{'} \right)  \right)$. %\\
%E^{s} = C =   \forall \mathit{st} \in \mathit{St} \not\exists \big( \left(  {W^i}^{'}, {W^j}^{'} \in \mathit{st}  \right)
%\wedge \left( {R^k}^{'}, {R^l}^{'} \in \mathit{St} \right) \big) \\
%\big( \left( {W^i}^{'} F {W^j}^{'} \in \mathit{st} \right) \wedge
...
${\mathit{op}}(x), {\mathit{op}^{'}}(x)$  on an object $x$ in the temporal order specified in the global session trace $\mathit{St}$.  This, in turn,  implies there must exist a partial order $\preccurlyeq$ over all operations comprised in  $\mathit{St}$. Thus, $E^s = \forall x, \mathit{st}, \mathit{st}^{'} \in \mathit{St} \not\exists \mathit{op}(x), \mathit{op}^{'}(x), \mathit{op}^{''}(x), \mathit{op}^{'''}(x) \in \mathcal{O}  \big( \left( \mathit{Op}^{'}(x) F \mathit{Op}^{''} (x) \in \mathit{st} \right)  \vee \big(  \left( \mathit{Op}^{'}(x) F \mathit{Op}^{'''} (x) \in \mathit{st} \right)  \\ \wedge 
 \left( \mathit{Op}^{''}(x) F \mathit{Op}^{''''} (x) \in \mathit{st}^{'} \right) \wedge \left( \mathit{Op}^{'''}(x) F \mathit{Op}^{''''} (x) \in \mathit{st}^{'} \right) % \wedge\ \left( v^{''} = v \right) \wedge \left( v^{'} = v^{'''} \right)  
 \big) \big) \rightarrow   \left( \mathit{Op}^{'}(x) \preccurlyeq(x) \mathit{Op}^{''} (x) \right)$. %, i.e., there must exist a common serialization comprising all operations executed from the
