...
\begin{abstract}
 ...
 any possible consistency model or isolation level that a storage system may provide, in a uniform syntax and form,
  ...
  Each new storage system would redefine a consistency model in terms of its design and implementation. The absence of a uniform
   definition makes it impossible to compare and verify the consistency provided by a given target storage system. Existing
...
\section{Introduction}
 %Internet services and applications that use storage systems as backends to store service and application states and
...
% storage system are important factors to be considered. %, as discussed below.
  %We denote the respective client application, session, or transaction, that a storage operation is invoked from, as
%  \emph{application scope} of that storage operation. Depending on the nature and domain of the application, storage system
%  operations may be invoked from special-purpose application scopes, like session, transaction, or client application.
 % Concurrent applications and services invoke storage operations, like read or write, on the underlying storage system, from
%   geographically distributed client locations.
  Development of modern internet-based applications and services requires application developers to take into
   account the consistency \cite{Burckhardt:2014:PEC:2693641.2693642} and isolation level \cite{DBLP:conf/icde/AdyaLO00} provided by the underlying storage system, to ensure correctness and performance
...
%  replicas in a replicated storage system, resulting in increased observed latency, while guaranteeing that an operation always
%   sees latest updated versions of any object in the storage system. This ensures that there are no anomalies in the observed
...
%   result in anomalies in the observed results, but cause lower observed latency.
 Currently, there is a large number of possible consistency models and isolation levels, many of which are not
    clearly defined using formal semantics. The universe of consistency models and isolation levels is, in fact, expanding
     ...
     specific storage system implementations, and not generic enough to be used in the context of a different (newer) storage system.
...
%   definition specifies a combination of valid dependency relations comprising operations on storage system objects.
...
%can be used to specify any consistency model or isolation level that is not specific to any particular target storage system.
...
% Let $j$ be the index of the read operation in the execution sequence of  $\mathit{Cl}$ observed in a session trace.
 ...
 \par  The specifications comprise some special purpose LTL operators. Such operators are often qualified by scope operators
  ...
  \par %State-of-the-art consistency models and isolation levels have been defined in different forms.
  ...
  A serialization is said to be \emph{legal} if every read operation returns the value written by the latest write
   ...
   %Here, we formally define the various forms of legal serialization that will be used to define the different consistency
%  models.
   %Let the symbol $\mathit{st}$ denotes a given session trace for a given client application.
...
% We use the notation $S$ to denote \emph{serialization set},  which is a set comprising a legal
%  serialization for global execution of each client. %, i.e., $S$ is a set comprising a legal serialization of all operations
%   %executed from each client application on a storage system.
%
%   \begin{definition}(Serialization Set)
%   Formally, we define serialization set $S$ $S$ as a set comprising a legal serialization of all operations
%   executed from each client application on a storage system. Thus,
...
%    $S$ =  \[ \Set{S_p \; is \; partial\; serialization\; for\; a\; client\; \mathit{Cl}} {\forall \mathit{Cl} \in \mathit{St}} \]
%       \end{definition}\label{def:ser}
...
%can be used to specify any consistency model or isolation level that is not specific to any particular target storage system. For a given consistency model or isolation level, we derive
...
% Traditional \emph{server trace analysis} approaches perform trace analysis on server level execution traces, i.e.,
% execution traces collected from the server nodes comprising the underlying storage system.  Since the such approaches
...
%  operations $\mathit{op}^i$ and $\mathit{op}^j$, respectively, i.e., either $v_i = v_j$ or $v_i \not= v_j$.
 ...
 comprising a linear arrangement of the operations performed by a client application, that the model allows.
   ...
   %Thus, $E^s$ comprises to forms - \textbf{form-1} and \textbf{form-2};
%    \textbf{form-1} is a conjunction
%   of constraints  imposed on the global execution comprised of operations performed from concurrent clients, and
%   \textbf{form-2} is a conjunction of constraints on an equivalent legal serialization for operations comprising the above
%   global execution. In the former format of specification, the global execution is represented by the set
%   $\mathit{St}$.
%   \par
%  % \begin{definition}(Generalized Specification)
%   %A ConSpec specification is a logical expression of the general form $E^s$ = $E,$ %; \mathcal{C}$,
%    %where $E$ is a conjunction of one or more propositional expression.
%     The \textbf{form-1} format of ConSpec specifications is expressed in the general format $E^s = A \rightarrow B$, where $A$ and $B$ are LTL-like expressions comprising propositional logic variables, composed with both traditional LTL operators and specialised LTL-like operators. $\rightarrow$ is  the traditional ``implies" operator.
%     $A$ is a precondition on the observed results or precedence of conflicting operations observed in a session trace $\mathit{st}$. Thus, $A$ is denoted by
%    an LTL expression of the form
%    $A$ = ${\mathit{Op}^i}^{'} R {\mathit{Op}^j}^{'} \wedge ... \wedge {\mathit{Op}^k}^{'} R {\mathit{Op}^l}^{'}$, where
%    ${\mathit{Op}^i}^{'}$, ${\mathit{Op}^j}^{'}$,  ${\mathit{Op}^k}^{'}$, and  ${\mathit{Op}^l}^{'}$ are LTL expressions denoting the temporal event of execution of operations $\mathit{op}^j$,
%     $\mathit{op}^j$, $\mathit{op}^k$, and $\mathit{op}^l$, respectively. $R$ and $R^{'}$ is an LTL operator. $B$ is a postcondition that comprises a conjunction of one more constraints on the results or precedence of operations  in an equivalent legal serialization for a global execution comprised of operations performed by one or more concurrent clients. Thus, the postcondition $B$ is  expressed in the form $B$ = $\exists \mathit{Ser} \mathit{
%                                        {\mathit{Op}^i}^{'} R^{'} {\mathit{Op}^j}^{'} \in  \mathit{Ser}}$, where $\mathit{Ser}$ is a legal serialization
%     belonging to any of the formats specified in Definitions \ref{def:ser}, \ref{def:clientser}, or \ref{def:parser}.
%     $R^{'}$ is an LTL operator, and $\wedge$ is the logical conjunction operator.
%    Thus, the generic syntax of \textbf{form-1} is
%    \begin{align}\label{form-1}
%    \begin{split}
%    E^s = \left( {\mathit{Op}^i}^{'} R {\mathit{Op}^j}^{'} \in \mathit{st} \wedge ... \wedge {\mathit{Op}^k}^{'} R {\mathit{Op}^l}^{'} \in \mathit{st}
%     \right)\; \rightarrow \\
%      \exists \mathit{Ser} \; \left( {\mathit{Op}^i}^{'} R^{'} {\mathit{Op}^j}^{'} \in  \mathit{Ser} \right).
%    \end{split}
%    \end{align} %, and $<...>$ is a qualifier which denotes that the expression enclosed within the qualifier is optional, i.e., the condition represented by  enclosed expression is not a necessary condition.
%     The \textbf{form-2} format of ConSpec specifications is expressed as $E^s$ = $C$, where $C$ is a conjunction of
%     several LTL expressions, each of which imposes constraints on the results and precedence of operations in the
%     global execution $\mathit{St}$. Thus, $C$ is expressed in the form $C$ = $\mathit{\wedge
%                                        {\mathit{Op}^i}^{'} R {\mathit{Op}^j}^{'} \in \mathit{St}}$.
%     Thus, \textbf{form-2} format of ConSpec specifications is expressed as
%     \begin{align}\label{form-2}
%     \begin{split}
%    E^s = C =  {\mathit{Op}^i}^{'}, {\mathit{Op}^j}^{'}, {\mathit{Op}^k}^{'}, {\mathit{Op}^l}^{'}  \in \mathit{St} \\
%         \left( {\mathit{Op}^i}^{'} R {\mathit{Op}^j}^{'} \wedge ... \wedge  {\mathit{Op}^k}^{'} R^{'} {\mathit{Op}^l}^{'} \right)
%    \end{split}
%    \end{align}
%     \par Here, we show that the specification formats form-1 and form 2, defined in Equations \ref{form-1} and \ref{form-2},
%    are special instances of Definition \ref{def:form0}. %The condition 2 for Definition \ref{def:form0} can be derived from the Equations \ref{form-1}
%   %and \ref{form-2} as follows.
%    First, we consider that $E^s$ is of the form specified according to {form-1 given in
%    Equation \ref{form-1}.  Instead of $\mathit{St}$, form-1 is expressed in terms of an equivalent legal serialization
%    $\mathit{Ser}$ comprising all operations in $\mathit{St}$. Since $\mathit{Ser}$ is a reordering of the operations in
%    $\mathit{St}$, $\mathit{Ser}$ can be expressed as a partial order $\preccurlyeq^{'}$ on $\mathit{St}$, i.e.,
%    $\mathit{Ser} \; \equiv \; \left( \mathit{St}, \preccurlyeq^{'} \right)$. Thus, the term $\mathit{Ser}$ in  form-1 is a subset of
%    the partial order set $ \preccurlyeq , \mathit{St}$. Further, by definition, the binary relations $R, ... ,R^{'}$,
%    are a specialized instance of the partial order $\preccurlyeq$ (refer to Condition 1 in Definition \ref{def:form0}).
%    Hence, the terms ${\mathit{Op}^i}^{'} R {\mathit{Op}^j}^{'} \in \mathit{st} \wedge ... \wedge {\mathit{Op}^k}^{'} R {\mathit{Op}^l}^{'} \in \mathit{st}$
%   in the LHS of form-1  and the term ${\mathit{Op}^i}^{'} R^{'} {\mathit{Op}^j}^{'}$ in RHS of  form-1 satisfy the
%   partial order $\preccurlyeq$. Combining the above conditions, it is trivially observed that LHS and RHS of form-1
%   satisfy $\preccurlyeq$.
%   Following the same line of reasoning we can prove that form-2 (refer to Equation \ref{form-2}) satisfies $\preccurlyeq$;
%   this, in turn, proves Condition 2.
...
%expressed in the form of $B \rightarrow C$,
%because of the absence of notions of $B$ and $C$. Those consistency models are specified as a conjunction of multiple restricting conditions.
 ...
 Processor consistency and Total Store Order, which are not defined in terms
  ...
  a given client, along with only write operations from other clients. Specifications of these consistency models, $E^s$, comprise a
   ...
   % \end{definition}\label{def:long}
    ...
    %In the above generalized expression $\mathcal{E}$, $B$ is a precondition expressed in terms of the of precedence relation among operations observed in a given session trace, $C$ is the necessary and sufficient postcondition that specifies a valid legal serialization for the given consistency or isolation level, and alternatively $\mathcal{A}$ represents the anomaly proscribed by the concerned consistency or isolation level. In other words, $A$ expresses some conditions
...
%      as the complexity of verification (time complexity) is better while considering $C$ instead of $E^s$.
%, are specified using a \emph{restrictive} form of specification, given below.
...
%     \end{definition}\label{def:restrict}
 %\par A serialization is a sequence in which a group of storage operations are  executed on a storage system. A serialization is said to be legal if every read operation returns the value written by the latest write operation preceding it in the serialization. Chockler et al. \cite{Chockler2000} define a consistency model in terms of a legal serialization,
% comprising a linear arrangement of the operations performed by a client application, that the model allows.
...
%  \big( \not\exists {\mathit{Op}^k}^{'} = {W^k}^{'} \left( {\mathit{Op}^i}^{'}  F {\mathit{Op}^k}^{'} F {\mathit{Op}^j}^{'} \right) \big) \wedge \left( {v_j}^{'} = {v_i}^{'} \right) \big)$.
%  \end{definition}\label{def:parser}
   %We use the notation $S$ to denote a legal
...
%   a sequence of propositional variables ${\mathit{Op}^i}^{'}$, each variable ${\mathit{Op}^i}$ denoting an execution of an
%    operation ${\mathit{op}}^i$ in a global session trace $\mathit{St}$, as follows.
%    $S$ = $\left( {\mathit{Op}^i}^{'} \right),$ ${\mathit{Op}^i}^{'} \in \mathit{St}$, such that
%  $\forall \left( {\mathit{Op}^i}^{'} = {W^i}^{'} \wedge {\mathit{Op}^j}^{'} = {R^j}^{'} \right)
%  \big( \left( {\mathit{Op}^i}^{'} F {\mathit{Op}^j}^{'} \right) \wedge \\
%  \big(\not\exists {\mathit{Op}^k}^{'} = {W^k}^{'} \left( {\mathit{Op}^i}^{'}  F {\mathit{Op}^k}^{'} F {\mathit{Op}^j}^{'} \right) \big) \wedge \left( {v_j}^{'} = {v_i}^{'} \right) \big)$.
%   \end{definition}\label{def:ser}
%   \begin{definition}(Serialization of a Client)
%   We formally define serialization of a client, $S_c$, as the restriction of the serialization to  the operations invoked by a particular client application, which in turn,
%   corresponds to operations observed in a particular session trace $\mathit{st}$.
%    $S_c$ = $\left( {\mathit{Op}^i}^{'} \right),$ \\
%     $\left( {\mathit{Op}^i}^{'} \in S \wedge {\mathit{Op}^i}^{'} \in \mathit{st} \right)$.
%   \end{definition}\label{def:clientser}
%   \begin{definition}(Serialization of a Partial Execution)
%   The notation $S_p$ is used to denote a legal serialization of a partial execution comprising all    operations
%   performed by a given client application and all write
%  operations  executed from all other concurrent client applications; the above partial execution is denoted as $\sigma |i + w$ by Chockler et al.. Thus, we formally denote $S_p$ as
%  \\ $S_p$ = $\left( {\mathit{Op}^i}^{'} \right),$ $\left( {\mathit{Op}^i}^{'} \in \mathit{st} \right) \vee \left(
%  {\mathit{Op}^i}^{'} = {W^i}^{'} \wedge {\mathit{Op}^i}^{'} \not\in \mathit{st} \right),$ such that
%  $\forall \left( {\mathit{Op}^i}^{'} = {W^i}^{'} \wedge {\mathit{Op}^j}^{'} = {R^j}^{'} \right)
%  \big( \left( {\mathit{Op}^i}^{'} F {\mathit{Op}^j}^{'} \right) \wedge \\
%  \big( \not\exists {\mathit{Op}^k}^{'} = {W^k}^{'} \left( {\mathit{Op}^i}^{'}  F {\mathit{Op}^k}^{'} F {\mathit{Op}^j}^{'} \right) \big) \wedge \left( {v_j}^{'} = {v_i}^{'} \right) \big)$.
%  \end{definition}\label{def:parser}
 ...
 \section{List of ConSpec Specifications for Various Consistency and Isolation Levels}
  ...
  operations  executed from all other concurrent client applications. Following Definition \ref{def:form0}, the above RYW definitions are reduced to LTL-like expression, as follows.
...
% \rightarrow \; \exists S_p \left(  {W^i}^{'} F {R^j}^{'} \in S_p \right)\big)
...
%\rightarrow \\ \bigcap_{\mathit{store}_n}^{\mathit{stores}} {\forall S \left( \mathit{Op}^i X W^j \in S \right) }
%\wedge \not\exists R^k  \left( \mathit{Op}^i X^o W^j X^o R^k \in \mathit{st} \right) \\ \wedge \left( v^k = v^i  \right) \big)
%\forall i, j, o, \mathit{st} \in \mathit{St} \big( {W^i}^{'} F {W^j}^{'} \in \mathit{st} \rightarrow  \exists S \left( {W^i}^{'} F {W^j}^{'} \in S \right) \wedge \\
...
\end{split}
  \end{align}
 \par Processor Consistency specifies: operations in a client application must execute according to the invocation order
...
%\\ \wedge  \not\exists R^k_\mathit{tx} \in \mathit{st} \left( W^i_\mathit{tx} F W^j_\mathit{tx} F R^k_\mathit{tx} \in \mathit{st} \right) \wedge \left( W^j_\mathit{tx} F R^k_\mathit{tx} \in \mathit{st} \right) \\ \wedge \left( v^k = v^i \right) \big),
 E^s = C = \forall  \mathit{st} \in \mathit{St}, \mathit{tx}, \mathit{ty} \in \mathit{St}, x, y,\\
...
\section{Rewriting CAP Theorem in terms of ConSpec}
 ...
 CAP-strong, i.e., strong enough to be bounded by the constraints of CAP theorem, as follows. %. We rewrite CAP as follows.

\begin{theorem}[Extended CAP theorem] 
 It is possible for a storage system to satisfy a consistency model that enforces a strict partial order over operations performed by concurrent clients executing on the system, while simultaneously providing high availability and partition tolerance.
\end{theorem} \label{th:cap}
 To elaborate, if the specification $E^s$ (following the generic specification format given in Definition \ref{def:form0}) of a consistency model imposes a strict partial order $\preccurlyeq$ among operations performed by concurrent clients executing on underlying storage system (i.e., among  operations comprised in a global execution represented by a global session trace $\mathit{St}$), that consistency model is regarded as a CAP-weak consistency model. On the other hand, a consistency model that imposes a total order $\preccurlyeq$ on  $\mathit{St}$ is regarded as CAP-strong. It directly follows from Theorem \ref{th:cap} that the CAP-strong condition, in turn,
 implies that the specified partial order $\preccurlyeq$ as per Definition \ref{def:form0} must be a total order on $\mathit{St}$. In other words, for a CAP-strong consistency model, Condition 1 in  Definition \ref{def:form0} can be rewritten as 
 ...
 must hold for all operations
  $\mathit{op}^i$ and $\mathit{op}^j$ comprised in a global execution, represented by the observed global session trace
  $\mathit{St}$.
\par \textbf{Proof:} Following the proof style of Glibert et al.  \cite{Gilbert:2002:BCF:564585.564601}, the above theorem is proved as follows. Let us consider a pair of concurrent clients, namely $\mathit{Cl}_1$ and $\mathit{Cl}_2$ executing
...
these components are disconnected, none of these writes are propagated among the servers $N_1$ and $N_2$.
 ...
 for the conflicting object $x$; whereas  $\mathit{St}$, as observed by $\mathit{Cl}_2$,  is a sequence $\alpha, \gamma$.  A consistency model $\mathcal{C}$ that imposes a strict partial order $\preccurlyeq$ on a global execution comprising operations performed by all clients (represented by a global session trace $\mathit{St}$) must, by definition of a strict partial order, satisfy the following properties: 1)  irreflexivity, 2) transitivity, and 3) asymmetry. The results in each of the above  global session traces $\mathit{St}$ are resulting from a linear execution of operations from each client $\mathit{Cl}_1$ and $\mathit{Cl}_2$, i.e., each client executes its operations according to the sequence specified in its respective program order.  Thus each operation can not loop into itself (state diagram for an execution must be devoid of self loops); hence, condition 1 is trivially satisfied in the above execution.  Since the operations performed by clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$ are executed in  a linear temporal order which follows from the program order of each client, there can not be any cycle among the operations in each of the above session traces; hence, condition 2 also holds for the above execution.  The global session traces observed by clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$ comprise sequences of operations, given as  $\alpha, \beta$ and  $\alpha, \gamma$, respectively. Because of the lack of communication of updates between the servers $N_1$ and $N_2$ serving $\mathit{Cl}_1$ and $\mathit{Cl}_2$ in presence of a partition, the global session trace observed by $\mathit{Cl}_1$  satisfies the partial order condition $\alpha  \preccurlyeq \beta$ or $\beta  \preccurlyeq \alpha$; whereas the global session trace observed by $\mathit{Cl}_2$ satisfies a different partial order $\alpha \preccurlyeq \gamma$ or $ \gamma \preccurlyeq \alpha$.  Since $\beta \not= \gamma$, the above session traces satisfy the antisymmetry conditions of a strict partial order $\preccurlyeq$, i.e., Condition 3 is also satisfied. Hence, in presence of partitions, the above system remains available, while satisfying a strict partial order on the global session trace $\mathit{St}$. Thus, such a consistency model can be implemented in a manner which simultaneously achieves high availability and partition tolerance.  
 \begin{corollary}
  It is impossible for a storage system to satisfy a consistency model that enforces a total order  over operations performed by all clients executing on the system, while simultaneously providing high availability and partition tolerance.
 \end{corollary}
 We prove the above corollary by contradiction as follows. Following the approach of the proof to Theorem \ref{th:cap}, we consider two servers $N_1$ and $N_2$ that serve requests from clients  $\mathit{Cl}_1$ and $\mathit{Cl}_2$. We consider that a partition occurs at a particular instant between the servers $N_1$ and $N_2$. Consider that the storage system enforces a consistency model which imposes a total order $\preccurlyeq$ among operations from all concurrent clients executing on the underlying storage system. By definition, apart from transitivity and antisymmetry, a total order $\preccurlyeq$ relation requires satisfaction of the 
\emph{totality} condition, i.e., each pair of operations $\mathit{op}_i$ and $\mathit{op}_j$ in each of the global session traces observed by  the clients  $\mathit{Cl}_1$ and $\mathit{Cl}_2$ must either satisfy $\mathit{op}_i \preccurlyeq \mathit{op}_j$ or $\mathit{op}_j \preccurlyeq \mathit{op}_i$, but not both simultaneously. Let operations $\mathit{op}_i$ and $\mathit{op}_j$  be the prefix of the executions  $\beta$, and $\gamma$ observed by clients $\mathit{Cl}_1$ and $\mathit{Cl}_2$, respectively.  Since $\beta \not= \gamma$ in $\mathit{St}$, hence $\mathit{op}_i \preccurlyeq \mathit{op}_j$ holds in $\beta$, while $\mathit{op}_j \preccurlyeq \mathit{op}_i$ holds in $\gamma$; hence the above condition of totality is violated for the global session traces $\alpha, \beta$ and  $\alpha, \gamma$.  This contradicts the requirement of a total order $\preccurlyeq$ on the
operations  comprised in $\mathit{St}$.  Hence, such a consistency model can not achieve high availability and partition tolerance at the same time. 
...
each client is maintained. Hence, these comprise a weaker class of consistency models that can be satisfied without synchronization
...
\\ \rightarrow
\exists \left( {\mathit{Op}^i}^{'}, {\mathit{Op}^j}^{'} \in \mathit{Ser} \wedge {\mathit{Op}^i}^{'}, {\mathit{Op}^j}^{'} \in \mathit{st} \right) \left( {\mathit{Op}^i}^{'} X^o {\mathit{Op}^j}^{'} \in S_c \right) \big).$
...
comprised of all operations in a global execution. Thus, Strict serializability imposes a total order with respect to operations from all clients comprised in  $\mathit{St}$.
...
% eventually operator $F$ or special-purpose next operator $X^o$) among each pair of operations.
 TSO and PC
 imposes restrictions on the execution order and observed results among all operations performed by all clients observed in a global session trace $\mathit{St}$. Hence, it directly follows from the definitions that TSO and PC impose total order on  $\mathit{St}$, where each operation in $\mathit{St}$ is bounded by a total order relation with another operation in $\mathit{St}$.
 Hence, these consistency models require strict synchronization among operations from different clients, such that
 operations from each client must  be executed in isolation from other concurrent clients.   %Under these consistency models,
% a pair of concurrent clients, namely $\mathit{Cl}_1$ and $\mathit{Cl}_2$, fails to provide high availability guarantees in a partitioned network, where there
% all communication between $\mathit{Cl}_1$  and  $\mathit{Cl}_2$ are lost; hence, they are CAP-strong.
% Thus, CAP-strong consistency models either restrict the observed results or execution order among operations in
% a given global execution (represented by global session trace $\mathit{St}$), or in an equivalent legal serialization $\mathit{Ser}$
% comprising all operations from all concurrent clients.
% Generalizing from the above discussion, a consistency model is CAP-strong whenever it imposes constraints on the
% observed results or execution order among operations in a global execution comprising operations from all clients, or
% in an equivalent legal serialization for a given global execution.
 %For MW, $E^s$ imposes constraints on operations comprised in an equivalent legal serializatiosn $S$,
%consisting of all operations performed by all concurrent clients in a given global execution.
 %PC and TSO
%imposes LTL constraints on operations comprised in the global execution composed of all operations performed
%by all concurrent clients, represented by the global session trace $\mathit{St}$.
%Thus, Strict Serialization, PC, and TSO imposes constraints on the order among operations in a
% global execution  comprising all operations performed by all clients in the storage system,
%  or in an equivalent legal serialization for a global execution.
 % Following the above line of reasoning, we rewrite CAP theorem as follows.
%\begin{figure*}[htpb]
%\centering
%%\noindent
...
% clients: $S_p^1$ = $W^1 F R^2 F W^2 F R^1$, $S_p^2$ = $W^2 F R^1 F W^1 F R^2$,
%  and $S_p^3$ = $R^1 F W^2 F W^1 F R^2$.
% Only for $S_p^1$, the condition $W^1 F R^2 \in S_p^1$ holds, which does not hold for $S_p^2$ and $S_p^3$.
...
%  serialization $S_p^1$ exists for the session trace such that $W^1 F R^2 \in S_p^1$, satisfying the first
...
%  $R^j$ = $R^1$. Again a legal partial serialization $S_p^1$ exists such that $W^2 F R^1 \in S_p^1$.
...
%  $R^j$ = $R^2$. $S_p^1$ is the only legal partial serialization that satisfies the condition $W^i F R^j \in S_p$
%   in the RHS of Equation \ref{eqn:RYW} for all the above write-read operation sequences. However, $S_p$ does not satisfy
%  $W^i F R^j \in S_p$ for the write-read sequence $W^2 F R^2$.
...
%  The above traces can be transformed into legal partial serialization order $S_p$ =
% $W^1 F R^1 F R^3 F W^2 F R^2$, such that $R^1 F R^2 \in S_p$ holds, thus satisfying
...
%  legal partial serialization $S_p$ exists for the above trace, such that $R^1 F R^3 \in S_p$
...
%  as $R^2 F R^3$. However, the above sequence of reads does not match the order among these reads in the serialization  $S_p$, i.e.,
%  $R^2 F R^3 \in S_p$ holds;
...
% the above session traces into a partial serialization $S_p$ =
...
% writes and reads satisfies the condition $W^1 F R^1 \in S_p$, thus satisfying the first condition the RHS of
...
% writes and reads do not satisfies the condition $W^2 F R^3 \in S_p$, violating the RHS of $E^s$ in Equation \ref{eqn:RYW}.
...
%%\\ \rightarrow \exists S_p \left( \forall \mathit{Op}^i F \mathit{Op}^j \in S_p \right)
...
% serialization $S$, which is, in this case, equivalent to  $S_p$ in absence of writes from other
...
% do not satisfy the condition  in the RHS of $E^s$, i.e., it violates the condition $W^1 X R^1 \in S$.
...
% $S_p$  = $W^1 F R^1 F R^2 F W^2$. The write operation $w^1(x,1)$ follows
...
% operations satisfies the condition  $W^1 F R^1 \in S_p$. Thus, the condition $E^s$ in the RHS of
...
%  i.e., $W^i F W^j \in S$. The above traces can be transformed into a legal serialization order $S$ =
...
%\end{align}.
 ...
 As in the case of RYW, the expressions $\mathit{op}^1 \xrightarrow{\sigma} \mathit{op}^2$,
  and $\mathit{op}^1 \xrightarrow{S} \mathit{op}^2$  can be rewritten as LTL expressions
   $R^i F R^j \in \mathit{st}$ and $R^i F R^j \in S_p$, respectively.
...
\rightarrow \exists S_c \left( \mathit{Op}^i X \mathit{Op}^j \in S_c \right)$.
...
\par Total Store Order (TSO or Total Order) is specified by Owen et al. \cite{Owens:2009:BXM:1616077.1616107}
 ...
 a pair of operations on a common storage system object $o$ must be executed in the same precedence order.  %serializations ${S_i}_i$ and ${S_i}_j$, the write operations on common storage system objects in ${S_i}_i$ and
...
%  \end{align}
 ...
 \section{Equivalence between Dependency Relations and LTL Relations of ConSpec}
