...
% $\left( {\mathit{Op}}^{'} \preccurlyeq {\mathit{Op}}^{''} \right) \vee \left( {\mathit{Op}}^{''}  \preccurlyeq {\mathit{Op}}^{'}\right)$, where  $\preccurlyeq$ denotes a partial order relation, and $\left( {\mathcal{O}_{St}}, \preccurlyeq \right)$ is the corresponding partial order set.
 ...
 The generalized definition of isolation definitions is given as follows. 
  \begin{definition}{ConSpec form for Isolation definition:}\label{def:form0}
...
\end{definition}
 Thus, replacing $\left( {\mathcal{O}_{St}}, \preccurlyeq \right)$ with  $\left( T, \preccurlyeq \right)$ in Definition \ref{def:form0}, PL-1 is specified %in the \textbf{form-2} format, given in Equation \ref{form-2},
 as follow.
...
\mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty} \quad \vee \quad
\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx}
\end{split}
  \end{align} where $\mathit{tx}$ and $\mathit{ty}$  are consecutive transactions, and $\preccurlyeq_{\mathit{ww}}$ restricts the partial order $\preccurlyeq$ among write operations in $\mathit{tx}$ and $\mathit{ty}$.
...
E^{s} =   \; \forall x, \; \mathit{tx}, \mathit{ty} \in T: 
 \mathit{tx}^{c}  \preccurlyeq_{\mathit{wr}} \mathit{ty} \quad \vee \quad
\mathit{ty}^{c} \preccurlyeq_{\mathit{wr}} \mathit{tx}  \wedge \\
\mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty} \quad \vee \quad
\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx} \wedge 
\mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty} \quad \vee \quad
\mathit{ty} \preccurlyeq_{\mathit{wr}} \mathit{tx},
\end{split}
  \end{align} where $\mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty}$ restricts the partial order $\preccurlyeq$ among write operations in $\mathit{tx}$ and read operations in $\mathit{ty}$. $\mathit{tx}^{c}$ and $\mathit{ty}^{c}$ denote that $\mathit{tx}^{c}$ and $\mathit{ty}^{c}$ are  committed transactions, i.e., operations in $\mathit{tx}^{c}$ and $\mathit{ty}^{c}$ are followed by commit statements, and have not been aborted. % $a_\mathit{tx}$ and $a_\mathit{ty}$ are the abort statements for the transactions $\mathit{tx}$ and $\mathit{ty}$, respectively. F is the traditional LTL next operator.
...
\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx} \wedge 
\mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty} \quad \vee \quad
\mathit{ty} \preccurlyeq_{\mathit{wr}} \mathit{tx} \wedge
\mathit{tx} \preccurlyeq_{\mathit{rw}} \mathit{ty} \quad \vee \quad
\mathit{ty} \preccurlyeq_{\mathit{rw}} \mathit{tx},
\end{split}
  \end{align} where $\mathit{tx} \preccurlyeq_{\mathit{rw}} \mathit{ty}$ restricts the partial order $\preccurlyeq$ among read operations in $\mathit{tx}$ and write operations in $\mathit{ty}$.
