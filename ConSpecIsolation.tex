...
\begin{split}
 E^{s} =  \; \forall R_{\mathit{ty}}(x){v}, R_{\mathit{ty}}(x){v^{'}} \in \mathcal{S}_t. \; \exists \mathit{tx} \in T: 
\mathit{tx}^{c} \preccurlyeq \mathit{ty} \quad \wedge \quad \mathit{ty} \not \preccurlyeq \mathit{ty} \quad \wedge \quad v = v^{''} \quad \wedge   \quad \not \exists \mathit{tz} \in T: \\ \mathit{tz} \preccurlyeq {\mathit{ty}}. %\quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T:  
%\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx}
\end{split}
  \end{align}
...
\end{split}
  ...
  SI-b: A transaction observes complete effects of transactions that committed before it started.  SI-a can be interpreted as follows.   If a pair of transactions can observe results of each other, they are not concurrent, i.e., $\preccurlyeq$ among a pair of transactions in Definition \ref{def:form0} is a total order $\prec$.  According to SI-b the above total order must be among committed version of $\mathit{tx}$ and $\mathit{ty}$, i.e., $\mathit{tx}^{c} \prec \mathit{ty}$. The above condition reduces to: $ \forall \mathit{tx}, \mathit{ty} \in T,  W_{\mathit{tx} }(x, v),  %W_{\mathit{ty} }(x, v^{'}), %R_{\mathit{tx}}(x){v^{''}},    
...
\left( v = v^{'}%W_{\mathit{ty} }(x, v^{'})  \operatorname{\it F}_\mathit{st} R_{\mathit{tx}}(x){v^{'''}} \quad 
 \mathit{tx}^{c} \prec \mathit{ty} \right)$. Snapshot isolation also includes the repeatable read read. condition, i.e., $\forall \mathit{tx}, \mathit{ty} \in T,  W_{\mathit{tx} }(x, v),  %W_{\mathit{ty} }(x, v^{'}), %R_{\mathit{tx}}(x){v^{''}},    
...
\left( v = v^{'}   %v = v^{''}  \quad
 ...
 %\wedge %\quad W_{\mathit{tx} }(x, v) \operatorname{\it F}_\mathit{st} R_{\mathit{ty}}(x){v^{'}}  \quad \wedge 
  ...
  \mathit{tx}^{c} \prec \mathit{ty} \right) \quad 
   \wedge \\
    \quad  \left( \mathit{tx} \not \preccurlyeq \mathit{tx} \quad \wedge \quad \mathit{ty} \not \preccurlyeq \mathit{ty} \quad \wedge \quad v^{'} = v^{''} \right) \quad \wedge   \left( \quad \not \exists \mathit{tz} \in T: \; \mathit{tz} \preccurlyeq {\mathit{ty}} \right).
