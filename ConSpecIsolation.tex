...
Given a global session trace $\mathcal{S}_t$ comprising a sequence of all operations performed from all transactions comprised in the set $T$ of all transactions executing on a system, % session trace $\mathcal{S}_t$,
% representing a global execution of all concurrent clients on a given storage
%system,
...
%$\left( {\mathcal{O}_{T}}, \preccurlyeq \right)$ over the set ${\mathcal{O}_{T}}$ comprising operations  present in all transactions in $T$, i.e.,  ${\mathcal{O}_{T}} = \bigcup_{{\mathit{tx}} \in T} \{ o \mid o \in {\mathit{tx}} \},$
 such that 1) %${\mathit{tx}} \preccurlyeq {\mathit{ty}}$  implies: 
 if a transaction ${\mathit{tx}}$ precedes a transaction $ {\mathit{ty}}$ in $\preccurlyeq$ and  an operation $o_{\mathit{ty}}$  from ${\mathit{ty}} $ overwrites/returns the value of $x$ resulting from an operation $o_{\mathit{tx}}$ from ${\mathit{tx}}$, 
 then  no operation from ${\mathit{tx}} $ can overwrite/return the result of  $x$ obtained from any operation from ${\mathit{ty}}$, i.e., $\not\exists \; x, \; o(x)_{\mathit{tx}}(v)^{'},  \;  o(x, v^{'} )^{'}_{\mathit{ty}} \in \mathcal{S}_t: \;  v = v^{'}$,   %, in $\preccurlyeq$, invocation and response of all operations in ${\mathit{tx}}$ precede invocation and response of all operations in ${\mathit{ty}} $ on a common object   
...
Condition 1 ensures  that result of no operation in  ${\mathit{ty}} $ can be observed before results of all operations in ${\mathit{tx}}$ are observed. 
 Isolation levels are defined in terms of a partial order $\left( T, \preccurlyeq \right)$ 
 %$\left( {\mathcal{O}_T}, \preccurlyeq \right)$ over a set ${\mathcal{O}_T}$ comprising operations performed from all transactions ${\mathit{tx}} \in T$ 
 where $T$ is the set of all transactions executing in the system.   Thus, %replacing $\left( {\mathcal{O}_{St}}, \preccurlyeq \right)$ with  $\left( T, \preccurlyeq \right)$ in Definition \ref{def:form0}, 
 The isolation level PL-1 mandates that write operations in  a transaction must be executed such that write-write
 dependency cycles among transactions are proscribed. In terms of ConSpec, PL-1 is specified %in the \textbf{form-2} format, given in Equation \ref{form-2},
 as follow.
...
\mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty} \quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T:  
\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx}
\end{split}
  \end{align} where $\mathit{tx}$ and $\mathit{ty}$  are consecutive transactions. $\preccurlyeq_{\mathit{ww}}$ restricts the partial order $\preccurlyeq$ among write operations in $\mathit{tx}$ and $\mathit{ty}$, i.e., $\mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty}  \;  \equiv \; \forall {\mathit{W}}_ \mathit{tx} \in \mathit{tx}, {\mathit{W}}_ \mathit{ty} \in \mathit{ty}: {\mathit{W}}_ \mathit{tx} \preccurlyeq {\mathit{W}}_ \mathit{ty}$.
...
\mathit{tx}^{c} \preccurlyeq_{\mathit{wr}} \mathit{ty}^{c} \quad \vee \quad \forall x, \; \mathit{tx}^{c}, \mathit{ty}^{c} \in T: \mathit{ty}^{c} \preccurlyeq_{\mathit{wr}} \mathit{tx}^{c} \quad \wedge \\
...
\end{split}
  \end{align} where $\mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty}$ restricts the partial order $\preccurlyeq$ among write operations in $\mathit{tx}$ and read operations in $\mathit{ty}$. Thus, , i.e., $\mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty}  \;  \equiv \; \forall {\mathit{W}}_ \mathit{tx} \in \mathit{tx}, {\mathit{R}}_ \mathit{ty} \in \mathit{ty}: {\mathit{W}}_ \mathit{tx} \preccurlyeq {\mathit{R}}_ \mathit{ty}$. $\mathit{tx}^{c}$ and $\mathit{ty}^{c}$ denotes committed transactions, i.e., all operations in $\mathit{tx}^{c}$ and $\mathit{ty}^{c}$ are followed by commit operations $c_\mathit{tx}$ and $c_\mathit{ty}$,respectively. It further implies that $\mathit{tx}$ and $\mathit{ty}$ has not been aborted, i.e., they do not comprise abort operations $a_\mathit{tx}$ and $a_\mathit{ty}$, respectively. % $a_\mathit{tx}$ and $a_\mathit{ty}$ are the abort statements for the transactions $\mathit{tx}$ and $\mathit{ty}$, respectively. F is the traditional LTL next operator.
...
\mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty} \quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T:  
\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx} \quad \wedge \\
\forall x, \; \mathit{tx}, \mathit{ty} \in T: \mathit{tx} \preccurlyeq_{\mathit{rw}} \mathit{ty} \quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T: 
\mathit{ty} \preccurlyeq_{\mathit{rw}} \mathit{tx},
\end{split}
  \end{align} where $\mathit{tx} \preccurlyeq_{\mathit{rw}} \mathit{ty}$ restricts the partial order $\preccurlyeq$ among read operations in $\mathit{tx}$ and write operations in $\mathit{ty}$, i.e., $\mathit{tx} \preccurlyeq_{\mathit{rw}} \mathit{ty}  \;  \equiv \; \forall {\mathit{R}}_ \mathit{tx} \in \mathit{tx}, {\mathit{W}}_ \mathit{ty} \in \mathit{ty}: {\mathit{R}}_ \mathit{tx} \preccurlyeq {\mathit{W}}_ \mathit{ty}$..
