...
\mathit{tx}^{c} \preccurlyeq \mathit{ty}. %\quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T:  
...
\end{split}
 ...
 W_{\mathit{tx} }(x, v) \operatorname{\it F}_\mathit{st} R_{\mathit{tx}}(x){v^{''}} \quad \wedge \quad v = v^{''}  
 \wedge \quad W_{\mathit{ty} }(x, v^{'})  \operatorname{\it F}_\mathit{st} R_{\mathit{tx}}(x){v^{'''}} \quad \wedge \quad  \quad v^{'} = v^{'''} \quad  \Rightarrow  \quad \mathit{tx}^{c} \prec_{\mathit{ww}} \mathit{ty} \quad \wedge \quad \mathit{tx}^{c} \prec_{\mathit{wr}} \mathit{ty} $. SI-b can be interpreted as: if a transaction  ${\mathit{ty}} $ observes values from another transaction  ${\mathit{tx}} $ implies a partial order  $\preccurlyeq$ on  $\mathit{tx}^{c}$ and ${\mathit{ty}} $.  This can be expressed as $\forall \mathit{tx}, \mathit{ty} \in T,  W_{\mathit{tx} }(x, v),  R_{\mathit{ty}}(x){v^{'''}} \in  \mathcal{S}_t: 
 W_{\mathit{tx} }(x, v) \operatorname{\it F}_\mathit{st} R_{\mathit{ty}}(x){v^{'}}  \quad \wedge \quad v = v^{'''} \quad  \Rightarrow  \quad \mathit{tx}^{c} \preccurlyeq_{\mathit{wr}} \mathit{ty}$. Combining the above expressions, we have: 
...
\begin{split}
 ...
 W_{\mathit{tx} }(x, v) \operatorname{\it F}_\mathit{st} R_{\mathit{tx}}(x){v^{''}} \\ \quad \wedge \quad v = v^{''}  
 \wedge \quad W_{\mathit{ty} }(x, v^{'})  \operatorname{\it F}_\mathit{st} R_{\mathit{ty}}(x){v^{'''}} \quad \wedge   v^{'} = v^{'''}   \Rightarrow  \quad \mathit{tx}^{c} \prec_{\mathit{ww}} \mathit{ty}  \quad  
 \wedge \quad W_{\mathit{tx} }(x, v) \operatorname{\it F}_\mathit{st} R_{\mathit{ty}}(x){v^{'}}  \quad \wedge \\ \quad v = v^{'''}  \quad  \Rightarrow  \quad \mathit{tx}^{c} \prec_{\mathit{wr}} \mathit{ty} \quad \wedge \quad  \mathit{tx} \not \prec_{\mathit{wr}} \mathit{tx} \quad \wedge \quad \mathit{ty} \not \prec_{\mathit{wr}} \mathit{ty} \quad \wedge \quad  v = v^{''''}.
...
%\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx} 
 \quad \wedge 
\forall x, \; \mathit{tx}, \mathit{ty} \in T: \mathit{tx} \prec_{\mathit{rw}} \mathit{ty}. %\quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T: 
