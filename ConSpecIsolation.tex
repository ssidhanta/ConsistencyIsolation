...
The remainder of this paper is organized as follows. Section \ref{sec:related} discusses relevant related work. Section  \ref{sec:definitions} presents the assumed system model, and basic definitions and terminology used in the paper.  Section \ref{sec:syntax} presents a general format of specifications based on ConSpec. Section \ref{sec:list} lists the specifications of various  consistency models, expressed using ConSpec.  Section \ref{sec:cap} presents a reformulation of the CAP theorem, and formally proves the same.  Section \ref{sec:capanalyze} analyzes the previously mentioned consistency models in terms of the restated CAP theorem.
 Finally, Section \ref{sec:impl} presents the design of a prototype automated verification tool that can validate a session trace with respect to a ConSpec specification, and we conclude in Section 9 with a series of future work directions.  %In Sections \ref{sec:derive}, we prove equivalence of each of the listed ConSpec specifications with their original definitions. %; because of lack of space, we include some of the proofs in the Appendix.
...
% %$\left( {\mathcal{O}_T}, \preccurlyeq \right)$ over a set ${\mathcal{O}_T}$ comprising operations performed from all transactions ${\mathit{tx}} \in T$ 
...
Given a global session trace $\mathcal{S}_t$ comprising %a sequence of all 
 operations from all transactions in the set $T$ of all transactions executing on a system, % session trace $\mathcal{S}_t$,
% representing a global execution of all concurrent clients on a given storage
%system,
we say  that   $\mathcal{S}_t$ satisfies an isolation level $\mathcal{I}$
 if there exists a partial order  $\left( T, \preccurlyeq \right)$ over $T$ %of all transactions executing in the system 
%$\left( {\mathcal{O}_{T}}, \preccurlyeq \right)$ over the set ${\mathcal{O}_{T}}$ comprising operations  present in all transactions in $T$, i.e.,  ${\mathcal{O}_{T}} = \bigcup_{{\mathit{tx}} \in T} \{ o \mid o \in {\mathit{tx}} \},$
 such that:  %${\mathit{tx}} \preccurlyeq {\mathit{ty}}$  implies the following: 
 1) $\left( T, \preccurlyeq \right)$ obeys $E^s$, which is an LTL expression restricting $\left( T, \preccurlyeq \right)$, and 2) for all transactions $\mathit{tx}, \mathit{ty} \in T$, which read or write on some common objects, %. at least one of the following conditions are satisfied: 
 %2) %for each operation in a transaction ${\mathit{tx}}$ comprised in $\mathcal{S}_t$,  the value of each object is equal to the value observed by the sequential  specification of a linear extension of all transactions preceding ${\mathit{tx}}$ in $\preccurlyeq$, i.e., it is the value written by
if $ {\mathit{ty}}$ reads the result of at least one write operation invoked by $ {\mathit{tx}}$, this  implies $\mathit{tx} \preccurlyeq \mathit{ty}$. %which we refer to as $\mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty}$,  where the suffix ${\mathit{wr}}$ restricts the partial order $\preccurlyeq$ among write operations in $\mathit{tx}$ and read operations in $\mathit{ty}$, %which 
  %precedes the transaction $ {\mathit{tx}}$ in $\preccurlyeq$,  
  %i.e.,  $\exists W(x,v)_{\mathit{tx}}, R(x){v^{'}}_{\mathit{ty}} \in  \mathcal{S}_t: %W(x,v)_{\mathit{tx}} \operatorname{\it F}_\mathit{st} R(x){v^{'}}_{\mathit{ty}} \wedge 
  % v = v^{'}  \rightarrow \mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty}$,  
%   3)  if $\mathit{tx}$ and  $ {\mathit{ty}}$ both write to $x$, and any transaction  $\mathit{tz}$, which satisfies the condition $\mathit{ty} \preccurlyeq \mathit{tz}$ as per Condition 2 given above, reads the result of the write performed by $\mathit{ty}$ on $x$ instead of a write by  $\mathit{tx}$, this  implies $\mathit{tx} \preccurlyeq \mathit{ty}$, %which we refer to as $\mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty}$, where the suffix ${\mathit{ww}}$ restricts the partial order $\preccurlyeq$ among write operations in $\mathit{tx}$ and $\mathit{ty}$, i.e., $ \exists {\mathit{tz}} \in T, W(x,v)_{\mathit{tx}},W(x,v^{'})_{\mathit{ty}}, R(x){v^{''}}_{\mathit{tz}} \in  \mathcal{S}_t: %W(x,v)_{\mathit{tx}} \operatorname{\it F}_\mathit{st} R(x){v^{'}}_{\mathit{ty}} \wedge 
%    %\mathit{ty} \preccurlyeq_{\mathit{wr}} \mathit{tz} \wedge v^{'} = v^{''}   \rightarrow \mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty}$, 
%    and 4) if $\mathit{tx}$ reads from $x$ and  $ {\mathit{ty}}$ writes to $x$, and  any transaction  $\mathit{tz}$, which satisfies the condition $\mathit{ty} \preccurlyeq \mathit{tz}$ as per Condition 2, reads the result of the write performed by $\mathit{ty}$ on $x$ instead of the result  of $\mathit{tx}$, this implies $\mathit{tx} \preccurlyeq \mathit{ty}$.  %which we refer to as $\mathit{tx} \preccurlyeq_{\mathit{rw}} \mathit{ty}$, where the suffix ${\mathit{rw}}$ restricts the partial order $\preccurlyeq$ among read operations in $\mathit{tx}$ and write operations in $\mathit{ty}$, i.e., $\exists {\mathit{tz}} \in T, R(x,v)_{\mathit{tx}}, W(x,v^{'})_{\mathit{ty}}, R(x){v^{''}}_{\mathit{tz}} \in  \mathcal{S}_t: %W(x,v)_{\mathit{tx}} \operatorname{\it F}_\mathit{st} R(x){v^{'}}_{\mathit{ty}} \wedge 
    %\mathit{ty} \preccurlyeq_{\mathit{wr}} \mathit{tz} \wedge v^{'} = v^{''}  \rightarrow \mathit{tx} \preccurlyeq_{\mathit{rw}} \mathit{ty}$. %for each operation in a transaction ${\mathit{tx}}$ comprised in $\mathcal{S}_t$,  the value of each object is equal to the val
 %an operation $o_{\mathit{ty}}$  from ${\mathit{ty}} $ modifies (writes) /returns (reads) the value of $x$ resulting from an operation $o_{\mathit{tx}}$ from ${\mathit{tx}}$, 
 %3)  %no operation from ${\mathit{tx}} $ can modify (write) /return (read) the result of  $x$ obtained from any operation from ${\mathit{ty}}$, i.e., 
 %$\not\exists \; x, \;  w(x,v^{''})_{\mathit{tx}} \vee r(x){v^{''}}_{\mathit{tx}} \in {\mathit{tx}},  \;  \left( w(x,v^{'''})_{\mathit{ty}} \vee r(x){v^{'''}}_{\mathit{ty}} \right) F r(x){v^{''''}}_{\mathit{ty}} \in {\mathit{ty}}: \;  v^{''''} = v^{'''}$,   %, in $\preccurlyeq$, invocation and response of all operations in ${\mathit{tx}}$ precede invocation and response of all operations in ${\mathit{ty}} $ on a common object   
  %and  %1) for every operation $\mathit{o}$ in $\mathcal{O}_T$, its output is equal to the one obtain by executing the sequential specification of a linear extension of the operations preceding $\mathit{o}$ in $\preccurlyeq$,
% the same irrespective of the
 %%among operations preceding it in $\preccurlyeq$, %$\exists {\mathit{Op}^i}^{'}, {\mathit{Op}^j}^{'} \in \mathcal{S}_t:
 %{\mathit{Op}^i}^{'} \preccurlyeq {\mathit{Op}^j}^{'}\; \Rightarrow \;{\mathit{Op}^i}^{'} R {\mathit{Op}^j}^{'}$, %\; \vee \; v_i \sim v_j$,{\mathcal{O}_{St}}, \preccurlyeq
% and 2) 
...
%Condition 1 ensures  that result of no operation in  ${\mathit{ty}} $ can be observed before results of all operations in ${\mathit{tx}}$ are observed. 
 ...
 where $T$ is the set of all transactions executing in the system.   %$\mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty}$ restricts the partial order $\preccurlyeq$ among write operations in $\mathit{tx}$ and read operations in $\mathit{ty}$. By inspection of Definition \ref{def:form0} , $\mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty}  \;  \equiv \; \mathit{tx} \preccurlyeq \mathit{ty} $.  
  %Thus, %replacing $\left( {\mathcal{O}_{St}}, \preccurlyeq \right)$ with  $\left( T, \preccurlyeq \right)$ in Definition \ref{def:form0}, 
The partial order relation $\mathit{tx} \preccurlyeq \mathit{ty}$ in Condition 2 %as $\mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty}$,  where the suffix ${\mathit{wr}}$ restricts the partial order $\preccurlyeq$ among write operations in $\mathit{tx}$ and read operations in $\mathit{ty}$, %which 
  %precedes the transaction $ {\mathit{tx}}$ in $\preccurlyeq$,  
  implies  $\exists W(x,v)_{\mathit{tx}}, R(x){v^{'}}_{\mathit{ty}} \in  \mathcal{O}_{St}: %W(x,v)_{\mathit{tx}} \operatorname{\it F}_\mathit{st} R(x){v^{'}}_{\mathit{ty}} \wedge 
   v = v^{'}  \rightarrow \mathit{tx} \preccurlyeq \mathit{ty}$. 
   %We refer to the partial order relation $\mathit{tx} \preccurlyeq \mathit{ty}$ in Condition 3 as $\mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty}$, where the suffix ${\mathit{ww}}$ restricts the partial order $\preccurlyeq$ among write operations in $\mathit{tx}$ and $\mathit{ty}$, i.e., $ \exists {\mathit{tz}} \in T, W(x,v)_{\mathit{tx}},W(x,v^{'})_{\mathit{ty}}, R(x){v^{''}}_{\mathit{tz}} \in  \mathcal{S}_t: %W(x,v)_{\mathit{tx}} \operatorname{\it F}_\mathit{st} R(x){v^{'}}_{\mathit{ty}} \wedge 
    %\mathit{ty} \preccurlyeq_{\mathit{wr}} \mathit{tz} \wedge v^{'} = v^{''}   \rightarrow \mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty}$. We refer to the partial order relation $\mathit{tx} \preccurlyeq \mathit{ty}$ in Condition 4 as $\mathit{tx} \preccurlyeq_{\mathit{rw}} \mathit{ty}$, where the suffix ${\mathit{rw}}$ restricts the partial order $\preccurlyeq$ among read operations in $\mathit{tx}$ and write operations in $\mathit{ty}$, i.e., $\exists {\mathit{tz}} \in T, R(x,v)_{\mathit{tx}}, W(x,v^{'})_{\mathit{ty}}, R(x){v^{''}}_{\mathit{tz}} \in  \mathcal{S}_t: %W(x,v)_{\mathit{tx}} \operatorname{\it F}_\mathit{st} R(x){v^{'}}_{\mathit{ty}} \wedge 
 %   \mathit{ty} \preccurlyeq_{\mathit{wr}} \mathit{tz} \wedge v^{'} = v^{''}  \rightarrow \mathit{tx} \preccurlyeq_{\mathit{rw}} \mathit{ty}$.  
  ...
  %which 
  %precedes the transaction $ {\mathit{tx}}$ in $\preccurlyeq$,  
  	$\exists W(x,v)_{\mathit{tx}}, R(x){v^{'}}_{\mathit{ty}} \in  \mathcal{O}_{St}: %W(x,v)_{\mathit{tx}} \operatorname{\it F}_\mathit{st} R(x){v^{'}}_{\mathit{ty}} \wedge 
   v = v^{'}  \rightarrow \mathit{tx} \prec \mathit{ty}$ or $\exists W(x,v)_{\mathit{ty}}, R(x){v^{'}}_{\mathit{tx}} \in  \mathcal{O}_{St}: %W(x,v)_{\mathit{tx}} \operatorname{\it F}_\mathit{st} R(x){v^{'}}_{\mathit{ty}} \wedge 
   v = v^{'}  \rightarrow \mathit{ty} \prec \mathit{tx}$.  %The above total order proscribes wr dependency cycles among transactions.  
...
\begin{split}
% E^{s} = \forall y, \;  \mathit{tx}, \mathit{ty} \in T. 
% \;  \mathit{tx} \preccurlyeq_y {\mathit{ty}} \quad \Rightarrow  \quad \not \exists x, \; \mathit{tz} \in T: \;
%   \mathit{tz} \preccurlyeq_x {\mathit{ty}} \quad \wedge \quad \mathit{tx} \preccurlyeq {\mathit{tz}}.
 E^{s} = \forall \mathit{tx}, \mathit{ty} \in T. \;  \exists W(x,v)_{\mathit{tx}}, R(x){v^{'}}_{\mathit{ty}} \in  \mathcal{O}_{St}: 
  \mathit{tx} \preccurlyeq {\mathit{ty}} \\
   \Rightarrow  \quad \not \exists \mathit{tz} \in T, W(x,v^{''})_{\mathit{tz}}, R(x){v^{'''}}_{\mathit{tz}} \in  \mathcal{O}_{St}: 
   \mathit{tz} \preccurlyeq {\mathit{ty}} \quad \wedge \quad \mathit{tx} \preccurlyeq {\mathit{tz}} .
% \; \forall x, {\mathit{tx}}, {\mathit{ty}} \in T: \;
%\mathit{tx} \preccurlyeq_x {\mathit{ty}} \quad \Rightarrow  \quad \not \exists \mathit{tz} \in T: \; \mathit{tz} \preccurlyeq {\mathit{ty}}. %\quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T:  
%\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx}
\end{split}
  \end{align}
...
\begin{split}
 E^{s} =  \forall \mathit{tx}, \mathit{ty} \in T. \;  \exists W(x,v)_{\mathit{tx}}, R(x){v^{'}}_{\mathit{ty}} \in  \mathcal{O}_{St}: 
  \mathit{tx}^{c} \preccurlyeq {\mathit{ty}} \\
   \wedge  \quad \left( \not \exists \mathit{tz} \in T, W(x,v^{''})_{\mathit{tz}}, R(x){v^{'''}}_{\mathit{tz}} \in  \mathcal{O}_{St}: 
   \mathit{tz} \preccurlyeq {\mathit{ty}} \quad \wedge \quad \mathit{tx} \preccurlyeq {\mathit{tz}} \right). %\quad \not \exists \mathit{tz} \in T, W(x,v^{''})_{\mathit{tz}} \in  \mathcal{S}_t: \\
   %\mathit{tz} \preccurlyeq_x {\mathit{ty}} \quad \wedge \quad \mathit{tx} \preccurlyeq {\mathit{tz}}. 
 %E^{s} =  \; \forall R_{\mathit{ty}}(x){v} \in \mathcal{S}_t. \; \exists \mathit{tx}^{c} \in T: 
%\mathit{tx}^{c} \preccurlyeq {ty} \quad \wedge   \quad \not \exists \mathit{tz} \in T: \; \mathit{tz} \preccurlyeq {\mathit{ty}}. %\quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T:  
%\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx}
\end{split}
  \end{align}
...
\begin{split}
 E^{s} =  \forall \mathit{tx}, \mathit{ty} \in T, R(x){v^{'}}_{\mathit{ty}}, R_{\mathit{ty}}(x){v^{''}} \in \mathcal{O}_{St}. \; \exists W(x,v)_{\mathit{tx}} \in \mathcal{O}_{St}: \\ %\; \exists \mathit{tx} \in T:  \\
\left( \mathit{tx}^{c} \preccurlyeq \mathit{ty} \right) \quad \wedge \quad %\mathit{ty} \not \preccurlyeq \mathit{ty} \quad \wedge \quad 
  \left( v^{'} = v^{''} \quad \wedge \quad \mathit{tx} \not \preccurlyeq \mathit{tx} \quad \wedge \quad \mathit{ty} \not \preccurlyeq \mathit{ty}  \right) \quad \wedge \\
  \left( \not \exists \mathit{tz} \in T, W(x,v^{'''})_{\mathit{tz}}, R(x){v^{''''}}_{\mathit{tz}} \in \mathcal{O}_{St}: 
   \mathit{tz} \preccurlyeq {\mathit{ty}} \quad \wedge \quad \mathit{tx} \preccurlyeq {\mathit{tz}} \right). %\quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T:  
%\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx}
\end{split}
  \end{align}
%  \par Cursor stability isolation level is specified as follows. 
%   \begin{align}\label{eqn:cursor}
%\begin{split}
% E^{s} =  \; \forall W_{\mathit{tx} }(x, v^{''}), R_{\mathit{ty}}(x){v}, R_{\mathit{ty}}(x){v^{'}} \in \mathcal{S}_t: \; % . \; \exists \mathit{tx} \in T: 
%\mathit{tx}^{c} \prec \mathit{ty} %\quad \wedge \quad \mathit{ty} \not \preccurlyeq \mathit{ty} \quad \wedge \quad v = v^{'} \\ 
%  \quad \wedge   \quad \not \exists \mathit{tz} \in T: \; \mathit{tz} \preccurlyeq {\mathit{ty}}. %\quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T:  
%%\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx}
%\end{split}
%  \end{align}
  \par We use the definition of snapshot isolation level found in \cite{kemme-thesis}. Additionally, Adya et al. \cite{Adya:1999:WCG:930194} specifies the following anomalies that snapshot isolation level proscribes. 
  SI-a: Concurrent transactions do not observe the effects of one another, and do not modify the same objects. 
  SI-b: A transaction observes complete effects of transactions that committed before it started.  SI-a can be interpreted as follows.   If a pair of transactions can observe results of each other, they are not concurrent, i.e., $\preccurlyeq$ among a pair of transactions in Definition \ref{def:form0} is a total order $\prec$.  According to SI-b the above total order must be among committed version of $\mathit{tx}$ and $\mathit{ty}$, i.e., $\mathit{tx}^{c} \prec \mathit{ty}$. The above condition reduces to: $ \exists \mathit{tx}^{c}, \mathit{ty} \in T, W(x,v)_{\mathit{tx}}, R(x){v^{'}}_{\mathit{ty}}, R_{\mathit{ty}}(x){v^{''}} \in \mathcal{O}_{St}:  \; \exists \mathit{tx} \in T:  
v^{'} = v^{''} = v  \quad \Rightarrow \quad %\mathit{ty} \not \preccurlyeq \mathit{ty} \quad \wedge \quad 
 \mathit{tx}^{c} \prec \mathit{ty}
  %\forall \mathit{tx}, \mathit{ty} \in T,  W_{\mathit{tx} }(x, v),  %W_{\mathit{ty} }(x, v^{'}), %R_{\mathit{tx}}(x){v^{''}},    
  %R_{\mathit{ty}}(x){v^{'}}, R_{\mathit{ty}}(x){v^{''}} \in  \mathcal{S}_t:
 %W_{\mathit{tx} }(x, v) \operatorname{\it F}_\mathit{st} R_{\mathit{tx}}(x){v^{''}} \quad \wedge \quad 
%\left( v = v^{'}%W_{\mathit{ty} }(x, v^{'})  \operatorname{\it F}_\mathit{st} R_{\mathit{tx}}(x){v^{'''}} \quad 
  %\quad  \Rightarrow  \quad \mathit{tx}^{c} \prec \mathit{ty} \right)
  $. Snapshot isolation also includes the repeatable read read. condition, i.e., $\forall \mathit{tx}, \mathit{ty} \in T,  W_{\mathit{tx} }(x, v),  %W_{\mathit{ty} }(x, v^{'}), %R_{\mathit{tx}}(x){v^{''}},    
  R_{\mathit{ty}}(x){v^{'}}, R_{\mathit{ty}}(x){v^{''}} \in  \mathcal{O}_{St}: \left( \mathit{tx} \not \preccurlyeq \mathit{tx} \quad \wedge \quad \mathit{ty} \not \preccurlyeq \mathit{ty} \quad \wedge \quad v^{'} = v^{''} \right)$.  %SI-b can be interpreted as: if a transaction  ${\mathit{ty}} $ observes values from another transaction  ${\mathit{tx}} $ implies a partial order  $\preccurlyeq$ on  $\mathit{tx}^{c}$ and ${\mathit{ty}} $.  This can be expressed as $\forall \mathit{tx}, \mathit{ty} \in T,  W_{\mathit{tx} }(x, v),  R_{\mathit{ty}}(x){v^{'''}} \in  \mathcal{S}_t: 
 %W_{\mathit{tx} }(x, v) \operatorname{\it F}_\mathit{st} R_{\mathit{ty}}(x){v^{'}}  \quad \wedge \quad 
% v = v^{'''} \quad  \Rightarrow  \quad \mathit{tx}^{c} \preccurlyeq \mathit{ty}$. 
 Combining the above expressions, we have: 
   \begin{align}\label{eqn:snap}
\begin{split}
 E^{s} =  \; \forall \mathit{tx}, \mathit{ty} \in T, R(x){v^{'}}_{\mathit{ty}}, R_{\mathit{ty}}(x){v^{''}} \in \mathcal{O}_{St}. \; \exists W(x,v)_{\mathit{tx}} \in \mathcal{O}_{St}: \; %\; \exists \mathit{tx} \in T:  \\
\mathit{tx}^{c} \preccurlyeq \mathit{ty} \\ \wedge \quad  \left( v^{'} = v^{''} = v  \quad \Rightarrow \quad %\mathit{ty} \not \preccurlyeq \mathit{ty} \quad \wedge \quad 
 \mathit{tx}^{c} \prec \mathit{ty} \right) \quad
 %E^{s} =  \; \forall \mathit{tx}, \mathit{ty} \in T,  W_{\mathit{tx} }(x, v),  %W_{\mathit{ty} }(x, v^{'}), %R_{\mathit{tx}}(x){v^{''}},    
  %R_{\mathit{ty}}(x){v^{'}}, R_{\mathit{ty}}(x){v^{''}} \in  \mathcal{S}_t:  \;
 %W_{\mathit{tx} }(x, v) \operatorname{\it F}_\mathit{st} R_{\mathit{tx}}(x){v^{''}} \\ \quad \wedge \quad 
%\left( v = v^{'}   %v = v^{''}  \quad
 %\wedge \quad %W_{\mathit{ty} }(x, v^{'})  \operatorname{\it F}_\mathit{st} R_{\mathit{ty}}(x){v^{'''}} \quad \wedge  
  %v^{'} = v^{'''}   
  %\quad  \Rightarrow  \quad %\mathit{tx}^{c} \prec_{\mathit{ww}} \mathit{ty} % \right)  \\ 
  %\quad  
 %\wedge %\quad W_{\mathit{tx} }(x, v) \operatorname{\it F}_\mathit{st} R_{\mathit{ty}}(x){v^{'}}  \quad \wedge 
  %\quad %\left( v = v^{'''}  \quad  \Rightarrow  \quad \mathit{tx}^{c} \prec_{\mathit{wr}} \mathit{ty} \right)
  %\mathit{tx}^{c} \prec \mathit{ty} \right) \quad 
   \wedge \quad 
   \left( \mathit{tx} \not \preccurlyeq \mathit{tx} \quad \wedge \quad \mathit{ty} \not \preccurlyeq \mathit{ty} \quad \wedge \quad v^{'} = v^{''} \right) \quad \wedge \\
   % \quad  \left( \mathit{tx} \not \preccurlyeq \mathit{tx} \quad \wedge \quad \mathit{ty} \not \preccurlyeq \mathit{ty} \quad 
   % \wedge \quad v^{'} = v^{''} \right) 
   \left( \not \exists \mathit{tz} \in T, W(x,v^{'''})_{\mathit{tz}}, R(x){v^{''''}}_{\mathit{tz}} \in  \mathcal{O}_{St} 
   \mathit{tz} \preccurlyeq {\mathit{ty}} \quad \wedge \quad \mathit{tx} \preccurlyeq {\mathit{tz}} \right).
   %\quad \wedge   \left( \quad \not \exists \mathit{tz} \in T: \; \mathit{tz} \preccurlyeq {\mathit{ty}} \right).
% E^{s} =  \; \forall x, \; \mathit{tx}, \mathit{ty} \in T, R_{\mathit{tx}}(x){v^{''}}, W_{\mathit{tx} }(x, v), R_{\mathit{tx}}(x){v^{'''}} \in  \mathit{tx}, R_{\mathit{ty}}(x){v^{'}} \in \mathit{ty}: 
%\mathit{tx}  \operatorname{\it F}_\mathit{st} \mathit{ty}  \quad \vdash \\
% W_{\mathit{tx} }(x, v) \;  \operatorname{\it F}_\mathit{st} \;  c_\mathit{tx} \;  \operatorname{\it F}_\mathit{st} \; R_{\mathit{ty}}(x){v^{'}} \quad \Rightarrow \quad v = v^{'} \quad \wedge \\ 
%R_{\mathit{tx}}(x){v^{''}} \;  \operatorname{\it F}_\mathit{st} \; W_{\mathit{tx} }(x, v) \;  \operatorname{\it F}_\mathit{st} \;  R_{\mathit{tx}}(x){v^{'''}}   \quad \Rightarrow  \quad v^{'''} = v^{''}  
\end{split}
  \end{align}
...
\begin{split}
 E^{s} =  \; \forall {\mathit{tx}}, \mathit{ty} \in T: \;
\mathit{tx}^{c} \preccurlyeq_S \mathit{ty}^{c}. %\quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T:  
%\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx}
\end{split}
  \end{align}%In ConSpec, the notation $\mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty}$ restricts the partial order $\preccurlyeq$ among write operations in $\mathit{tx}$ and $\mathit{ty}$, i.e., $\exists \; W(x,v)_{\mathit{tx}}, W(x,v^{'})_{\mathit{ty}}, R(x){v^{''}}_{\mathit{ty}} \in  \mathcal{S}_t: W(x,v)_{\mathit{tx}} \operatorname{\it F}_\mathit{st} W(x,v^{'})_{\mathit{ty}} \wedge W(x,v^{'})_{\mathit{ty}} \operatorname{\it F}_\mathit{st} R(x,v^{''})_{\mathit{ty}} \wedge  v^{''} = v^{'}  \Leftrightarrow \mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty}$. 
...
%%in the \textbf{form-2} format, given in Equation \ref{form-2},
% as follow.
...
%%E^{s} =  & \; \forall x, y, \mathcal{S}_t, \; {\mathit{W}}_\mathit{tx}(x), {\mathit{W}}'_ \mathit{ty}(x), {\mathit{W}}_\mathit{tx}(x)'', {\mathit{W}}'''_ \mathit{ty}(y) \in \mathcal{S}_t : \\
%%& {\mathit{W}}_\mathit{tx}(x) \preccurlyeq {\mathit{W}}'_ \mathit{ty}(y) \quad \Rightarrow \quad
%%{\mathit{W}}_\mathit{tx}(x)'' \preccurlyeq {\mathit{W}}'''_ \mathit{ty}(y)
%%E^{s} =  & \; \forall x, \; \mathit{tx}, \mathit{ty} \in T \; {\mathit{W}}_\mathit{tx}(x), {\mathit{W}}'_ \mathit{ty}(x) \in \mathcal{S}_t : \\
%%& {\mathit{W}}_\mathit{tx}(x) \preccurlyeq {\mathit{W}}'_ \mathit{ty}(x) \quad \vee \quad
%%{\mathit{W}}'_\mathit{ty}(x) \preccurlyeq {\mathit{W}}_ \mathit{tx}(x)
%E^{s} =  \; \forall x, \; \mathit{tx}, \mathit{ty} \in T: 
%\mathit{tx} \prec_{\mathit{ww}} \mathit{ty}. %\quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T:  
%%\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx}
%\end{split}
%  \end{align}
...
%%E^{s} =   \; \forall x, y, \mathit{st} \in \mathcal{S}_t, {\mathit{R}}_\mathit{ty}(x) \in \mathit{st}, \; {\mathit{W}}_\mathit{tx}(x), {\mathit{W}}_\mathit{ty}(y)', {\mathit{W}}_\mathit{tx}(x)'', {\mathit{W}}_ \mathit{ty}(y)''', {\mathit{R}}_\mathit{ty}(x)' \in \mathcal{S}_t : \\
%%{\mathit{W}}_\mathit{tx}(x) \operatorname{\it F}_\mathit{st} {\mathit{W}}_\mathit{ty}(x)'  \operatorname{\it F}_\mathit{st} a_\mathit{ty}\operatorname{\it F}_\mathit{st} {\mathit{R}}_\mathit{ty}(x)  \quad \Rightarrow \quad
%%\not\exists {\mathit{W}}_\mathit{ty}(x)'  \preccurlyeq {\mathit{R}}_\mathit{ty}(x) \wedge \\
%%{\mathit{W}}_\mathit{tx}(x) \operatorname{\it F}_\mathit{st} {\mathit{W}}_ \mathit{ty}(y)' \quad \Rightarrow \quad
%%{\mathit{W}}_\mathit{tx}(x)'' \preccurlyeq {\mathit{W}}'''_ \mathit{ty}(y) \wedge \\
%%{\mathit{W}}_\mathit{tx}(x) \operatorname{\it F}_\mathit{st} {\mathit{R}}_ \mathit{ty}(y)' \quad \Rightarrow \quad
%%{\mathit{W}}_\mathit{tx}(y)'' \preccurlyeq {\mathit{R}}_ \mathit{ty}(x)',
%%E^{s} =   \; \forall x, \; \mathit{tx}, \mathit{ty} \in T, \; {\mathit{W}}_\mathit{tx}(x), {\mathit{W}}'_ \mathit{ty}(x), {\mathit{R}}_\mathit{ty}(x) \in \mathcal{S}_t : \\
%% \not\exists {\mathit{W}}_\mathit{tx}(x) \preccurlyeq a_\mathit{tx} \preccurlyeq {\mathit{R}}_\mathit{ty}(x) \quad \wedge \quad  \not\exists c_\mathit{tx} \preccurlyeq  {\mathit{W}}_\mathit{tx}(x) \preccurlyeq {\mathit{R}}_\mathit{ty}(x)  \wedge \\
%%{\mathit{W}}_\mathit{tx}(x) \preccurlyeq {\mathit{W}}'_ \mathit{ty}(x) \quad \vee \quad
%%{\mathit{W}}'_\mathit{ty}(x) \preccurlyeq {\mathit{W}}_ \mathit{tx}(x) \wedge \\
%%{\mathit{W}}_\mathit{tx}(x) \preccurlyeq {\mathit{R}}_ \mathit{ty}(x) \quad \vee \quad
%%{\mathit{R}}_\mathit{ty}(x) \preccurlyeq {\mathit{W}}_ \mathit{tx}(x),
%E^{s} =    \; \forall x, \; \mathit{tx}, \mathit{ty} %\mathit{tx}^{c}, \mathit{ty}^{c} 
% \in T: 
%\mathit{tx}^{c} \prec_{\mathit{wr}} \mathit{ty}^{c} %\quad \vee \quad \forall x, \; \mathit{tx}^{c}, \mathit{ty}^{c} \in T: \mathit{ty}^{c} \preccurlyeq_{\mathit{wr}} \mathit{tx}^{c} 
% \quad \wedge \quad 
%%\; \forall x, \; \mathit{tx}, \mathit{ty} \in T: 
%\mathit{tx} \prec_{\mathit{ww}} \mathit{ty}. % \quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T:  
%%\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx}. % \quad \wedge \\
%%\forall x, \; \mathit{tx}, \mathit{ty} \in T: \mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty} \quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T: 
%%\mathit{ty} \preccurlyeq_{\mathit{wr}} \mathit{tx}, %\quad \wedge \quad
%%\left( \mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty} \quad \vee \quad
%%\mathit{ty} \preccurlyeq_{\mathit{wr}} \mathit{tx} \right),
%\end{split}
%  \end{align} % $a_\mathit{tx}$ and $a_\mathit{ty}$ are the abort statements for the transactions $\mathit{tx}$ and $\mathit{ty}$, respectively. F is the traditional LTL next operator.
%
...
%%E^{s} =   \; \forall x, y, \mathit{st} \in \mathcal{S}_t, {\mathit{R}}_\mathit{ty}(y) \in \mathit{st}, \; {\mathit{W}}_\mathit{tx}(x), {\mathit{W}}_\mathit{ty}(y)', {\mathit{W}}_\mathit{tx}(x)'', {\mathit{W}}_ \mathit{ty}(y)''', \\ {\mathit{R}}_\mathit{ty}(x)', {\mathit{R}}_\mathit{tx}(x)'', {\mathit{R}}'''_ \mathit{ty}(y) \in \mathcal{S}_t :
%%{\mathit{W}}_\mathit{tx}(x) \operatorname{\it F}_\mathit{st} {\mathit{W}}_\mathit{ty}(x)'  \operatorname{\it F}_\mathit{st} a_\mathit{ty}\operatorname{\it F}_\mathit{st} {\mathit{R}}_\mathit{ty}(x) \\  \quad \Rightarrow \quad
%%\not\exists {\mathit{W}}_\mathit{ty}(x)'  \preccurlyeq {\mathit{R}}_\mathit{ty}(x) \wedge
%%{\mathit{W}}_\mathit{tx}(x) \operatorname{\it F}_\mathit{st} {\mathit{W}}_ \mathit{ty}(y)' \quad \Rightarrow \quad
%%{\mathit{W}}_\mathit{tx}(x)'' \preccurlyeq {\mathit{W}}'''_ \mathit{ty}(y) \wedge \\
%%{\mathit{W}}_\mathit{tx}(x) \operatorname{\it F}_\mathit{st} {\mathit{R}}_ \mathit{ty}(y) \quad \Rightarrow \quad
%%{\mathit{W}}_\mathit{tx}(y)'' \preccurlyeq {\mathit{R}}_ \mathit{ty}(x)' \wedge
%%{\mathit{R}}_ \mathit{ty}(x)' \preccurlyeq  {\mathit{R}}_ \mathit{ty}(y)  \quad \Rightarrow \quad
%%{\mathit{R}}_\mathit{tx}(x)'' \preccurlyeq {\mathit{R}}'''_ \mathit{ty}(y),
%% \; \forall x, \; \mathit{tx}, \mathit{ty} \in T, \; {\mathit{W}}_\mathit{tx}(x), {\mathit{W}}'_ \mathit{ty}(x), {\mathit{R}}_\mathit{ty}(x), {\mathit{R}}'_\mathit{tx}(x) \in \mathcal{S}_t : \\
%% \not\exists {\mathit{W}}_\mathit{tx}(x) \preccurlyeq a_\mathit{tx} \preccurlyeq {\mathit{R}}_\mathit{ty}(x) \quad \wedge  \quad \not\exists c_\mathit{tx} \preccurlyeq  {\mathit{W}}_\mathit{tx}(x) \preccurlyeq {\mathit{R}}_\mathit{ty}(x) \quad \wedge \\
%%{\mathit{W}}_\mathit{tx}(x) \preccurlyeq {\mathit{W}}'_ \mathit{ty}(x) \quad \vee \quad
%%{\mathit{W}}'_\mathit{ty}(x) \preccurlyeq {\mathit{W}}_ \mathit{tx}(x) \quad \wedge \\
%%{\mathit{W}}_\mathit{tx}(x) \preccurlyeq {\mathit{R}}_ \mathit{ty}(x) \quad \vee \quad
%%{\mathit{R}}_\mathit{ty}(x) \preccurlyeq {\mathit{W}}_ \mathit{tx}(x) \quad \wedge \\
%%{\mathit{R}}'_\mathit{tx}(x) \preccurlyeq {\mathit{W}}'_ \mathit{ty}(x) \quad \vee \quad
%%{\mathit{W}}'_\mathit{ty}(x) \preccurlyeq {\mathit{R}}'_ \mathit{tx}(x),
%E^{s} =  \; \forall x, \; \mathit{tx}, \mathit{ty} %\mathit{tx}^{c}, \mathit{ty}^{c}
% \in T: 
%\mathit{tx}^{c} \prec_{\mathit{wr}} \mathit{ty}^{c} %\quad \vee \quad \forall x, \; \mathit{tx}^{c}, \mathit{ty}^{c} \in T: \mathit{ty}^{c} \preccurlyeq_{\mathit{wr}} \mathit{tx}^{c} 
% \quad \wedge \quad 
...
%%E^{s} =   \; \forall x, y, \mathit{st} \in \mathcal{S}_t, {\mathit{R}}_\mathit{ty}(y) \in \mathit{st}, \; {\mathit{W}}_\mathit{tx}(x), {\mathit{W}}_\mathit{ty}(y)', {\mathit{W}}_\mathit{tx}(x)'', {\mathit{W}}_ \mathit{ty}(y)''', \\ {\mathit{R}}_\mathit{ty}(x)', \in \mathcal{S}_t :
%%{\mathit{W}}_\mathit{tx}(x) \operatorname{\it F}_\mathit{st} {\mathit{W}}_\mathit{ty}(x)'  \operatorname{\it F}_\mathit{st} a_\mathit{ty}\operatorname{\it F}_\mathit{st} {\mathit{R}}_\mathit{ty}(x) \\  \quad \Rightarrow \quad
%%\not\exists {\mathit{W}}_\mathit{ty}(x)'  \preccurlyeq {\mathit{R}}_\mathit{ty}(x) \wedge
%%{\mathit{W}}_\mathit{tx}(x) \operatorname{\it F}_\mathit{st} {\mathit{W}}_ \mathit{ty}(y)' \quad \Rightarrow \quad
%%{\mathit{W}}_\mathit{tx}(x)'' \preccurlyeq {\mathit{W}}'''_ \mathit{ty}(y) \wedge \\
%%{\mathit{W}}_\mathit{tx}(x) \operatorname{\it F}_\mathit{st} {\mathit{R}}_ \mathit{ty}(y) \quad \Rightarrow \quad
%%{\mathit{W}}_\mathit{tx}(y)'' \preccurlyeq {\mathit{R}}_ \mathit{ty}(x)' \wedge
%%{\mathit{R}}_ \mathit{ty}(x)' \preccurlyeq  {\mathit{W}}_ \mathit{ty}(y)'  \quad \Rightarrow \quad
%%{\mathit{R}}_\mathit{tx}(x)'' \preccurlyeq {\mathit{W}}'''_ \mathit{ty}(y),
%% \; \forall x, \; \mathit{tx}, \mathit{ty} \in T, \; {\mathit{W}}_\mathit{tx}(x), {\mathit{W}}'_ \mathit{ty}(x), {\mathit{R}}_\mathit{ty}(x), {\mathit{R}}'_\mathit{tx}(x) \in \mathcal{S}_t : \\
%% \not\exists {\mathit{W}}_\mathit{tx}(x) \preccurlyeq a_\mathit{tx} \preccurlyeq {\mathit{R}}_\mathit{ty}(x) \quad \wedge  \quad \not\exists c_\mathit{tx} \preccurlyeq  {\mathit{W}}_\mathit{tx}(x) \preccurlyeq {\mathit{R}}_\mathit{ty}(x) \quad \wedge \\
%%{\mathit{W}}_\mathit{tx}(x) \preccurlyeq {\mathit{W}}'_ \mathit{ty}(x) \quad \vee \quad
%%{\mathit{W}}'_\mathit{ty}(x) \preccurlyeq {\mathit{W}}_ \mathit{tx}(x) \quad \wedge \\
...
%%{\mathit{W}}'_\mathit{ty}(x) \preccurlyeq {\mathit{R}}'_ \mathit{tx}(x),
%E^{s} =   \; \forall x, \; \mathit{tx}, \mathit{ty} \in T: 
% \left( \mathit{tx}^{c}  \preccurlyeq_{\mathit{wr}} \mathit{ty} \quad \vee \quad
%\mathit{ty}^{c} \preccurlyeq_{\mathit{wr}} \mathit{tx}  \right) \quad \wedge \\
%\left( \mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty} \quad \vee \quad
%\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx} \right) \quad \wedge % \quad
%%\left( \mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty} \quad \vee \quad
%%\mathit{ty} \preccurlyeq_{\mathit{wr}} \mathit{tx} \right) \quad \wedge 
%\quad
%\left( \mathit{tx} \preccurlyeq_{\mathit{rw}} \mathit{ty} \quad \vee \quad
%\mathit{ty} \preccurlyeq_{\mathit{rw}} \mathit{tx} \right)
%\end{split}
%  \end{align}
...
%if and only if a valid partial orderings allowed by $\mathcal{I}$  for any global session invocation trace $\mathcal{S}_{it}$ restricts how operations comprised in a transaction observes/modifies the results of operations performed from another transaction.
...
\subsection{Deriving ConSpec Specifications From Original Definitions}\label{sec:derive}
 ...
 %ogic expressions.
 RYW, MR, WFR, MW, Causal and Sequential consistency have been defined precisely by Chockler et al. \cite{Chockler2000}, whereas PC have been defined in \cite{Ahamad:1993:PPC:165231.165264, Owens:2009:BXM:1616077.1616107}.  In this section, % and in Section \ref{sec:restderive},
  we formally derive ConSpec specification for each of these consistency models from the equivalent definitions provided in \cite{Chockler2000, Ahamad:1993:PPC:165231.165264, Owens:2009:BXM:1616077.1616107}. We begin by discussing the  syntax  and notations used by Chockler et al., and how they can be translated to %map to
 ...
 $\mathit{o}^2$ a pair of  operations invoked from %a session $\mathit{st}$ of
 the $i^{th}$  process $ p_i$ in the system, that  read or write to an object $x$. % , such that $\mathit{o}^1$ is a write operation that updates
...
% $\mathit{o}^2$, respectively.
 ...
 equivalent special-purpose LTL operator $F_\mathit{st}$ that denotes an ``eventually" relationship  among operations performed by the client $\mathit{Cl}_i$ with respect to the session $\mathit{st}$ (defined in Section \ref{sec:definitions}).  %Since $\mathit{st}$ orders the operations in terms of invocation times,  Condition 1 can be expressed in terms of an LTL expression that specifies the precedence among events in $\mathit{st}$ as follows.
...
In the postcondition (Condition 2), Chockler et al.  specify a precedence relation $\mathit{o}^1 \xrightarrow{S_p} \mathit{o}^2$ , which  restricts a precedence relation among $\mathit{o}^1$ and $\mathit{o}^2$ in the equivalent legal serialization ${S_p}$ for the given $\sigma |i + w$.   By definition of $S_p$, both the invocation and response for  an operation $\mathit{o}^2$ in $S_p$ must appear after the invocation and response of a preceding operation $\mathit{o}^1$ in $S_p$, i.e., $ \mathit{o}^1 \xrightarrow{S_p} \mathit{o}^2 $ implies $ \mathit{inv}^1 \xrightarrow{S_p} \mathit{inv}^2 $ and $ \mathit{resp}^1 \xrightarrow{S_p} \mathit{resp}^2 $.
  ...
  can be rewritten as $\mathit{Op}^1 F_{S_p} \mathit{Op}^2$, which is syntactically equivalent to $\mathit{Op}^1 F \mathit{Op}^2 \in S_p$. %,  where
...
$\mathit{o} \xrightarrow{\sigma_i} \mathit{o}^{'} $  holds for a given execution $\sigma_i$. Causal consistency specifies: if a transitive closure  $\xRightarrow{\star}$ exists
 ...
 The above postcondition can be expressed in terms of LTL as  $\exists S_p \left( \forall \mathit{Op}^{'} F_{S_p} \mathit{Op}^{''} \right)$, for each client.  %The condition ${\mathit{Op}}^{'}  F_{S_p} {\mathit{Op}}^{''} $ denotes a partial order among ${\mathit{Op}}^{'} $ and ${\mathit{Op}}^{''} $ with respect to each client. Hence,
...
% (refer to Equation \ref{eqn:Strict}).
 ...
 $\mathit{o}^1 \xrightarrow{\sigma_i} \mathit{o}^2$ implies $\mathit{o}^1 \xrightarrow{S_p} \mathit{o}^2$. Condition 2 states: all writes to a given object in a system must follow an identical precedence order in valid legal serializations for all processors in the system. %In this paper, we refer to a processor  $p_i$ as a client $\mathit{Cl}_i$, and the session trace comprising the results observed by  $\mathit{Cl}_i$ is denoted as $\mathit{st}$. Hence, in our case, Condition 1 implies that the order among $w(x,v)$ and $ w^{'}(y,v^{'})$ invoked by in any valid legal serialization $S_p$ comprising operations performed by a  client $\mathit{Cl}_i$ must match the precedence order of $w(x,v)$ and  $w^{'}(y,v^{'})$ in a session trace $\mathit{st}$ observed by $\mathit{Cl}_i$.
  ...
  Let us denote $ \preccurlyeq$ %and $ \preccurlyeq_{{st}^{'}+w}$
