...
if transaction $ {\mathit{ty}}$ reads the result of at least one write operation invoked by a transaction $ {\mathit{tx}}$, this  implies $\mathit{tx} \preccurlyeq \mathit{ty}$, %which we refer to as $\mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty}$,  where the suffix ${\mathit{wr}}$ restricts the partial order $\preccurlyeq$ among write operations in $\mathit{tx}$ and read operations in $\mathit{ty}$, %which 
...
We refer to the partial order relation $\mathit{tx} \preccurlyeq \mathit{ty}$ in Condition 2 as $\mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty}$,  where the suffix ${\mathit{wr}}$ restricts the partial order $\preccurlyeq$ among write operations in $\mathit{tx}$ and read operations in $\mathit{ty}$, %which 
  %precedes the transaction $ {\mathit{tx}}$ in $\preccurlyeq$,  
  i.e.,  $\exists W(x,v)_{\mathit{tx}}, R(x){v^{'}}_{\mathit{ty}} \in  \mathcal{S}_t: %W(x,v)_{\mathit{tx}} \operatorname{\it F}_\mathit{st} R(x){v^{'}}_{\mathit{ty}} \wedge 
   v = v^{'}  \rightarrow \mathit{tx} \preccurlyeq_{\mathit{wr}} \mathit{ty}$. 
  ...
  %precedes the transaction $ {\mathit{tx}}$ in $\preccurlyeq$,  
   ...
   v = v^{'}  \rightarrow \mathit{tx} \prec_{\mathit{wr}} \mathit{ty}$ or $\exists W(x,v)_{\mathit{ty}}, R(x){v^{'}}_{\mathit{tx}} \in  \mathcal{S}_t: %W(x,v)_{\mathit{tx}} \operatorname{\it F}_\mathit{st} R(x){v^{'}}_{\mathit{ty}} \wedge 
   v = v^{'}  \rightarrow \mathit{ty} \prec_{\mathit{wr}} \mathit{tx}$.  The above total order proscribes wr dependency cycles among transactions.  
   
 \section{Specification of Existing Isolation Levels}
  Read committed isolation level is specified as follows. 
   \begin{align}\label{eqn:readcomm}
\begin{split}
 E^{s} =  \; \forall R_{\mathit{ty}}(x){v} \in \mathcal{S}_t. \; \exists \mathit{tx}^{c} \in T: 
\mathit{tx}^{c} \preccurlyeq_{\mathit{wr}} {ty}. %\quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T:  
...
\mathit{tx}^{c} \preccurlyeq_{\mathit{wr}} \mathit{ty} \quad \wedge \quad \mathit{tx} \not \preccurlyeq_{\mathit{wr}} \mathit{tx} \quad \wedge \quad v = v^{''}. %\quad \vee \quad \forall x, \; \mathit{tx}, \mathit{ty} \in T:  
%\mathit{ty} \preccurlyeq_{\mathit{ww}} \mathit{tx}
\end{split}
  \end{align}
  ...
  SI-b: A transaction observes complete effects of transactions that committed before it started.  SI-a can be interpreted as follows.   If a pair of transactions can observe results of each other, they are not concurrent, i.e., $\preccurlyeq$ among a pair of transactions in Definition \ref{def:form0} is a total order $\prec$.  Thus, $\forall \mathit{tx}, \mathit{ty} \in T,  W_{\mathit{tx} }(x, v),  W_{\mathit{ty} }(x, v^{'}), R_{\mathit{tx}}(x){v^{''}}, R_{\mathit{ty}}(x){v^{'''}} \in  \mathcal{S}_t: 
 %W_{\mathit{tx} }(x, v) \operatorname{\it F}_\mathit{st} R_{\mathit{tx}}(x){v^{''}} \quad \wedge \quad 
 v = v^{''}  \quad \wedge \quad %W_{\mathit{ty} }(x, v^{'})  \operatorname{\it F}_\mathit{st} R_{\mathit{tx}}(x){v^{'''}} \quad 
 \wedge \quad  \quad v^{'} = v^{'''} \quad  \Rightarrow  \quad \mathit{tx}^{c} \prec_{\mathit{ww}} \mathit{ty} \quad \wedge \quad \mathit{tx}^{c} \prec_{\mathit{wr}} \mathit{ty} $. SI-b can be interpreted as: if a transaction  ${\mathit{ty}} $ observes values from another transaction  ${\mathit{tx}} $ implies a partial order  $\preccurlyeq$ on  $\mathit{tx}^{c}$ and ${\mathit{ty}} $.  This can be expressed as $\forall \mathit{tx}, \mathit{ty} \in T,  W_{\mathit{tx} }(x, v),  R_{\mathit{ty}}(x){v^{'''}} \in  \mathcal{S}_t: 
 %W_{\mathit{tx} }(x, v) \operatorname{\it F}_\mathit{st} R_{\mathit{ty}}(x){v^{'}}  \quad \wedge \quad 
 v = v^{'''} \quad  \Rightarrow  \quad \mathit{tx}^{c} \preccurlyeq_{\mathit{wr}} \mathit{ty}$. Combining the above expressions, we have: 
   \begin{align}\label{eqn:snap}
\begin{split}
 E^{s} =  \; \forall \mathit{tx}, \mathit{ty} \in T,  W_{\mathit{tx} }(x, v),  W_{\mathit{ty} }(x, v^{'}), R_{\mathit{tx}}(x){v^{''}}, R_{\mathit{ty}}(x){v^{'''}}, R_{\mathit{ty}}(x){v^{''''}} \in  \mathcal{S}_t:  \\
 %W_{\mathit{tx} }(x, v) \operatorname{\it F}_\mathit{st} R_{\mathit{tx}}(x){v^{''}} \\ \quad \wedge \quad 
\left( v = v^{''}  \quad
 \wedge \quad %W_{\mathit{ty} }(x, v^{'})  \operatorname{\it F}_\mathit{st} R_{\mathit{ty}}(x){v^{'''}} \quad \wedge  
  v^{'} = v^{'''}   \quad  \Rightarrow  \quad \mathit{tx}^{c} \prec_{\mathit{ww}} \mathit{ty} % \right)  \\ 
  \quad  
 \wedge %\quad W_{\mathit{tx} }(x, v) \operatorname{\it F}_\mathit{st} R_{\mathit{ty}}(x){v^{'}}  \quad \wedge 
  ...
  \mathit{tx}^{c} \prec_{\mathit{wr}} \mathit{ty} \right) \quad \\
   \wedge \quad  \left( \mathit{tx} \not \prec_{\mathit{wr}} \mathit{tx} \quad \wedge \quad \mathit{ty} \not \prec_{\mathit{wr}} \mathit{ty} \quad \wedge \quad  v = v^{''''} \right).
...
\end{split}
  \end{align}
 \par %In ConSpec, the notation $\mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty}$ restricts the partial order $\preccurlyeq$ among write operations in $\mathit{tx}$ and $\mathit{ty}$, i.e., $\exists \; W(x,v)_{\mathit{tx}}, W(x,v^{'})_{\mathit{ty}}, R(x){v^{''}}_{\mathit{ty}} \in  \mathcal{S}_t: W(x,v)_{\mathit{tx}} \operatorname{\it F}_\mathit{st} W(x,v^{'})_{\mathit{ty}} \wedge W(x,v^{'})_{\mathit{ty}} \operatorname{\it F}_\mathit{st} R(x,v^{''})_{\mathit{ty}} \wedge  v^{''} = v^{'}  \Leftrightarrow \mathit{tx} \preccurlyeq_{\mathit{ww}} \mathit{ty}$. 
