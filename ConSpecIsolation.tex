...
% representing a global execution of all concurrent clients on a given storage
%system,
we say  that   $\mathcal{S}_t$ satisfies a isolation level $\mathcal{I}$
...
%$\left( {\mathcal{O}_{T}}, \preccurlyeq \right)$ over the set ${\mathcal{O}_{T}}$ comprising operations  present in all transactions in $T$, i.e.,  ${\mathcal{O}_{T}} = \bigcup_{{\mathit{tx}} \in T} \{ o \mid o \in {\mathit{tx}} \},$
 such that 1) if  an operation $o_{\mathit{tx}}$ on an object $x$ from ${\mathit{tx}} $ precedes an operation $o_{\mathit{ty}}$ from ${\mathit{ty}}$ on  $x$ in $\preccurlyeq$, 
 then  no operation from ${\mathit{tx}} $ can return the result of any operation from ${\mathit{ty}}$ on $x$, i.e., $\not\exists \; x, \; o(x)_{\mathit{tx}}(v)^{'},  \;  o(x, v^{'} )^{'}_{\mathit{ty}} \in \mathcal{S}_t: \;  v = v^{'}$,   %, in $\preccurlyeq$, invocation and response of all operations in ${\mathit{tx}}$ precede invocation and response of all operations in ${\mathit{ty}} $ on a common object   
